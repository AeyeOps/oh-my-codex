...
- When deep-interview is active in OMX CLI/runtime, ask interview rounds via `omx question`; if `omx question` renders inline in the current terminal (for example, an interactive native Windows session without tmux), finish that inline prompt and read the JSON answer before continuing; after launching `omx question` in a background terminal, wait for that terminal to finish and read the JSON answer before continuing; do not substitute `request_user_input` or ad hoc plain-text questioning, and respect Stop-hook blocking while a deep-interview question obligation is pending.
