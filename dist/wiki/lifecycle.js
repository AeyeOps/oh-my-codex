...
import { appendLogUnsafe, getWikiDir, listPages, readAllPages, readIndex, readPage, updateIndexUnsafe, withWikiLock, writePageUnsafe, } from './storage.js';
import { DEFAULT_WIKI_CONFIG, WIKI_SCHEMA_VERSION } from './types.js';
...
function feedProjectMemory(root) {
    try {
        ...
        if (!existsSync(projectMemoryPath))
            return;
        const parsed = JSON.parse(readFileSync(projectMemoryPath, 'utf8'));
        ...
        if (Array.isArray(parsed.directives) && parsed.directives.length > 0) {
            ...
            for (const directive of parsed.directives.slice(0, 20)) {
                const content = typeof directive.directive === 'string'
