...
export function validateStateModeSegment(mode) {
...
export function validateStateFileName(fileName) {
...
function convertWindowsToWslPath(raw) {
...
function convertWslToWindowsPath(raw) {
...
export function resolveWorkingDirectoryForState(workingDirectory) {
...
function parseAllowedWorkingDirectoryRoots() {
...
function enforceWorkingDirectoryPolicy(resolvedWorkingDirectory) {
    const roots = parseAllowedWorkingDirectoryRoots();
    if (roots.length === 0)
        return;
    const allowed = roots.some((root) => isWithinRoot(resolvedWorkingDirectory, root));
    if (!allowed) {
        throw new Error(`workingDirectory "${resolvedWorkingDirectory}" is outside allowed roots (${WORKDIR_ALLOWLIST_ENV})`);
    }
}
export function getBaseStateDir(workingDirectory) {
...
export async function readCurrentSessionId(workingDirectory) {
    ...
    if (envSessionId) {
        ...
        if (existsSync(envScopedDir))
            return envSessionId;
    }
    return (await readUsableSessionState(cwd))?.session_id;
}
export async function resolveStateScope(workingDirectory, explicitSessionId) {
 ...
 */
export async function getReadScopedStateDirs(workingDirectory, explicitSessionId) {
    ...
    if (scope.source === 'explicit') {
        if (existsSync(scope.stateDir))
            return [scope.stateDir];
        return [scope.stateDir, getBaseStateDir(workingDirectory)];
    }
    return [scope.stateDir, getBaseStateDir(workingDirectory)];
}
export async function getReadScopedStatePaths(mode, workingDirectory, explicitSessionId) {
...
export async function getReadScopedStateFilePaths(fileName, workingDirectory, explicitSessionId, options = {}) {
...
export async function getAllSessionScopedStatePaths(mode, workingDirectory) {
    const sessionDirs = await getAllSessionScopedStateDirs(workingDirectory);
    const fileName = getStateFilename(mode);
    return sessionDirs.map((dir) => join(dir, fileName));
}
export async function getAllScopedStatePaths(mode, workingDirectory) {
    return [
        getStatePath(mode, workingDirectory),
        ...(await getAllSessionScopedStatePaths(mode, workingDirectory)),
    ];
}
export async function getAllSessionScopedStateDirs(workingDirectory) {
...
export async function listModeStateFilesWithScopePreference(workingDirectory, explicitSessionId) {
    ...
    for (const dir of [...readDirs].reverse()) {
        const scope = dir === rootDir ? 'root' : 'session';
