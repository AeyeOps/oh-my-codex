...
async function withAmbientTmuxEnv(env, run) {
    ...
    if (typeof env.TMUX === 'string')
        process.env.TMUX = env.TMUX;
    else
        delete process.env.TMUX;
    if (typeof env.TMUX_PANE === 'string')
        process.env.TMUX_PANE = env.TMUX_PANE;
    else
...
describe('state-server directory initialization', () => {
    ...
    it('bootstraps state-tool tmux-hook from the current tmux pane when available', async () => {
        ...
        try {
            ...
            const tmuxConfig = JSON.parse(await readFile(tmuxHookConfig, 'utf-8'));
    ...
    it('writes and reads deep-interview state', async () => {
        ...
        try {
            ...
            assert.deepEqual(JSON.parse(writeResponse.content[0]?.text || '{}'), {
                success: true,
                mode: 'deep-interview',
            ...
            const readBody = JSON.parse(readResponse.content[0]?.text || '{}');
    ...
    it('creates session-scoped state directory when session_id is provided', async () => {
        ...
        try {
            ...
            assert.deepEqual(JSON.parse(response.content[0]?.text || '{}'), { statuses: {} });
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it('state_write accepts canonical lifecycle_outcome while preserving compatibility run_outcome', async () => {
        ...
        try {
            ...
            const state = JSON.parse(await readFile(join(wd, '.omx', 'state', 'autopilot-state.json'), 'utf-8'));
    ...
    it('syncs canonical skill-active state for tracked mode writes and clears', async () => {
        ...
        try {
            ...
            const clearedCanonical = JSON.parse(await readFile(canonicalPath, 'utf-8'));
    ...
    it('writes a session-scoped inactive tombstone when clearing a mode under an active session', async () => {
        ...
        try {
            ...
            await writeFile(join(stateDir, 'deep-interview-state.json'), JSON.stringify({ active: true, mode: 'deep-interview', current_phase: 'legacy-root' }, null, 2));
            await writeFile(join(sessionDir, 'deep-interview-state.json'), JSON.stringify({ active: true, mode: 'deep-interview', current_phase: 'session-active' }, null, 2));
            await handleStateToolCall({
                params: {
                    name: 'state_clear',
                    arguments: {
            ...
            const sessionState = JSON.parse(await readFile(join(sessionDir, 'deep-interview-state.json'), 'utf-8'));
    ...
    it('denies unsupported overlaps without writing the requested mode state', async () => {
        ...
        try {
            ...
            const canonical = JSON.parse(await readFile(join(wd, '.omx', 'state', 'sessions', 'sess-deny', 'skill-active-state.json'), 'utf-8'));
    ...
    it('allows ultrawork when canonical session state is stricter than mode files', async () => {
        ...
        try {
            ...
            await writeFile(join(wd, '.omx', 'state', 'team-state.json'), JSON.stringify({ active: true, mode: 'team', current_phase: 'running' }, null, 2));
            ...
            await writeFile(join(wd, '.omx', 'state', 'sessions', 'sess-canonical-deny', 'skill-active-state.json'), JSON.stringify({
                version: 1,
                active: true,
                skill: 'team',
            ...
            assert.equal(existsSync(join(wd, '.omx', 'state', 'sessions', 'sess-canonical-deny', 'ultrawork-state.json')), true);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it('propagates root clears into inherited session canonical copies', async () => {
        ...
        try {
            ...
            const sessionCanonical = JSON.parse(await readFile(join(wd, '.omx', 'state', 'sessions', 'sess-root-clear', 'skill-active-state.json'), 'utf-8'));
    ...
    it('preserves root-scoped team state when session-scoped ralph is added via state_write', async () => {
        ...
        try {
            ...
            const rootCanonical = JSON.parse(await readFile(join(wd, '.omx', 'state', 'skill-active-state.json'), 'utf-8'));
            assert.deepEqual(rootCanonical.active_skills?.map(({ skill, phase, session_id }) => ({
                skill,
                phase,
                session_id,
            })), [{ skill: 'team', phase: 'running', session_id: undefined }]);
            const sessionCanonical = JSON.parse(await readFile(join(wd, '.omx', 'state', 'sessions', 'sess-team-ralph', 'skill-active-state.json'), 'utf-8'));
    ...
    it('rejects standalone overlaps without mutating canonical state', async () => {
        ...
        try {
            ...
            const body = JSON.parse(invalidTeamWrite.content[0]?.text || '{}');
            assert.match(body.error || '', /omx state/i);
            assert.match(body.error || '', /omx_state\.\*/i);
            const canonical = JSON.parse(await readFile(join(wd, '.omx', 'state', 'sessions', 'sess-standalone', 'skill-active-state.json'), 'utf-8'));
            assert.deepEqual(canonical.active_skills?.map(({ skill, phase, session_id }) => ({
                skill,
                phase,
                session_id,
    ...
    it('auto-completes deep-interview when starting ralplan and returns transition messaging', async () => {
        ...
        try {
            ...
            await writeFile(join(wd, '.omx', 'state', 'sessions', 'sess-handoff', 'deep-interview-state.json'), JSON.stringify({ active: true, mode: 'deep-interview', current_phase: 'intent-first' }, null, 2));
            const response = await handleStateToolCall({
                params: {
                    name: 'state_write',
                    arguments: {
            ...
            const completed = JSON.parse(await readFile(join(wd, '.omx', 'state', 'sessions', 'sess-handoff', 'deep-interview-state.json'), 'utf-8'));
    ...
    it('does not auto-complete existing workflow state when tracked write validation fails', async () => {
        ...
        try {
            ...
            await writeFile(join(wd, '.omx', 'state', 'sessions', 'sess-invalid', 'ralplan-state.json'), JSON.stringify({ active: true, mode: 'ralplan', current_phase: 'planning' }, null, 2));
            const denied = await handleStateToolCall({
                params: {
                    name: 'state_write',
                    arguments: {
            ...
            const ralplanState = JSON.parse(await readFile(join(wd, '.omx', 'state', 'sessions', 'sess-invalid', 'ralplan-state.json'), 'utf-8'));
...
//# sourceMappingURL=state-server.test.js.map