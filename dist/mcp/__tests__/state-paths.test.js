...
describe('state paths', () => {
    ...
    it('enforces OMX_MCP_WORKDIR_ROOTS allowlist when configured', async () => {
        ...
        try {
            assert.equal(resolveWorkingDirectoryForState(join(allowedRoot, 'nested')), join(allowedRoot, 'nested'));
            assert.throws(() => resolveWorkingDirectoryForState(disallowedRoot), /outside allowed roots \(OMX_MCP_WORKDIR_ROOTS\)/);
        }
        finally {
    ...
    it('builds session state paths', () => {
        ...
        assert.equal(getStatePath('ralph', '/repo', 'sess1'), '/repo/.omx/state/sessions/sess1/ralph-state.json');
        assert.equal(getStateFilePath('hud-state.json', '/repo', 'sess1'), '/repo/.omx/state/sessions/sess1/hud-state.json');
    });
    ...
    it('prefers OMX_SESSION_ID over stale session.json when resolving current session id', async () => {
        ...
        try {
            ...
            if (typeof previousSessionId === 'string')
                process.env.OMX_SESSION_ID = previousSessionId;
            else
