...
import { analyzeDuplicateSiblingState, MCP_ENTRYPOINT_MARKER_ENV, extractMcpEntrypointMarker, isParentProcessAlive, parseProcessTable, resolveCurrentMcpEntrypointMarker, shouldAutoStartMcpServer, shouldSelfExitForDuplicateSibling, } from '../bootstrap.js';
const ALL_SERVERS = [
    'state',
    'memory',
    'code_intel',
...
describe('mcp duplicate sibling detection', () => {
    it('extracts same-entrypoint markers from command lines', () => {
        assert.equal(extractMcpEntrypointMarker('node /tmp/oh-my-codex/dist/mcp/state-server.js'), 'state-server.js');
        assert.equal(extractMcpEntrypointMarker('node C:\\\\tmp\\\\oh-my-codex\\\\dist\\\\mcp\\\\trace-server.ts'), 'trace-server.ts');
        assert.equal(extractMcpEntrypointMarker('node /tmp/dist/cli/omx.js mcp-serve state'), 'state-server.js');
        assert.equal(extractMcpEntrypointMarker('node /tmp/dist/cli/omx.js mcp-serve code-intel'), 'code-intel-server.js');
    ...
    it('prefers an explicit MCP entrypoint marker over argv[1]', () => {
        assert.equal(resolveCurrentMcpEntrypointMarker({ [MCP_ENTRYPOINT_MARKER_ENV]: 'trace-server.js' }, '/repo/dist/cli/omx.js'), 'trace-server.js');
    });
    it('falls back to argv[1] when no explicit MCP entrypoint marker is set', () => {
        assert.equal(resolveCurrentMcpEntrypointMarker({}, '/repo/dist/mcp/state-server.js'), 'state-server.js');
    });
    it('parses ps output into process table entries', () => {
        assert.deepEqual(parseProcessTable('101 55 node /tmp/dist/mcp/state-server.js\n'), [{ pid: 101, ppid: 55, command: 'node /tmp/dist/mcp/state-server.js' }]);
    });
    it('treats a single instance as unique and no-op', () => {
        const observation = analyzeDuplicateSiblingState([{ pid: 101, ppid: 55, command: 'node /tmp/dist/mcp/state-server.js' }], 101, 55, 'state-server.js');
        assert.equal(observation.status, 'unique');
        assert.deepEqual(observation.matchingPids, [101]);
        assert.equal(shouldSelfExitForDuplicateSibling(observation, 10_000, 9_000, null), false);
    });
    ...
    it('only lets older duplicates self-exit after the conservative grace window before traffic', () => {
        ...
        assert.equal(shouldSelfExitForDuplicateSibling(observation, 10_500, 9_000, null), false);
        assert.equal(shouldSelfExitForDuplicateSibling(observation, 11_100, 9_000, null), true);
    });
    it('does not self-exit after post-duplicate traffic until the duplicate has remained safely idle long enough', () => {
        ...
        assert.equal(shouldSelfExitForDuplicateSibling(observation, 35_000, 1_000, 10_000), false);
        assert.equal(shouldSelfExitForDuplicateSibling(observation, 311_000, 1_000, 10_000), true);
    });
    it('uses the duplicate grace window when last traffic predates duplicate observation', () => {
        ...
        assert.equal(shouldSelfExitForDuplicateSibling(observation, 10_500, 9_000, 1_000), false);
        assert.equal(shouldSelfExitForDuplicateSibling(observation, 11_100, 9_000, 1_000), true);
    });
    it('treats future or non-finite traffic timestamps as a do-not-self-kill marker', () => {
        ...
        assert.equal(shouldSelfExitForDuplicateSibling(observation, 499_000, 200_000, 500_000), false);
        assert.equal(shouldSelfExitForDuplicateSibling(observation, 900_000, 200_000, Number.NaN), false);
    });
    it('treats ambiguous duplicate state as no-op', () => {
        const missingSelf = analyzeDuplicateSiblingState([{ pid: 140, ppid: 55, command: 'node /tmp/dist/mcp/state-server.js' }], 101, 55, 'state-server.js');
        const mismatchedSelfMarker = analyzeDuplicateSiblingState([
            { pid: 101, ppid: 55, command: 'node /tmp/dist/mcp/memory-server.js' },
            { pid: 140, ppid: 55, command: 'node /tmp/dist/mcp/state-server.js' },
        ], 101, 55, 'state-server.js');
        assert.equal(missingSelf.status, 'ambiguous');
        assert.equal(mismatchedSelfMarker.status, 'ambiguous');
        assert.equal(shouldSelfExitForDuplicateSibling(missingSelf, 50_000, 10_000, null), false);
    });
});
