...
describe('trace-server session-scoped mode discovery', () => {
    it('includes mode events from session-scoped state files', async () => {
        ...
        try {
            ...
            assert.ok(events.some((e) => e.event === 'mode_start' && e.mode === 'ralph'));
            assert.ok(events.some((e) => e.event === 'mode_end' && e.mode === 'ralph'));
            assert.ok(events.every((e) => e.details?.scope === 'session'));
        }
        finally {
    ...
    it('does not include unrelated session mode events when a current session is active', async () => {
        ...
        try {
            ...
            const events = await readModeEvents(wd);
            assert.ok(events.some((e) => e.timestamp === '2020-01-01T00:00:00.000Z'));
            assert.equal(events.some((e) => e.timestamp === '2020-01-02T00:00:00.000Z'), false);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
...
describe('trace-server log readers', () => {
    it('returns only the most recent entries when last is provided', async () => {
        ...
        try {
            ...
            assert.deepEqual(result.map((e) => e.timestamp), entries.slice(-75).map(e => e.timestamp));
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
...
//# sourceMappingURL=trace-server.test.js.map