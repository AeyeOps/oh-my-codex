...
import { appendLog, deletePage, ingestKnowledge, lintWiki, listPages, normalizeWikiPageName, queryWiki, readIndex, readPage, titleToSlug, updateIndexUnsafe, withWikiLock, } from '../wiki/index.js';
const server = new Server({ name: 'omx-wiki', version: '0.1.0' }, { capabilities: { tools: {} } });
...
function resolveRoot(args) {
    return resolveWorkingDirectoryForState(typeof args.workingDirectory === 'string' ? args.workingDirectory : undefined);
}
        ...
        switch (name) {
            case 'wiki_ingest': {
                const result = ingestKnowledge(root, {
                    ...
                    category: String(args.category || 'reference'),
            ...
            case 'wiki_add': {
                ...
                const result = ingestKnowledge(root, {
                    ...
                    content: String(args.content || ''),
                    tags: Array.isArray(args.tags) ? args.tags.map(String) : [],
                    category: typeof args.category === 'string' ? args.category : 'reference',
                });
                appendLog(root, {
            ...
            case 'wiki_read': {
                ...
                if (!page)
                    return errorText('Wiki page not found');
                return text(page);
            }
            case 'wiki_delete': {
                ...
                if (!deleted)
                    return errorText(`Wiki page not found or reserved: ${filename}`);
                appendLog(root, {
                    timestamp: new Date().toISOString(),
                    operation: 'delete',
...
//# sourceMappingURL=wiki-server.js.map