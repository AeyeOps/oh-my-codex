...
export async function runTscDiagnostics(target, projectDir, severity, runCommand = exec) {
...
export function buildAstGrepRunArgs(pattern, language, options) {
...
export async function handleCodeIntelToolCall(request) {
    ...
    switch (name) {
        ...
        case 'lsp_find_references': {
            ...
            const line = a.line;
            const char = a.character;
            const includeDeclaration = a.includeDeclaration;
            ...
            try {
                ...
                const declarationLines = new Set(extractSymbols(content)
                ...
                const filteredRefs = effectiveIncludeDeclaration
                    ? refs
                    : refs.filter((ref) => {
                        if (resolve(ref.file) !== normalizedTargetFile)
                            return true;
