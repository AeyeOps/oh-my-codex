...
export function resolveCurrentMcpEntrypointMarker(env = process.env, argv1 = process.argv[1]) {
    const explicitMarker = extractMcpEntrypointMarker(env[MCP_ENTRYPOINT_MARKER_ENV] ?? '');
    if (explicitMarker)
...
export function listProcessTable(readPs = execFileSync) {
...
export function analyzeDuplicateSiblingState(processes, currentPid, currentParentPid, currentEntrypoint) {
...
function resolveLifecycleTimingConfig(env) {
    return {
        parentWatchdogIntervalMs: readPositiveIntegerEnv(env, PARENT_WATCHDOG_INTERVAL_ENV, DEFAULT_PARENT_WATCHDOG_INTERVAL_MS),
        duplicateSiblingWatchdogIntervalMs: readPositiveIntegerEnv(env, DUPLICATE_SIBLING_WATCHDOG_INTERVAL_ENV, DEFAULT_DUPLICATE_SIBLING_WATCHDOG_INTERVAL_MS),
        duplicateSiblingPreTrafficGraceMs: readPositiveIntegerEnv(env, DUPLICATE_SIBLING_PRE_TRAFFIC_GRACE_ENV, DEFAULT_DUPLICATE_SIBLING_PRE_TRAFFIC_GRACE_MS),
        duplicateSiblingPostTrafficIdleMs: readPositiveIntegerEnv(env, DUPLICATE_SIBLING_POST_TRAFFIC_IDLE_ENV, DEFAULT_DUPLICATE_SIBLING_POST_TRAFFIC_IDLE_MS),
    };
}
export function shouldSelfExitForDuplicateSibling(observation, nowMs, duplicateObservedAtMs, lastTrafficAtMs, preTrafficGraceMs = DEFAULT_DUPLICATE_SIBLING_PRE_TRAFFIC_GRACE_MS, postTrafficIdleMs = DEFAULT_DUPLICATE_SIBLING_POST_TRAFFIC_IDLE_MS) {
...
export function isParentProcessAlive(parentPid, signalProcess = process.kill) {
...
export function shouldAutoStartMcpServer(server, env = process.env) {
...
export function autoStartStdioMcpServer(serverName, server, env = process.env) {
    ...
    const trackedEntrypoint = resolveCurrentMcpEntrypointMarker(env, process.argv[1] ?? '');
    let lastTrafficAtMs = null;
    let duplicateObservedAtMs = null;
    const logLifecycle = (message, error) => {
    ...
    const duplicateSiblingWatchdog = trackedParentPid > 1 && trackedEntrypoint
        ? setInterval(() => {
            ...
            const observation = analyzeDuplicateSiblingState(processes, process.pid, trackedParentPid, trackedEntrypoint);
            if (observation.status !== 'older_duplicate') {
                duplicateObservedAtMs = null;
                return;
            }
            duplicateObservedAtMs ??= Date.now();
            if (!shouldSelfExitForDuplicateSibling(observation, Date.now(), duplicateObservedAtMs, lastTrafficAtMs, lifecycleTiming.duplicateSiblingPreTrafficGraceMs, lifecycleTiming.duplicateSiblingPostTrafficIdleMs)) {
                return;
            }
            void shutdown(lastTrafficAtMs !== null && lastTrafficAtMs > duplicateObservedAtMs
