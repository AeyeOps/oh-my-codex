...
`;
export async function tmuxHookCommand(args) {
...
function omxDir(cwd = process.cwd()) {
    return join(cwd, '.omx');
}
function tmuxHookConfigPath(cwd = process.cwd()) {
    return join(omxDir(cwd), 'tmux-hook.json');
}
function tmuxHookStatePath(cwd = process.cwd()) {
    return join(omxDir(cwd), 'state', 'tmux-hook-state.json');
}
function tmuxHookLogPath(cwd = process.cwd()) {
...
async function readValidatedConfig(cwd = process.cwd()) {
...
async function loadConfigForCommand(commandName, cwd = process.cwd()) {
    const configPath = tmuxHookConfigPath(cwd);
    let initResult = null;
    if (!existsSync(configPath)) {
...
function resolveValidateTarget(config) {
...
async function initTmuxHookConfig(opts) {
    ...
    const initial = {
        ...DEFAULT_CONFIG,
        target: detected?.target ?? { type: 'pane', value: 'replace-with-tmux-pane-id' },
    };
...
export async function ensureTmuxHookInitialized(cwd = process.cwd()) {
    try {
...
async function showTmuxHookStatus() {
...
async function validateTmuxHookConfig() {
    const cwd = process.cwd();
...
async function testTmuxHook(args) {
    const cwd = process.cwd();
...
//# sourceMappingURL=tmux-hook.js.map