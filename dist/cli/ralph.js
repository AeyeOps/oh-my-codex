...
function isStoryMarkedPassedOrCompleted(story) {
...
function hasApprovedArchitectValidation(story) {
...
function describeStory(story, index) {
...
function readAndValidateRequiredRalphPrdJson(cwd) {
    ...
    for (const [index, story] of parsed.userStories.entries()) {
        if (!isRecord(story))
            continue;
        if (!isStoryMarkedPassedOrCompleted(story))
            continue;
        if (hasApprovedArchitectValidation(story))
            continue;
...
export function isRalphPrdMode(args) {
    return args.some((arg) => arg === '--prd' || arg.startsWith('--prd='));
}
export function assertRequiredRalphPrdJson(cwd, args) {
    if (!isRalphPrdMode(args))
        return;
    const requiredPath = join(cwd, REQUIRED_RALPH_PRD_JSON_PATH);
...
export function extractRalphTaskDescription(args, fallbackTask) {
    const words = [];
    let i = 0;
    while (i < args.length) {
        const token = args[i];
...
function buildRalphApprovedContextLines(approvedHint) {
    if (!approvedHint)
        return [];
    const lines = [
...
export function normalizeRalphCliArgs(args) {
    const normalized = [];
    let i = 0;
    while (i < args.length) {
        const token = args[i];
...
export function filterRalphCodexArgs(args) {
    const filtered = [];
    for (const token of args) {
        if (RALPH_OMX_FLAGS.has(token.toLowerCase()))
            continue;
        filtered.push(token);
    }
    return filtered;
}
export function buildRalphChangedFilesSeedContents() {
    return [
...
export function buildRalphAppendInstructions(task, options) {
    return [
...
async function writeRalphSessionFiles(cwd, task, options) {
    ...
    await writeFile(instructionsPath, `${buildRalphAppendInstructions(task, { changedFilesPath: '.omx/ralph/changed-files.txt', noDeslop: options.noDeslop, approvedHint: options.approvedHint ?? null })}\n`);
    return { instructionsPath, changedFilesPath: '.omx/ralph/changed-files.txt' };
}
export async function ralphCommand(args) {
    ...
    try {
        ...
        if (typeof previousAppendixEnv === 'string')
            process.env[RALPH_APPEND_ENV] = previousAppendixEnv;
        else
            delete process.env[RALPH_APPEND_ENV];
    }
}
