...
function formatEntry(entry) {
    ...
    if (entry.canonical)
        parts.push(`-> ${entry.canonical}`);
    return `- ${parts.join("  ")}`;
}
function printHumanList(contract) {
    console.log(`OMX catalog ${contract.version}`);
    console.log(`Skills: ${contract.counts.skillCount} (${contract.counts.activeSkillCount} active)`);
    for (const skill of contract.skills)
        console.log(formatEntry(skill));
    console.log(`Agents: ${contract.counts.promptCount} (${contract.counts.activeAgentCount} active)`);
    for (const agent of contract.agents)
        console.log(formatEntry(agent));
}
