...
import { isInstallVersionBump, readUserInstallStamp, writeUserInstallStamp, } from "./update.js";
...
export async function ensureInstallStamp() {
    try {
        const currentVersion = await getCurrentVersion();
        if (!currentVersion)
            return;
        const stamp = await readUserInstallStamp();
        if (!isInstallVersionBump(currentVersion, stamp))
            return;
        ...
        console.log(`[omx] Detected oh-my-codex v${stampVersion}. Run \`omx setup\` or \`omx update\` to apply changes.`);
    }
    catch {
