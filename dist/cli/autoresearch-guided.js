...
import { isLaunchReadyEvaluatorCommand, writeAutoresearchDeepInterviewArtifacts, } from "./autoresearch-intake.js";
...
async function promptWithDefault(io, prompt, currentValue, structuredQuestion) {
...
async function promptAction(io, launchReady, structuredQuestion) {
    if (structuredQuestion) {
        const response = await structuredQuestion({
            header: "Deep Interview / Autoresearch",
        ...
        const answerValue = typeof response.answer.value === "string"
            ...
            : "";
        if (answerValue === "launch")
            return "launch";
        if (answerValue === "refine")
            return "refine";
...
function createStructuredQuestionAsker(repoRoot) {
    return (input) => runDeepInterviewQuestion({
        header: input.header,
...
async function ensureStructuredQuestionFallbackAllowed(repoRoot) {
    const policy = await evaluateQuestionPolicy({ cwd: repoRoot });
    if (policy.allowed || policy.fallbackAllowed !== false)
        return;
    throw new OmxQuestionError(policy.code ?? "question_policy_denied", policy.message ?? "Structured questions are unavailable in the current OMX workflow context.");
}
function shouldFallbackFromStructuredQuestion(error) {
    if (error instanceof OmxQuestionError) {
        if (error.code === "worker_blocked"
...
export function buildAutoresearchDeepInterviewPrompt(seedInputs = {}) {
    const seedLines = [
...
export async function runAutoresearchNoviceBridge(repoRoot, seedInputs = {}, io = createQuestionIO(), structuredQuestion) {
    if (!process.stdin.isTTY) {
        throw new Error("Guided setup requires an interactive terminal. Use `--topic/--evaluator/--keep-policy/--slug` to seed deep-interview intake before launching `$autoresearch`.");
    }
    ...
    const withStructuredQuestionFallback = async (operation) => {
        if (!activeStructuredQuestion)
            return operation();
        try {
            return await operation(activeStructuredQuestion);
        }
        catch (error) {
            if (!shouldFallbackFromStructuredQuestion(error))
                throw error;
            activeStructuredQuestion = undefined;
            if (!warnedAboutStructuredQuestionFallback) {
                warnedAboutStructuredQuestionFallback = true;
                console.warn(`[omx] warning: structured question UI unavailable (${error instanceof Error ? error.message : String(error)}). Falling back to plain terminal prompts.`);
            }
            return operation();
        }
    };
    try {
        while (true) {
            topic = await withStructuredQuestionFallback((question) => promptWithDefault(io, "Research topic/goal", topic, question));
            if (!topic) {
                throw new Error("Research topic is required.");
            }
            const evaluatorIntent = await withStructuredQuestionFallback((question) => promptWithDefault(io, "\nHow should OMX judge success? Describe it in plain language", topic, question));
            evaluatorCommand = await withStructuredQuestionFallback((question) => promptWithDefault(io, "\nEvaluator command (leave placeholder to refine further; must output {pass:boolean, score?:number} JSON before launch)", evaluatorCommand ||
                `TODO replace with evaluator command for: ${evaluatorIntent}`, question));
            const keepPolicyInput = await withStructuredQuestionFallback((question) => promptWithDefault(io, "\nKeep policy [score_improvement/pass_only]", keepPolicy, question));
            keepPolicy =
                keepPolicyInput.trim().toLowerCase() === "pass_only"
                    ? "pass_only"
                    : "score_improvement";
            slug = await withStructuredQuestionFallback((question) => promptWithDefault(io, "\nMission slug", slug || slugifyMissionName(topic), question));
            ...
            const action = await withStructuredQuestionFallback((question) => promptAction(io, deepInterview.launchReady, question));
            if (action === "refine")
                continue;
...
export async function guidedAutoresearchSetup(repoRoot) {
    ...
    return runAutoresearchNoviceBridge(repoRoot, {}, createQuestionIO(), createStructuredQuestionAsker(repoRoot));
}
