...
function isManagedAgentsInitFile(content) {
    return content.includes(MANAGED_MARKER);
}
function extractManualSection(existingContent, fallbackBody) {
    if (!existingContent)
    ...
    if (start === -1 || end === -1 || end < start)
        return fallbackBody.trim();
    return (existingContent.slice(start + MANUAL_START.length, end).trim() ||
        fallbackBody.trim());
}
function wrapManagedContent(body, manualBody) {
    return `${MANAGED_MARKER}\n${body.trimEnd()}\n\n${MANUAL_START}\n${manualBody.trim()}\n${MANUAL_END}\n`;
}
export function applyProjectScopePathRewritesToAgentsTemplate(content) {
    return content.replaceAll("~/.codex", "./.codex");
}
async function readProjectRootTemplate() {
    const pkgRoot = getPackageRoot();
    const templatePath = join(pkgRoot, "templates", "AGENTS.md");
    return readFile(templatePath, "utf-8");
}
export async function renderManagedProjectRootAgents(existingContent) {
...
function formatList(items, suffix = "", limit = DEFAULT_LIST_LIMIT) {
    if (items.length === 0)
        return ["- None"];
...
function renderParentReference(dir, assumeParentAgents = false) {
    const parentAgentsPath = join(dirname(dir), "AGENTS.md");
    if (!assumeParentAgents && !existsSync(parentAgentsPath))
        return "";
    const relativePath = relative(dir, parentAgentsPath).replaceAll("\\", "/");
    return `<!-- Parent: ${relativePath} -->\n`;
}
export async function renderManagedDirectoryAgents(dir, existingContent, assumeParentAgents = false) {
...
async function ensureBackup(destinationPath, backupRoot, dryRun) {
...
function resolveTargetDirectories(targetDir) {
...
async function syncManagedAgentsFile(destinationPath, content, options, summary, backupRoot, skipReason) {
...
export async function agentsInit(options = {}) {
    ...
    if (!targetStat.isDirectory())
        throw new Error(`agents-init target must be a directory: ${requestedTarget}`);
    ...
    for (let index = 0; index < plannedDirs.length; index += 1) {
        ...
        const decision = await syncManagedAgentsFile(destinationPath, content, { dryRun, force, verbose }, summary, backupRoot, rootOverlayRisk
...
//# sourceMappingURL=agents-init.js.map