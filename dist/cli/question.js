...
import { createQuestionRecord, markQuestionTerminalError, markQuestionPrompting, waitForQuestionTerminalState, } from '../question/state.js';
...
function parseQuestionWaitTimeoutMs(env = process.env) {
    const raw = String(env.OMX_QUESTION_WAIT_TIMEOUT_MS ?? '').trim();
    if (!raw)
        return DEFAULT_QUESTION_WAIT_TIMEOUT_MS;
...
function parseQuestionArgs(args) {
    const parsed = { help: false, json: false, ui: false };
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (arg === '--state-path') {
            const next = args[index + 1];
            if (!next)
                throw new Error('Missing path value after --state-path');
...
function printJson(payload, compact) {
    console.log(JSON.stringify(payload, null, compact ? 0 : 2));
}
function extractErrorMessage(error) {
    return error instanceof Error ? error.message : String(error);
}
function createJsonSafeInlineQuestionOutput() {
    return {
        get isTTY() {
...
export async function questionCommand(args) {
    ...
    if (parsed.ui) {
        if (!parsed.statePath)
            throw new Error('--ui requires --state-path');
        await runQuestionUi(parsed.statePath);
        return;
    }
    if (!parsed.input)
        throw new Error('omx question requires --input in normal mode');
    let rawInput;
    try {
    ...
    try {
        ...
        if (renderer.renderer === 'inline-tty') {
            await runQuestionUi(recordPath, parsed.json ? { output: createJsonSafeInlineQuestionOutput() } : {});
        }
        finalRecord = await waitForQuestionTerminalState(recordPath, {
            ...
            rendererDeathMessage: (currentRecord) => (`Question renderer ${currentRecord.renderer?.renderer ?? renderer.renderer} ${currentRecord.renderer?.target ?? renderer.target} exited before answering.`),
        });
    }
    catch (error) {
        const message = extractErrorMessage(error);
        await markQuestionTerminalError(recordPath, 'error', 'question_runtime_failed', message);
        printJson({
            ok: false,
