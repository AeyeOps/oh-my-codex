...
import { SPARKSHELL_BIN_ENV as SPARKSHELL_BIN_ENV_SHARED, getPackageVersion, hydrateNativeBinary, resolveLinuxNativeLibcPreference, resolveCachedNativeBinaryCandidatePaths, } from './native-assets.js';
...
export function sparkshellBinaryName(platform = process.platform) {
    return platform === 'win32' ? 'omx-sparkshell.exe' : 'omx-sparkshell';
}
export function packagedSparkShellBinaryPath(packageRoot = getPackageRoot(), platform = process.platform, arch = osArch(), libc) {
...
export function packagedSparkShellBinaryCandidatePaths(packageRoot = getPackageRoot(), platform = process.platform, arch = osArch(), env = process.env, linuxLibcPreference) {
    const candidates = [];
    if (platform === 'linux') {
...
export function repoLocalSparkShellBinaryPath(packageRoot = getPackageRoot(), platform = process.platform) {
    return join(packageRoot, 'target', 'release', sparkshellBinaryName(platform));
}
export function nestedRepoLocalSparkShellBinaryPath(packageRoot = getPackageRoot(), platform = process.platform) {
    return join(packageRoot, 'native', 'omx-sparkshell', 'target', 'release', sparkshellBinaryName(platform));
}
export function resolveSparkShellBinaryPath(options = {}) {
    const { cwd = process.cwd(), env = process.env, packageRoot = getPackageRoot(), platform = process.platform, arch = osArch(), linuxLibcPreference, exists = existsSync, } = options;
    ...
    throw new Error(`[sparkshell] native binary not found. Checked ${packagedCandidates.join(', ')}, ${repoLocal}, and ${nestedRepoLocal}. `
        + `Set ${OMX_SPARKSHELL_BIN_ENV} to override the path.`);
}
export async function resolveSparkShellBinaryPathWithHydration(options = {}) {
    const { cwd = process.cwd(), env = process.env, packageRoot = getPackageRoot(), platform = process.platform, arch = osArch(), linuxLibcPreference, exists = existsSync, } = options;
    const override = env[OMX_SPARKSHELL_BIN_ENV]?.trim();
    if (override) {
        return isAbsolute(override) ? override : resolve(cwd, override);
    ...
    if (hydrated)
        return hydrated;
    throw new Error(`[sparkshell] native binary not found. Checked cached/native candidates under ${packageRoot}, ${repoLocal}, and ${nestedRepoLocal}. `
        + `Reconnect to the network so OMX can fetch the release asset, or set ${OMX_SPARKSHELL_BIN_ENV} to override the path.`);
}
export function runSparkShellBinary(binaryPath, args, options = {}) {
...
function writeSparkShellResultOutput(result) {
...
export function isSparkShellNativeCompatibilityFailure(result) {
    if ((result.status ?? 0) === 0)
        return false;
    return SPARKSHELL_GLIBC_INCOMPATIBLE_PATTERN.test(result.stderr || '');
}
export function parseSparkShellFallbackInvocation(args) {
    if (args.length === 0) {
...
function runSparkShellFallback(args, options = {}) {
...
export async function sparkshellCommand(args) {
...
//# sourceMappingURL=sparkshell.js.map