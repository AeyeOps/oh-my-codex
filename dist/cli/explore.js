...
import { isSparkShellNativeCompatibilityFailure, resolveSparkShellBinaryPathWithHydration, runSparkShellBinary, } from './sparkshell.js';
import { DEFAULT_SPARK_MODEL, DEFAULT_STANDARD_MODEL, getEnvConfiguredSparkDefaultModel, getEnvConfiguredStandardDefaultModel, getSparkDefaultModel, getStandardDefaultModel, readConfiguredEnvOverrides, } from '../config/models.js';
import { EXPLORE_BIN_ENV as EXPLORE_BIN_ENV_SHARED, hydrateNativeBinary, isRepositoryCheckout, resolveCachedNativeBinaryCandidatePaths, getPackageVersion, } from './native-assets.js';
...
const WINDOWS_BUILTIN_EXPLORE_HARNESS_REASON = 'the built-in explore harness is not ready on Windows because its allowlist runtime relies on POSIX sh/bash wrappers. Set OMX_EXPLORE_BIN to a compatible custom harness, prefer `omx sparkshell` for shell-native read-only lookups, or run `omx doctor` for readiness details.';
export function getBuiltinExploreHarnessUnsupportedReason(platform = process.platform, env = process.env) {
    if (platform !== 'win32')
        return undefined;
    if (env[EXPLORE_BIN_ENV]?.trim())
        return undefined;
    return WINDOWS_BUILTIN_EXPLORE_HARNESS_REASON;
}
export function assertBuiltinExploreHarnessSupported(platform = process.platform, env = process.env) {
    const reason = getBuiltinExploreHarnessUnsupportedReason(platform, env);
    if (reason)
        throw new Error(`[explore] ${reason}`);
}
...
const WEAK_WIKI_NOTE = 'Wiki evidence is weak or missing. Fall back to broader repository search and recommend that the user build an initial project wiki under .omx/wiki/ if this repo benefits from persistent project knowledge.';
function formatWikiContextBlock(prompt, cwd) {
...
export function buildExplorePromptWithWikiContext(prompt, cwd) {
    const wikiContext = formatWikiContextBlock(prompt, cwd);
    return wikiContext ?? prompt;
}
function tokenizeExploreShellCommand(commandText) {
    const trimmed = commandText.trim();
    if (!trimmed || SHELL_ROUTE_DISALLOWED_PATTERN.test(trimmed) || trimmed.includes('\\'))
        return undefined;
...
function isReadOnlyGitArgs(args) {
...
function classifyLongOutputShellCommand(args) {
...
export function resolveExploreSparkShellRoute(prompt) {
    ...
    const shellNativeShape = explicitShellPrefix || argv.slice(1).some((arg) => (arg.startsWith('-')
        || arg.includes('/')
        || arg === '.'
        || arg.includes('*')));
    if (explicitShellPrefix
        && shellNativeShape
...
async function runExploreViaSparkShell(route, env = process.env) {
...
export function packagedExploreHarnessBinaryName(platform = process.platform) {
    return platform === 'win32' ? 'omx-explore-harness.exe' : 'omx-explore-harness';
}
export function resolvePackagedExploreHarnessCommand(packageRoot = getPackageRoot(), platform = process.platform, arch = process.arch) {
...
export function repoBuiltExploreHarnessCommand(packageRoot = getPackageRoot(), platform = process.platform) {
...
function exploreUsageError(reason) {
    return new Error(`${reason}\n${EXPLORE_USAGE}`);
}
function appendPromptValue(current, value, reason) {
    const trimmed = value.trim();
    if (!trimmed)
        throw exploreUsageError(reason);
    if (current !== undefined)
        throw exploreUsageError('Duplicate --prompt provided.');
    return trimmed;
}
function appendPromptFileValue(current, value, reason) {
    const trimmed = value.trim();
    if (!trimmed)
        throw exploreUsageError(reason);
    if (current !== undefined)
        throw exploreUsageError('Duplicate --prompt-file provided.');
    return trimmed;
}
function hasPromptSource(tokens, flag) {
    ...
    for (let i = 0; i < args.length; i += 1) {
        ...
        if (token === PROMPT_FILE_FLAG) {
            ...
            if (!value || value.startsWith('-'))
                throw exploreUsageError('Missing path after --prompt-file.');
...
export function resolveExploreHarnessCommand(packageRoot = getPackageRoot(), env = process.env) {
...
export async function resolveExploreHarnessCommandWithHydration(packageRoot = getPackageRoot(), env = process.env) {
    const override = env[EXPLORE_BIN_ENV]?.trim();
    if (override) {
        return { command: isAbsolute(override) ? override : join(packageRoot, override), args: [] };
    }
...
export function buildExploreHarnessArgs(prompt, cwd, env = process.env, packageRoot = getPackageRoot()) {
...
export async function loadExplorePrompt(parsed) {
    ...
    if (!trimmed)
        throw exploreUsageError(`Prompt file is empty: ${parsed.promptFile}`);
    return trimmed;
}
export async function exploreCommand(args) {
    ...
    if (result.status !== 0) {
        if (harness.command === 'cargo' && result.stderr?.includes('rustup could not choose')) {
            throw new Error('[explore] cargo is a rustup shim but no default toolchain is configured. ' +
