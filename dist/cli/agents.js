...
function isReservedNativeAgentName(name) {
...
function resolveAgentsDir(scope, cwd = process.cwd()) {
...
function inferMutationScope(cwd = process.cwd()) {
    const persistedScopePath = join(cwd, '.omx', 'setup-scope.json');
    if (existsSync(persistedScopePath)) {
        try {
            const parsed = JSON.parse(readFileSync(persistedScopePath, 'utf8'));
...
function getAgentFilePath(name, scope, cwd = process.cwd()) {
    return join(resolveAgentsDir(scope, cwd), `${name}.toml`);
}
function scaffoldAgentToml(name) {
...
function parseAgentInfo(content, path, scope) {
...
async function readScopeAgents(scope, cwd = process.cwd()) {
...
export async function listNativeAgents(cwd = process.cwd(), scope) {
    if (scope)
        return readScopeAgents(scope, cwd);
...
async function addNativeAgent(name, options = {}) {
...
function resolveExistingAgentPath(name, options = {}) {
    const cwd = options.cwd ?? process.cwd();
    const normalized = normalizeAgentName(name);
    const candidateScopes = options.scope ? [options.scope] : ['project', 'user'];
...
function ensureInteractiveRemove(force) {
    if (force)
        return;
...
async function editNativeAgent(name, options = {}) {
...
async function removeNativeAgent(name, options = {}) {
