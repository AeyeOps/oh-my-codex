...
async function assertProjectPluginModeArtifacts(wd) {
    ...
    assert.equal(existsSync(join(wd, ".codex", "skills", "help", "SKILL.md")), false);
    ...
    it("prints plugin-mode next steps without claiming native agent TOML files were written", async () => {
        ...
        try {
            await withIsolatedUserHome(wd, async () => {
                ...
                assert.match(output, /Codex plugin discovery supplies OMX skills and workflow surfaces/);
        ...
        try {
            await withIsolatedUserHome(wd, async (codexHomeDir) => {
                await withTempCwd(wd, async () => {
                    ...
                    const persisted = JSON.parse(await readFile(join(wd, ".omx", "setup-scope.json"), "utf-8"));
                    assert.deepEqual(persisted, { scope: "user", installMode: "plugin" });
                    assert.equal(existsSync(join(codexHomeDir, "skills", "help", "SKILL.md")), false);
        ...
        try {
            await withIsolatedUserHome(wd, async (codexHomeDir) => {
                await withTempCwd(wd, async () => {
                    ...
                    assert.equal(existsSync(join(codexHomeDir, "skills", "help", "SKILL.md")), true);
        ...
        try {
            await withIsolatedUserHome(wd, async () => {
                await withTempCwd(wd, async () => {
                    ...
                    assert.match(pluginOutput, /Codex plugin discovery supplies OMX skills and workflow surfaces/);
                    assert.match(pluginOutput, /Browse plugin-provided skills with \/skills/);
                    assert.match(pluginOutput, /Optional AGENTS\.md and developer_instructions defaults are only installed when selected/);
        ...
        try {
            await withIsolatedUserHome(wd, async (codexHomeDir) => {
                await withTempCwd(wd, async () => {
                    ...
                    const config = await readFile(configPath, "utf-8");
                    assert.match(config, /^codex_hooks = true$/m);
                    assert.doesNotMatch(config, /oh-my-codex|mcp_servers|notify|developer_instructions/);
                });
            });
        }
        finally {
