...
async function runSetupWithCapturedLogs(cwd, options) {
...
describe("omx setup refresh summary and dry-run behavior", () => {
    async function runSetupInTempDir(wd, options) {
        const previousCwd = process.cwd();
        process.chdir(wd);
        try {
            await setup(options);
    ...
    it("creates .gitignore with OMX project ignore rules during project-scoped setup", async () => {
        ...
        try {
            ...
            assert.equal(await readFile(join(wd, ".gitignore"), "utf-8"), EXPECTED_PROJECT_GITIGNORE);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it("ignores project-local config while keeping .codex agents, skills, and prompts trackable", async () => {
        ...
        try {
            ...
            await writeFile(join(wd, ".codex", "skills", ".system", "cache.json"), "{}\n");
    ...
    it("offers an upgrade from gpt-5.3-codex to gpt-5.5 when accepted", async () => {
        ...
        try {
            ...
            await writeFile(join(wd, ".codex", "config.toml"), 'model = \"gpt-5.3-codex\"\n');
            let promptCalls = 0;
            ...
            assert.match(config, /^# oh-my-codex seeded behavioral defaults \(uninstall removes unchanged defaults\)$/m);
    ...
    it("seeds [tui].status_line for Codex CLI >= 0.107.0 while preserving an existing customization", async () => {
        ...
        try {
            await mkdir(join(wd, ".omx", "state"), { recursive: true });
            await mkdir(join(wd, ".codex"), { recursive: true });
            await writeFile(join(wd, ".codex", "config.toml"), ['model = "gpt-5.5"', "", "[tui]", 'theme = "night"', 'status_line = ["git-branch"]', ""].join("\n"));
            const output = await runSetupWithCapturedLogs(wd, {
                scope: "project",
    ...
    it("seeds default [tui].status_line on fresh setup for Codex CLI >= 0.107.0", async () => {
        ...
        try {
            ...
            const config = await readFile(join(wd, ".codex", "config.toml"), "utf-8");
            assert.equal(config.match(/^\[tui\]$/gm)?.length ?? 0, 1);
            assert.match(config, /^status_line = \["model-with-reasoning", "git-branch", "context-remaining", "total-input-tokens", "total-output-tokens", "five-hour-limit", "weekly-limit"\]$/m);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it("backfills launcher-backed MCP startup timeouts during setup refresh", async () => {
        ...
        try {
            await mkdir(join(wd, ".omx", "state"), { recursive: true });
            await mkdir(join(wd, ".codex"), { recursive: true });
            await writeFile(join(wd, ".codex", "config.toml"), [
    ...
    it("repairs retired omx_team_run config during setup refresh", async () => {
        ...
        try {
            ...
            const config = await readFile(join(wd, ".codex", "config.toml"), "utf-8");
            assert.match(output, /Removed retired \[mcp_servers\.omx_team_run\] config during refresh\./);
    ...
    it("syncs shared MCP registry entries into ~/.claude/settings.json for user scope", async () => {
        ...
        try {
            ...
            await writeFile(join(wd, ".claude", "settings.json"), JSON.stringify({
                uiTheme: "dark",
                mcpServers: {
                    existing_server: {
            ...
            const settings = JSON.parse(await readFile(join(wd, ".claude", "settings.json"), "utf-8"));
    ...
    it("ignores legacy ~/.omc/mcp-registry.json during setup unless candidates are passed explicitly", async () => {
        ...
        try {
            ...
            await writeFile(join(wd, ".omc", "mcp-registry.json"), JSON.stringify({
...
//# sourceMappingURL=setup-refresh.test.js.map