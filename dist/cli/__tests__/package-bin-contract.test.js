...
describe('package bin contract', () => {
    it('declares omx with an explicit relative bin path and avoids packaging platform-specific native binaries', () => {
        ...
        const mcpServe = spawnSync(process.execPath, [binPath, 'mcp-serve', 'state'], {
            cwd: process.cwd(),
            encoding: 'utf-8',
            input: mcpInitialize,
            timeout: 5_000,
        ...
        const mcpResponse = JSON.parse(mcpServe.stdout);
        assert.deepEqual(mcpResponse.result?.serverInfo, { name: 'omx-state', version: '0.1.0' }, 'omx bin wrapper must keep mcp-serve alive long enough to complete stdio initialization');
        ...
        const results = JSON.parse(packed.stdout.slice(jsonStart));
        ...
        const pluginScopedHooksEntry = results[0]?.files?.find((file) => file.path === 'plugins/oh-my-codex/hooks.json'
        ...
        for (const skillName of installableSkillNames) {
            assert.equal(packedFilePaths.has(`plugins/oh-my-codex/skills/${skillName}/SKILL.md`), true, `expected npm pack output to include mirrored plugin ${skillName} skill`);
        }
        ...
        for (const agentName of installableNativeAgentNames) {
            assert.equal(packedFilePaths.has(`prompts/${agentName}.md`), true, `expected npm pack output to include prompt for native agent ${agentName}`);
        }
...
//# sourceMappingURL=package-bin-contract.test.js.map