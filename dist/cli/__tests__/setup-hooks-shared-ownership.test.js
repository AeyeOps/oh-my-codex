...
async function readHooksJson(filePath) {
    return JSON.parse(await readFile(filePath, "utf-8"));
}
async function writeHooksJson(filePath, hooksFile) {
    await writeFile(filePath, JSON.stringify(hooksFile, null, 2) + "\n");
}
function makeUserCommandHook(command, matcher) {
    return {
        ...(matcher ? { matcher } : {}),
        hooks: [
            {
...
describe("omx setup/uninstall shared ownership for native hooks", () => {
    it("setup merges managed wrappers into an existing user-owned hooks.json", async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(setupResult.error))
                return;
            assert.equal(setupResult.status, 0, setupResult.stderr || setupResult.stdout);
            const refreshed = await readHooksJson(hooksPath);
            assert.ok(hookCommands(refreshed.hooks?.SessionStart).includes('node "/custom/session-start.js"'), "setup should preserve pre-existing user SessionStart hooks");
            assert.ok(hookCommands(refreshed.hooks?.PostToolUse).includes('node "/custom/post-tool.js"'), "setup should preserve pre-existing user PostToolUse hooks");
            assert.equal(countManagedHooks(refreshed.hooks?.SessionStart), 1, "setup should append the managed SessionStart wrapper into user-owned hooks.json");
            assert.equal(countManagedHooks(refreshed.hooks?.PostToolUse), 1, "setup should append the managed PostToolUse wrapper into user-owned hooks.json");
        }
        finally {
    ...
    it("setup preserves user hooks while deduping stale OMX wrappers", async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(initial.error))
                return;
            ...
            if (shouldSkipForSpawnPermissions(refreshedSetup.error))
                return;
            ...
            assert.ok(sessionStartCommands.includes('node "/custom/session-start.js"'), "setup should preserve user SessionStart hooks");
            assert.ok(postToolCommands.includes('node "/custom/post-tool.js"'), "setup should preserve user PostToolUse hooks");
            assert.equal(countManagedHooks(refreshed.hooks?.SessionStart), 1, "setup should leave a single managed SessionStart wrapper after refresh");
            assert.equal(countManagedHooks(refreshed.hooks?.PostToolUse), 1, "setup should keep the managed PostToolUse wrapper alongside user hooks");
            assert.ok(hookCommands(refreshed.hooks?.UserPromptSubmit).some((command) => command.includes("codex-native-hook.js")), "setup should still register managed UserPromptSubmit hooks");
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it("uninstall removes only OMX-managed wrappers and preserves user hook content", async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(uninstall.error))
                return;
            ...
            assert.equal(allCommands.some((command) => command.includes("codex-native-hook.js")), false, "uninstall should strip only OMX-managed wrappers");
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
...
//# sourceMappingURL=setup-hooks-shared-ownership.test.js.map