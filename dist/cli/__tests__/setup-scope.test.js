...
describe("omx setup scope behavior", () => {
    it("accepts --scope project form", async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(bySeparateArg.error))
                return;
            ...
            if (shouldSkipForSpawnPermissions(byEqualsArg.error))
                return;
    ...
    it("doctor respects persisted project setup scope paths", async () => {
        ...
        try {
            ...
            await writeFile(join(wd, ".codex", "skills", "sample-skill", "SKILL.md"), "# skill\n");
            ...
            assert.match(res.stdout, new RegExp(`Codex home: (?:/private)?${wd.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}/\\.codex`));
    ...
    it("project scope writes prompts/skills/config/native-agents under cwd", async () => {
        ...
        try {
            ...
            const hooksJson = JSON.parse(await readFile(localHooks, "utf-8"));
            ...
            const persistedScope = JSON.parse(await readFile(scopeFile, "utf-8"));
    ...
    it("setup preserves user hooks while replacing stale OMX wrappers", async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            const hooksJson = JSON.parse(await readFile(join(codexDir, "hooks.json"), "utf-8"));
            const sessionStartHooks = hooksJson.hooks.SessionStart.flatMap((entry) => entry.hooks ?? []);
            const stopHooks = hooksJson.hooks.Stop.flatMap((entry) => entry.hooks ?? []);
            assert.equal(sessionStartHooks.filter((hook) => String(hook.command ?? "").includes("codex-native-hook.js")).length, 1);
            assert.equal(stopHooks.filter((hook) => String(hook.command ?? "").includes("codex-native-hook.js")).length, 1);
            assert.match(JSON.stringify(sessionStartHooks), /echo keep-me/);
            assert.doesNotMatch(JSON.stringify(hooksJson), /\/old\/dist\/scripts\/codex-native-hook\.js/);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it("defaults to user scope in non-interactive runs when no scope is persisted", async () => {
        ...
        try {
            ...
            const persistedScope = JSON.parse(await readFile(join(wd, ".omx", "setup-scope.json"), "utf-8"));
...
//# sourceMappingURL=setup-scope.test.js.map