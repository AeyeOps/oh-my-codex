...
async function writeOmxShim(binDir) {
    await mkdir(binDir, { recursive: true });
    if (process.platform === 'win32') {
        await writeFile(join(binDir, 'omx.cmd'), `@echo off\r\n"${process.execPath}" "${omxBin}" %*\r\n`, 'utf-8');
        return;
    }
...
describe('official Codex plugin layout', () => {
    ...
    it('keeps plugin MCP metadata aligned with the setup-managed MCP roster', async () => {
        ...
        const setupManagedServers = [...mergedConfig.matchAll(/^\[mcp_servers\.(omx_[^\]]+)\]$/gm)]
            .map((match) => match[1])
            .sort();
        assert.deepEqual(setupManagedServers, [...OMX_FIRST_PARTY_MCP_SERVER_NAMES].sort(), 'setup should expose the canonical first-party OMX MCP roster');
        assert.deepEqual(setupManagedServers, Object.keys(mcpManifest.mcpServers ?? {}).sort());
        const targetToEntrypoint = new Map(OMX_FIRST_PARTY_MCP_PLUGIN_TARGETS.map((target, index) => [target, OMX_FIRST_PARTY_MCP_ENTRYPOINTS[index]]));
        for (const [serverName, server] of Object.entries(mcpManifest.mcpServers ?? {})) {
            const target = server.args?.[1] ?? '';
            const entrypoint = targetToEntrypoint.get(target);
            assert.ok(entrypoint, `${serverName} should expose a canonical public target`);
            assert.match(mergedConfig, new RegExp(`\\[mcp_servers\\.${escapeRegex(serverName)}\\][\\s\\S]*?${escapeRegex(entrypoint)}`), `${serverName} should stay aligned with the setup-managed MCP entrypoint`);
        }
    });
    ...
    it('mirrors exactly the setup-installable skill subset from the canonical root skills', async () => {
        const manifest = await readJson(join(root, 'src', 'catalog', 'manifest.json'));
...
//# sourceMappingURL=codex-plugin-layout.test.js.map