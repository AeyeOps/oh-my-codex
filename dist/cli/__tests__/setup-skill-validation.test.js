...
describe('skill frontmatter validation', () => {
    ...
    it('rejects SKILL.md frontmatter without a description', () => {
        const content = `---\nname: help\n---\n\n# Help\n`;
        assert.throws(() => parseSkillFrontmatter(content, '/tmp/help/SKILL.md'), /missing a non-empty frontmatter "description"/i);
    });
    it('rejects SKILL.md frontmatter with unterminated quoted strings', () => {
        const content = `---\nname: help\ndescription: "broken\n---\n\n# Help\n`;
        assert.throws(() => parseSkillFrontmatter(content, '/tmp/help/SKILL.md'), /unterminated quoted string/i);
    });
});
describe('omx setup skill validation', () => {
    it('fails before installing a malformed shipped-style SKILL.md', async () => {
        ...
        try {
            ...
            await assert.rejects(() => installSkills(srcDir, dstDir, { backupRoot: join(root, 'backups'), baseRoot: root }, { force: false, dryRun: false, verbose: false }), /src-skills\/help\/SKILL\.md.*unterminated quoted string/i);
...
//# sourceMappingURL=setup-skill-validation.test.js.map