...
describe('version sync contract', () => {
    it('keeps package.json, workspace metadata, and Rust members aligned for releases', () => {
        ...
        const workspace = TOML.parse(readFileSync(join(process.cwd(), 'Cargo.toml'), 'utf-8'));
        const explore = TOML.parse(readFileSync(join(process.cwd(), 'crates', 'omx-explore', 'Cargo.toml'), 'utf-8'));
        const runtimeCore = TOML.parse(readFileSync(join(process.cwd(), 'crates', 'omx-runtime-core', 'Cargo.toml'), 'utf-8'));
        const mux = TOML.parse(readFileSync(join(process.cwd(), 'crates', 'omx-mux', 'Cargo.toml'), 'utf-8'));
        const runtime = TOML.parse(readFileSync(join(process.cwd(), 'crates', 'omx-runtime', 'Cargo.toml'), 'utf-8'));
        const sparkshell = TOML.parse(readFileSync(join(process.cwd(), 'crates', 'omx-sparkshell', 'Cargo.toml'), 'utf-8'));
...
//# sourceMappingURL=version-sync-contract.test.js.map