...
describe('omx setup skills overwrite behavior', () => {
    ...
    it('adds an [OMX] description badge to installed shipped skills without changing the shipped source files', async () => {
        ...
        try {
            ...
            assert.ok((await readFile(installedHelpSkill, 'utf-8')).includes('description: "[OMX] Guide on using oh-my-codex plugin"'));
            assert.ok((await readFile(shippedHelpSkill, 'utf-8')).includes('description: Guide on using oh-my-codex plugin'));
        }
        finally {
            process.chdir(previousCwd);
            await rm(wd, { recursive: true, force: true });
    ...
    it('installs only active/internal catalog skills (skips alias/merged)', async () => {
        ...
        try {
            ...
            assert.match(await readFile(join(skillsDir, 'analyze', 'SKILL.md'), 'utf-8'), /^---\nname: analyze/m);
            assert.match(await readFile(join(skillsDir, 'autoresearch', 'SKILL.md'), 'utf-8'), /^---\nname: autoresearch/m);
        }
        finally {
            process.chdir(previousCwd);
            await rm(wd, { recursive: true, force: true });
...
//# sourceMappingURL=setup-skills-overwrite.test.js.map