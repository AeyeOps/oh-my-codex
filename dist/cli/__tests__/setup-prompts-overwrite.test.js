...
import { NON_NATIVE_AGENT_PROMPT_ASSETS, getInstallableNativeAgentNames, } from '../../agents/policy.js';
describe('omx setup prompt/native-agent overwrite behavior', () => {
    ...
    it('installs setup-owned prompts separately from active/internal native agents', async () => {
        ...
        try {
            ...
            for (const promptOnlyAgent of [
                ...
                assert.equal(installedPrompts.has(`${promptOnlyAgent}.md`), true, `expected setup to preserve prompt-only role ${promptOnlyAgent}.md`);
            }
            for (const promptAsset of NON_NATIVE_AGENT_PROMPT_ASSETS) {
                assert.equal(installedPrompts.has(`${promptAsset}.md`), true, `expected setup to preserve explicit prompt asset ${promptAsset}.md`);
            }
            ...
            for (const agentName of installableNativeAgents) {
                assert.equal(installedNativeAgents.has(`${agentName}.toml`), true, `expected setup to install native agent ${agentName}.toml`);
            }
    ...
    it('preserves user-authored non-installable native agents during normal setup', async () => {
        ...
        try {
            ...
            await writeFile(userAuthoredPath, [
...
//# sourceMappingURL=setup-prompts-overwrite.test.js.map