...
async function writeStateFile(dir, content) {
    await mkdir(join(dir, '.omx', 'state'), { recursive: true });
    await writeFile(join(dir, '.omx', 'state', 'star-prompt.json'), JSON.stringify(content, null, 2));
}
...
describe('starRepo', () => {
    it('hides the Windows console window for gh star invocations', async () => {
        ...
        let seenOptions;
        starRepo(((_command, _args, options) => {
            seenOptions = options;
            return {
                status: 0,
                stdout: '',
...
//# sourceMappingURL=star-prompt.test.js.map