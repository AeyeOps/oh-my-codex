...
function buildConfigWithSeededModelContext() {
    return [
        '# oh-my-codex top-level settings (must be before any [table])',
        'notify = ["node", "/path/to/notify-hook.js"]',
        'model_reasoning_effort = "medium"',
...
function buildConfigWithEditedSeededModelContext() {
    return [
        '# oh-my-codex top-level settings (must be before any [table])',
        'notify = ["node", "/path/to/notify-hook.js"]',
        'model_reasoning_effort = "medium"',
...
describe('omx uninstall', () => {
    it('removes OMX block from config.toml with --dry-run', async () => {
        ...
        try {
            ...
            await writeFile(join(codexDir, 'hooks.json'), JSON.stringify(buildManagedCodexHooksConfig(wd), null, 2) + '\n');
    ...
    it('warns when overlapping legacy ~/.agents/skills remains after user-scope uninstall', async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            assert.match(res.stdout, /Warning: 1 overlapping skill names remain between .*\.codex[\\/]+skills and .*\.agents[\\/]+skills; 1 differ in SKILL\.md content\. omx uninstall only removes the active canonical skill root; archive or remove ~\/\.agents\/skills if Codex still shows duplicates/);
    ...
    it('warns when a distinct legacy ~/.agents/skills root remains after user-scope uninstall', async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            assert.match(res.stdout, /Warning: legacy ~\/\.agents\/skills still exists \(1 skills\)\. omx uninstall does not remove that historical root automatically; archive or remove ~\/\.agents\/skills if Codex still shows stale or duplicate skills/);
            assert.equal(existsSync(canonicalHelp), false, 'canonical OMX skill should be removed');
            assert.equal(existsSync(join(home, '.agents', 'skills')), true, 'legacy skill root should remain for manual cleanup');
        }
        finally {
    ...
    it('removes managed user-scope AGENTS.md from CODEX_HOME', async () => {
        ...
        try {
            ...
            await writeFile(join(codexHome, 'AGENTS.md'), '<!-- AUTONOMY DIRECTIVE — DO NOT REMOVE -->\n'
