...
describe('decomposeTaskString', () => {
    ...
    it('distributes explicit same-role verifier review tasks across all workers', () => {
        const tasks = decomposeTaskString('review landed coverage; inspect heavy/manual doc residual risks; report blind spots and verification evidence', 3, 'verifier', true);
        assert.equal(tasks.length, 3);
        assert.deepEqual(tasks.map((task) => task.owner), ['worker-1', 'worker-2', 'worker-3']);
    });
...
//# sourceMappingURL=team-decompose.test.js.map