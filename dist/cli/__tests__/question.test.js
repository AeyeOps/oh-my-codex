...
`, { mode: 0o755 });
    ...
    it('runs inline in interactive Windows non-attached sessions instead of hard-failing on missing TMUX', async () => {
        ...
        process.stdin.setRawMode = ((_) => process.stdin);
        ...
        try {
            ...
            if (typeof originalTmuxPane === 'string')
                ...
                delete process.env.TMUX_PANE;
            if (typeof originalQuestionReturnPane === 'string')
                process.env.OMX_QUESTION_RETURN_PANE = originalQuestionReturnPane;
            else
                delete process.env.OMX_QUESTION_RETURN_PANE;
            if (typeof originalLeaderPaneId === 'string')
                process.env.OMX_LEADER_PANE_ID = originalLeaderPaneId;
            else
                delete process.env.OMX_LEADER_PANE_ID;
        }
    });
