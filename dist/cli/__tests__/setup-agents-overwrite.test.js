...
describe('omx setup AGENTS refresh behavior', () => {
    ...
    it('refreshes the managed model table in non-interactive runs without requiring --force', async () => {
        ...
        try {
            await mkdir(join(wd, '.omx', 'state'), { recursive: true });
            const existing = upsertAgentsModelTable(addGeneratedAgentsMarker(template), {
            ...
            const agentsContent = await readFile(join(wd, 'AGENTS.md'), 'utf-8');
            const expectedContext = resolveAgentsModelTableContext(await readFile(join(wd, '.codex', 'config.toml'), 'utf-8'), { codexHomeOverride: join(wd, '.codex') });
            ...
            assert.match(agentsContent, new RegExp(`\\| Frontier \\(leader\\) \\| \`${expectedContext.frontierModel}\` \\| high \\|`));
            assert.match(agentsContent, new RegExp(`\\| Spark \\(explorer\\/fast\\) \\| \`${expectedContext.sparkModel}\` \\| low \\|`));
            assert.match(agentsContent, new RegExp(String.raw `\| \`executor\` \| \`${expectedContext.frontierModel}\` \| medium \| Code implementation, refactoring, feature work`));
...
//# sourceMappingURL=setup-agents-overwrite.test.js.map