...
import { isSparkShellNativeCompatibilityFailure, nestedRepoLocalSparkShellBinaryPath, packagedSparkShellBinaryCandidatePaths, parseSparkShellFallbackInvocation, repoLocalSparkShellBinaryPath, resolveSparkShellBinaryPath, resolveSparkShellBinaryPathWithHydration, runSparkShellBinary, } from '../sparkshell.js';
...
describe('resolveSparkShellBinaryPath', () => {
    it('prefers OMX_SPARKSHELL_BIN override', async () => {
        ...
        try {
            ...
            assert.equal(resolveSparkShellBinaryPath({
                cwd,
    ...
    it('checks Linux musl packaged paths before glibc and legacy paths', () => {
        assert.deepEqual(packagedSparkShellBinaryCandidatePaths('/repo', 'linux', 'x64', {}, ['musl', 'glibc']), [
    ...
    it('throws with checked paths when neither packaged nor repo-local binary exists', () => {
        assert.throws(() => resolveSparkShellBinaryPath({ packageRoot: '/repo', exists: () => false }), /native binary not found/);
    });
    ...
    it('falls back cleanly when hydration manifest is unavailable', async () => {
        ...
        try {
            ...
            try {
                await assert.rejects(() => resolveSparkShellBinaryPathWithHydration({
                    packageRoot: wd,
                    platform: process.platform === 'win32' ? 'win32' : 'linux',
                    arch: 'x64',
                    env: {
...
describe('runSparkShellBinary', () => {
    ...
    it('defaults to packaged lightweight instructions outside the role prompts directory', () => {
        ...
        assert.match(invokedEnv?.OMX_SPARKSHELL_MODEL_INSTRUCTIONS_FILE || '', /templates[\\/]+model-instructions[\\/]+sparkshell-lightweight-AGENTS\.md$/);
    });
});
describe('isSparkShellNativeCompatibilityFailure', () => {
    it('detects GLIBC symbol version failures from the native loader', () => {
        assert.equal(isSparkShellNativeCompatibilityFailure({
            pid: 1,
            output: [],
            stdout: '',
...
describe('parseSparkShellFallbackInvocation', () => {
    it('passes direct commands through unchanged', () => {
        assert.deepEqual(parseSparkShellFallbackInvocation(['git', 'log', '--oneline']), { kind: 'command', argv: ['git', 'log', '--oneline'] });
    });
    ...
    it('converts tmux pane mode into capture-pane argv', () => {
        assert.deepEqual(parseSparkShellFallbackInvocation(['--tmux-pane', '%12', '--tail-lines', '400']), { kind: 'tmux-pane', argv: ['tmux', 'capture-pane', '-t', '%12', '-p', '-S', '-400'] });
    });
});
describe('omx sparkshell', () => {
    ...
    it('falls back to raw execution when the packaged native binary is GLIBC-incompatible', async () => {
        ...
        try {
            ...
            await writeFile(stubPath, process.platform === 'win32'
...
//# sourceMappingURL=sparkshell-cli.test.js.map