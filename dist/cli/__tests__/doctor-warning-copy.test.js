...
`.trimStart());
            ...
            assert.match(res.stdout, /Config: config\.toml exists but no OMX entries yet \(expected before first setup; run "omx setup --force" once\)/);
            assert.match(res.stdout, /MCP Servers: 1 servers but no OMX servers yet \(expected before first setup; run "omx setup --force" once\)/);
        }
        finally {
...
`.trimStart());
            ...
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            assert.match(res.stdout, /Config: retired \[mcp_servers\.omx_team_run\] table still present; run "omx setup --force" to repair the config/);
            assert.match(res.stdout, /MCP Servers: 1 servers configured, but retired \[mcp_servers\.omx_team_run\] is not supported; run "omx setup --force" to repair the config/);
            assert.doesNotMatch(res.stdout, /Config: config\.toml has OMX entries/);
            assert.doesNotMatch(res.stdout, /MCP Servers: 1 servers but no OMX servers yet \(expected before first setup; run "omx setup --force" once\)/);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
        ...
        try {
            await withPackagedExploreHarnessHidden(async () => {
                ...
                if (shouldSkipForSpawnPermissions(res.error))
                    return;
                assert.equal(res.status, 0, res.stderr || res.stdout);
                assert.match(res.stdout, /Explore Harness: (Rust harness sources are packaged, but no compatible packaged prebuilt or cargo was found \(install Rust or set OMX_EXPLORE_BIN for omx explore\)|not ready \(no packaged binary, OMX_EXPLORE_BIN, or cargo toolchain\))/);
            });
        }
        finally {
            await rm(wd, { recursive: true, force: true });
            ...
            try {
                ...
                try {
                    ...
                    if (shouldSkipForSpawnPermissions(res.error))
                        return;
                    assert.equal(res.status, 0, res.stderr || res.stdout);
                    assert.match(res.stdout, /Explore Harness: ready \(packaged native binary:/);
                }
                finally {
                    if (originalBinary) {
...
`.trimStart());
            ...
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            assert.match(res.stdout, /Explore routing: disabled in config\.toml \[env\]; set USE_OMX_EXPLORE_CMD = "1" to restore default explore-first routing/);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it('warns when canonical and legacy skill roots overlap', async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            assert.match(res.stdout, /Legacy skill roots: 1 overlapping skill names between .*\.codex[\\/]+skills and .*\.agents[\\/]+skills; 1 differ in SKILL\.md content; Codex Enable\/Disable Skills may show duplicates until ~\/\.agents\/skills is cleaned up/);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it('warns when hooks.json is missing OMX-managed native hook coverage', async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            assert.match(res.stdout, /Native hooks: hooks\.json is missing OMX-managed coverage for PreToolUse, PostToolUse, UserPromptSubmit, Stop; run "omx setup --force" to restore native hooks/);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
...
`.trimStart());
            ...
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            assert.match(res.stdout, /Native hooks: hooks\.json not found even though config\.toml has OMX entries; run "omx setup --force" to restore native hook coverage/);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it('fails when hooks.json is invalid and native hook coverage cannot be read', async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            assert.match(res.stdout, /\[XX\] Native hooks: invalid hooks\.json; Codex may skip OMX hook coverage until "omx setup --force" repairs it/);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it('passes when legacy skill root is a link to the canonical skills directory', async () => {
        ...
        try {
            ...
            await writeFile(join(canonicalHelp, 'SKILL.md'), '# canonical help\n');
            await symlink(canonicalSkillsRoot, legacyRoot, process.platform === 'win32' ? 'junction' : 'dir');
            const res = runOmx(wd, ['doctor'], {
                HOME: home,
                CODEX_HOME: codexDir,
            });
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            assert.match(res.stdout, /Legacy skill roots: ~\/\.agents\/skills links to canonical .*\.codex[\\/]+skills; treating both paths as one shared skill root/);
