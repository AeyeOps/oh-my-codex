...
describe('omx agents', () => {
    it('lists project and user native agents with name, description, and model', async () => {
        ...
        try {
            ...
            await writeFile(join(projectAgentsDir, 'planner.toml'), 'name = "planner"\ndescription = "Project planner"\nmodel = "gpt-5.5"\ndeveloper_instructions = """plan"""\n');
            await writeFile(join(userAgentsDir, 'reviewer.toml'), 'name = "reviewer"\ndescription = "User reviewer"\ndeveloper_instructions = """review"""\n');
    ...
    it('edits an existing agent via $EDITOR and removes it with --force', async () => {
        ...
        try {
            ...
            await writeFile(agentPath, 'name = "editor-test"\ndescription = "Before edit"\ndeveloper_instructions = """before"""\n');
            ...
            if (shouldSkipForSpawnPermissions(editResult.error))
                return;
            ...
            if (shouldSkipForSpawnPermissions(removeResult.error))
                return;
    ...
    it('fails with clear guidance when remove runs in non-interactive mode without --force', async () => {
        ...
        try {
            ...
            await writeFile(agentPath, 'name = "non-interactive"\ndescription = "Remove me"\ndeveloper_instructions = """noop"""\n');
            ...
            assert.match(result.stderr, /remove requires an interactive terminal; rerun with --force in non-interactive environments/i);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
