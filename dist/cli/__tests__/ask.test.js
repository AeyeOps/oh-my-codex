...
function runProviderAdvisorScript(cwd, argv) {
    const testDir = dirname(fileURLToPath(import.meta.url));
    const repoRoot = join(testDir, '..', '..', '..');
...
describe('omx ask', () => {
    ...
    it('supports claude --print and gemini --prompt end-to-end through omx ask', async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(claudeRes.error))
                return;
    ...
    it('adds Claude skip-permissions for issue-work prompts only', async () => {
        ...
        try {
            ...
            if (shouldSkipForSpawnPermissions(issueRes.error))
                return;
    ...
    it('injects --agent-prompt content into final prompt while keeping Original task raw', async () => {
        ...
        try {
            ...
            await writeFile(join(promptsDir, 'executor.md'), 'You are Executor.\nFollow strict verification rules.');
            await writeFile(join(fakeBin, 'claude'), '#!/bin/sh\nif [ \"$1\" = \"--version\" ]; then echo \"fake-claude\"; exit 0; fi\nif [ \"$1\" = \"-p\" ]; then echo \"CLAUDE_FINAL_PROMPT:$2\"; exit 0; fi\necho \"unexpected\" 1>&2\nexit 3\n');
            await chmod(join(fakeBin, 'claude'), 0o755);
            const res = runOmx(wd, ['ask', 'claude', '--agent-prompt', 'executor', 'ship', 'feature'], { PATH: `${fakeBin}:${process.env.PATH || ''}`, CODEX_HOME: codexHome });
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 0, res.stderr || res.stdout);
            const artifactPath = res.stdout.trim();
    ...
    it('fails clearly when --agent-prompt role is missing from prompts directory', async () => {
        ...
        try {
            ...
            await chmod(join(fakeBin, 'gemini'), 0o755);
            const res = runOmx(wd, ['ask', 'gemini', '--agent-prompt=planner', '--prompt', 'do', 'planning'], { PATH: `${fakeBin}:${process.env.PATH || ''}`, CODEX_HOME: codexHome });
            if (shouldSkipForSpawnPermissions(res.error))
                return;
            assert.equal(res.status, 1, res.stderr || res.stdout);
