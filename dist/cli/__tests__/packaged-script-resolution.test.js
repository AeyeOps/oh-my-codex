...
describe('packaged script resolution contract', () => {
    it('does not reference nonexistent top-level scripts/*.js watcher entrypoints from runtime code', () => {
        ...
        assert.doesNotMatch(cliIndex, /join\(pkgRoot,\s*["']scripts["'],\s*["']notify-fallback-watcher\.js["']\)/);
        assert.doesNotMatch(cliIndex, /join\(pkgRoot,\s*["']scripts["'],\s*["']hook-derived-watcher\.js["']\)/);
    });
...
//# sourceMappingURL=packaged-script-resolution.test.js.map