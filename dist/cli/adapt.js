import { ADAPT_SUBCOMMANDS, } from "../adapt/contracts.js";
import { buildAdaptDoctorReportForTarget, buildAdaptEnvelopeForTarget, buildAdaptProbeReportForTarget, buildAdaptStatusReportForTarget, initAdaptFoundationForTarget, supportedAdaptTargets, } from "../adapt/index.js";
...
function render(value, json, stdout) {
    stdout(JSON.stringify(value, null, json ? 0 : 2));
}
export async function adaptCommand(args, deps = {}) {
    ...
    if (!target || wantsHelp) {
        ...
        const descriptor = getAdaptTargetDescriptor(target);
        if (!descriptor) {
            throw new Error(`Unknown adapt target: ${target}. Supported targets: ${supportedAdaptTargets().join(", ")}`);
        }
    ...
    if (!ADAPT_SUBCOMMANDS.includes(subcommand)) {
        throw new Error(`Unknown adapt subcommand: ${subcommand}. Supported subcommands: ${ADAPT_SUBCOMMANDS.join(", ")}`);
    }
    ...
    switch (subcommand) {
        case "probe":
            render(await buildAdaptProbeReportForTarget(cwd, descriptor.target), json, stdout);
            return;
        case "status":
            render(await buildAdaptStatusReportForTarget(cwd, descriptor.target), json, stdout);
            return;
        case "init":
            render(await initAdaptFoundationForTarget(cwd, descriptor.target, write), json, stdout);
            return;
        case "envelope":
            render(await buildAdaptEnvelopeForTarget(cwd, descriptor.target), json, stdout);
            return;
        case "doctor":
            render(await buildAdaptDoctorReportForTarget(cwd, descriptor.target), json, stdout);
            return;
    }
}
