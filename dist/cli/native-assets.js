...
function packageJsonPath(packageRoot = getPackageRoot()) {
    return join(packageRoot, 'package.json');
}
async function readPackageJson(packageRoot = getPackageRoot()) {
    const raw = await readFile(packageJsonPath(packageRoot), 'utf-8');
    return JSON.parse(raw);
}
export async function getPackageVersion(packageRoot = getPackageRoot()) {
    const pkg = await readPackageJson(packageRoot);
    if (!pkg.version?.trim())
        throw new Error('[native-assets] package.json is missing version');
    return pkg.version.trim();
}
function repositoryHttpBase(repository) {
...
export async function resolveNativeReleaseBaseUrl(packageRoot = getPackageRoot(), version, env = process.env) {
    ...
    if (!repo)
        throw new Error('[native-assets] unable to resolve GitHub repository URL for native release downloads');
    const resolvedVersion = version ?? await getPackageVersion(packageRoot);
    return `${repo}/releases/download/v${resolvedVersion}`;
}
export async function resolveNativeManifestUrl(packageRoot = getPackageRoot(), version, env = process.env) {
...
export function resolveNativeCacheRoot(env = process.env) {
...
export function resolveCachedNativeBinaryPath(product, version, platform = process.platform, arch = process.arch, env = process.env, libc) {
...
function inferRuntimeLibcFromText(text) {
    const normalized = String(text || '').toLowerCase();
    if (!normalized)
        return undefined;
    if (normalized.includes('musl'))
        return 'musl';
    if (normalized.includes('glibc') || normalized.includes('gnu libc'))
        return 'glibc';
    return undefined;
}
export function resolveLinuxNativeLibcPreference(options = {}) {
...
function detectLinuxRuntimeLibc(env = process.env) {
...
export function inferNativeAssetLibc(asset) {
    if (asset.libc === 'musl' || asset.libc === 'glibc')
        return asset.libc;
    const hint = [asset.target, asset.archive].filter(Boolean).join(' ').toLowerCase();
    if (hint.includes('musl'))
        return 'musl';
    if (hint.includes('linux-gnu') || hint.includes('glibc'))
        return 'glibc';
    return undefined;
}
export function resolveCachedNativeBinaryCandidatePaths(product, version, platform = process.platform, arch = process.arch, env = process.env, options = {}) {
    const candidates = [];
    if (platform === 'linux') {
...
export function resolveNativeReleaseAssetCandidates(manifest, product, version, platform, arch, options = {}) {
...
export function isRepositoryCheckout(packageRoot = getPackageRoot()) {
    return existsSync(join(packageRoot, '.git')) || existsSync(join(packageRoot, 'src'));
}
export async function loadNativeReleaseManifest(packageRoot = getPackageRoot(), version, env = process.env) {
...
async function extractArchive(archivePath, destinationDir) {
    ...
    if (ext === '.zip') {
        const { result } = spawnPlatformCommandSync(process.platform === 'win32' ? 'powershell' : 'unzip', process.platform === 'win32'
            ...
            : ['-oq', archivePath, '-d', destinationDir], { encoding: 'utf-8' });
...
async function findExtractedBinaryPath(rootDir, binaryPath) {
...
export async function hydrateNativeBinary(product, options = {}) {
    const { packageRoot = getPackageRoot(), env = process.env, platform = process.platform, arch = process.arch, } = options;
    if (env[NATIVE_AUTO_FETCH_ENV]?.trim() === '0')
        return undefined;
    ...
    try {
        ...
        if (isUnavailableManifestError(error))
            return undefined;
        throw error;
    }
    ...
    try {
        for (let index = 0; index < assets.length; index += 1) {
            ...
            const cachedBinaryPath = resolveCachedNativeBinaryPath(product, version, platform, arch, env, inferNativeAssetLibc(asset));
            try {
...
//# sourceMappingURL=native-assets.js.map