...
import { monitorTeam, resumeTeam, shutdownTeam, startTeam } from '../team/runtime.js';
...
import { parseWorktreeMode } from '../team/worktree.js';
...
function persistExactTeamModeState(cwd, updates, sessionId) {
...
function resolveApprovedTeamFollowupContext(cwd, task) {
...
function sampleValueForTeamApiField(field) {
    switch (field) {
...
function buildTeamApiOperationHelp(operation) {
...
function buildJsonBase() {
    return {
        schema_version: '1.0',
        timestamp: new Date().toISOString(),
    };
}
function parseStatusTailLines(args) {
...
function resolveDefaultTeamWorktreeMode(mode) {
    if (mode.enabled)
        return mode;
...
function snapshotHasDeadWorkerStall(snapshot) {
    return snapshot.deadWorkers.length > 0 && (snapshot.tasks.pending + snapshot.tasks.in_progress) > 0;
}
function buildDeadWorkerAwaitEvent(teamName, snapshot) {
...
function parseTeamArgs(args, cwd = process.cwd()) {
    const tokens = [...args];
    let workerCount = 3;
...
export function parseTeamStartArgs(args) {
...
function resolveImplicitTeamFallbackRole(agentType, explicitAgentType) {
    return !explicitAgentType && agentType === 'executor' ? 'team-executor' : agentType;
}
function looksLikeLowConfidenceAnalysisTask(task) {
...
function resolveTeamFanoutLimit(task, requestedWorkerCount, explicitAgentType, explicitWorkerCount, plan) {
...
export function buildTeamExecutionPlan(task, workerCount, agentType, explicitAgentType, explicitWorkerCount = false) {
    ...
    const tasks = usedAspectSubtasks && tasksWithRoles.length > 1 && normalizedRoles.size <= 1
        ...
        : distributeTasksToWorkers(tasksWithRoles, effectiveWorkerCount, explicitAgentType ? agentType : undefined);
    return {
        workerCount: effectiveWorkerCount,
        tasks,
    };
}
export function decomposeTaskString(task, workerCount, agentType, explicitAgentType, explicitWorkerCount = false) {
...
function hasAtomicParallelizationSignals(task, size) {
    ...
    if (fileRefCount >= 2)
        return true;
    if (fileRefCount >= 1 && symbolRefCount >= 1)
        return true;
    if (PARALLELIZATION_SIGNAL.test(task) && size === 'large')
        return true;
    return size === 'large' && countWords(task) >= 24;
}
function looksLikeStandaloneWeakSubtask(part) {
    const normalized = part.trim().replace(/^[*-]\s*/, '');
    return ACTIONABLE_TASK_PREFIX.test(normalized) || TASK_LABEL_PREFIX.test(normalized);
}
function canSafelySplitWeakTaskList(task, parts) {
    if (parts.length < 2)
        return false;
    if (countWords(task) > 18)
        return false;
    if (CONTEXTUAL_DECOMPOSITION_CLAUSE.test(task))
        return false;
...
function splitTaskString(task) {
    ...
    if (numberedMatches.length >= 2) {
        ...
        if (parts.length >= 2)
            return { strategy: 'numbered', subtasks: parts };
    }
...
function createAspectSubtasks(task, workerCount) {
    const aspects = [
...
function distributeTasksToWorkers(tasks, workerCount, workerRole) {
...
async function ensureTeamModeState(parsed, tasks) {
...
async function persistTeamShutdownModeState(teamName, cwd, configSnapshot) {
    const sessionStatePath = join(cwd, '.omx', 'state', 'session.json');
    let scopedSessionId;
    if (existsSync(sessionStatePath)) {
        try {
            const parsed = JSON.parse(readFileSync(sessionStatePath, 'utf-8'));
...
async function renderStartSummary(runtime, staffingPlan) {
    ...
    if (snapshot.performance) {
        console.log(`monitor_perf_ms: total=${snapshot.performance.total_ms} list=${snapshot.performance.list_tasks_ms} workers=${snapshot.performance.worker_scan_ms} mailbox=${snapshot.performance.mailbox_delivery_ms}`);
    }
    for (const hint of buildLeaderMonitoringHints(runtime.teamName)) {
        console.log(hint);
    }
}
export function buildLeaderMonitoringHints(teamName) {
...
export async function teamCommand(args, _options = {}) {
    ...
    if (subcommand === 'status') {
        ...
        const wantsJson = teamArgs.includes('--json');
        if (!name)
            throw new Error('Usage: omx team status <team-name> [--json]');
    ...
    if (subcommand === 'await') {
        const name = teamArgs[1];
        if (!name)
            throw new Error('Usage: omx team await <team-name> [--timeout-ms <ms>] [--after-event-id <id>] [--json]');
        const wantsJson = teamArgs.includes('--json');
        ...
        const result = immediateEvent
            ? { status: 'event', cursor: immediateEvent.event_id, event: immediateEvent }
            : snapshot && snapshotHasDeadWorkerStall(snapshot)
                ? await readTeamEvents(name, cwd, { wakeableOnly: true }).then((events) => {
                    ...
                    return fallbackEvent
                        ? { status: 'event', cursor: baselineCursor, event: fallbackEvent }
                        : { status: 'timeout', cursor: baselineCursor };
                })
    ...
    if (subcommand === 'resume') {
        const name = teamArgs[1];
        if (!name)
            throw new Error('Usage: omx team resume <team-name>');
    ...
    if (subcommand === 'shutdown') {
        const name = teamArgs[1];
        if (!name)
            throw new Error('Usage: omx team shutdown <team-name> [--force] [--confirm-issues]');
        ...
        await persistTeamShutdownModeState(name, cwd, configBeforeShutdown
            ? {
    ...
    const executionPlan = buildTeamExecutionPlan(parsed.task, parsed.workerCount, parsed.agentType, parsed.explicitAgentType, parsed.explicitWorkerCount);
    ...
    const runtime = await startTeam(parsed.teamName, parsed.task, parsed.agentType, executionPlan.workerCount, tasks, cwd, { worktreeMode });
