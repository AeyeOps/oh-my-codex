...
`;
function hooksDir(cwd = process.cwd()) {
    return join(cwd, '.omx', 'hooks');
}
function samplePluginPath(cwd = process.cwd()) {
    return join(hooksDir(cwd), 'sample-plugin.mjs');
}
async function validateHookPluginExport(filePath) {
    try {
...
async function statusHooks() {
    ...
    console.log(`Plugins enabled: ${isHookPluginsEnabled(process.env) ? 'yes' : 'no (disabled with OMX_HOOK_PLUGINS=0)'}`);
...
function normalizeDispatchResult(result) {
    ...
    const results = Array.isArray(obj.results)
        ? obj.results.filter((item) => !!item && typeof item === 'object')
        : [];
    return {
...
function pluginLabelFromResult(result) {
    if (typeof result.plugin_id === 'string' && result.plugin_id)
        return result.plugin_id;
    if (typeof result.plugin === 'string' && result.plugin)
        return result.plugin;
    if (typeof result.file === 'string' && result.file)
        return result.file;
    return 'unknown-plugin';
}
function pluginStatusFromResult(result) {
...
export function formatHooksStatusLine(plugin) {
