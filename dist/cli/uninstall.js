...
import { stripExistingOmxBlocks, stripOmxEnvSettings, stripOmxTopLevelKeys, stripOmxFeatureFlags, stripOmxSeededBehavioralDefaults, } from "../config/generator.js";
import { parseCodexHooksConfig, removeManagedCodexHooks, } from "../config/codex-hooks.js";
...
async function removeAgentsMd(agentsMdPath, options) {
...
async function detectLegacySkillRootWarning(scope) {
    if (scope !== "user")
        return null;
    ...
    if (overlap.overlappingSkillNames.length === 0) {
        return (`legacy ~/.agents/skills still exists (${overlap.legacySkillCount} skills). ` +
    ...
    const mismatchMessage = overlap.mismatchedSkillNames.length > 0
        ...
        : "";
    return (`${overlap.overlappingSkillNames.length} overlapping skill names remain between ` +
