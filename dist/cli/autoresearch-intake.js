...
function inferRepoRootFromResultPath(resultPath) {
    return dirname(dirname(dirname(dirname(resultPath))));
}
function isUsableAbsolutePath(path) {
...
async function readPersistedResult(resultPath) {
    ...
    const parsedCompileTarget = parsed.compileTarget;
...
export async function listAutoresearchDeepInterviewDraftPaths(repoRoot) {
...
export async function resolveAutoresearchDeepInterviewResult(repoRoot, options = {}) {
    ...
    const draftPaths = await filterRecentPaths(await listAutoresearchDeepInterviewDraftPaths(repoRoot), options.newerThanMs, options.excludeDraftPaths);
