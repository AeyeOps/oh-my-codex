...
function parseTimeoutMs(value, defaultTimeoutMs) {
    if (!value)
...
testArgs.push(...files);
console.error(`[run-test-files] running ${files.length} test file(s) from ${targets.join(', ')}${testTimeoutMs > 0 ? ` with per-test timeout ${testTimeoutMs}ms` : ' with per-test timeout disabled'}${runnerTimeoutMs > 0 ? ` and runner timeout ${runnerTimeoutMs}ms` : ' and runner timeout disabled'}`);
...
console.error(`[run-test-files] node --test did not exit normally${result.signal ? ` (signal: ${result.signal})` : ''}. `
...
process.exit(1);
//# sourceMappingURL=run-test-files.js.map