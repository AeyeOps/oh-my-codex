...
import { maybeAutoNudge, isDeepInterviewInputLockActive, isDeepInterviewStateActive, loadAutoNudgeConfig, normalizeAutoNudgeSignatureText, resolveAutoNudgeSignature, } from './notify-hook/auto-nudge.js';
...
import { checkWorkerPanesAlive, isLeaderStale, maybeNudgeTeamLeader, resolveLeaderStalenessThresholdMs, } from './notify-hook/team-leader-nudge.js';
...
async function writeJsonObjectAtomically(path, value) {
...
const maxLifetimeMs = runOnce
    ? 0
    : Math.max(pollMs, asNumber(argValue('--max-lifetime-ms', process.env.OMX_NOTIFY_FALLBACK_MAX_LIFETIME_MS || String(defaultMaxLifetimeMs)), defaultMaxLifetimeMs));
...
function shouldLogDispatchDrainTick(result) {
    if (!result || typeof result !== 'object')
        return false;
    const record = result;
    const processed = asNumber(record.processed, 0);
    const skipped = asNumber(record.skipped, 0);
    const failed = asNumber(record.failed, 0);
...
function hasRalphTerminalState(raw) {
    ...
    if (raw.active !== true)
        return true;
    if (!shouldContinueRun(raw))
        return true;
    const phase = safeString(raw.current_phase).trim().toLowerCase();
    if (phase && RALPH_TERMINAL_PHASES.has(phase))
        return true;
    if (isStaleRalphStartingPhase(raw))
        return true;
    if (safeString(raw.completed_at).trim())
        return true;
    return false;
}
function isStaleRalphStartingPhase(raw) {
    const phase = safeString(raw.current_phase).trim().toLowerCase();
    if (phase !== 'starting')
        return false;
...
async function loadPersistedWatcherState() {
    ...
    const persistedAutoNudge = persisted?.fallback_auto_nudge;
    if (persistedAutoNudge && typeof persistedAutoNudge === 'object') {
        lastFallbackAutoNudge = {
            enabled: persistedAutoNudge.enabled !== false,
            stall_ms: Number.isFinite(persistedAutoNudge.stall_ms) && persistedAutoNudge.stall_ms > 0
                ? persistedAutoNudge.stall_ms
                : AUTO_NUDGE_STALL_MS,
            last_tick_at: safeString(persistedAutoNudge.last_tick_at) || null,
            last_turn_at: safeString(persistedAutoNudge.last_turn_at),
            last_turn_count: Number.isFinite(persistedAutoNudge.last_turn_count) ? persistedAutoNudge.last_turn_count : null,
...
async function resolveActiveModeState(mode) {
    ...
    if (!candidateDirs.includes(stateDir))
        candidateDirs.push(stateDir);
...
async function readRalphSteerLock(path) {
...
async function withRalphSteerLock(task) {
    ...
    for (let attempt = 0; attempt < RALPH_STEER_LOCK_MAX_RETRIES; attempt++) {
        ...
        try {
            ...
            const code = error !== null && typeof error === 'object' ? error.code : '';
            if (code !== 'EEXIST')
                throw error;
...
async function readPidFileRecord(path) {
    const raw = await readFile(path, 'utf-8').catch(() => '');
    ...
    try {
        ...
        return {
            ...
            max_lifetime_ms: asNumber(parsed.max_lifetime_ms, 0) || undefined,
...
async function buildWatcherManagedPayload() {
    const session = await readSessionState(cwd).catch(() => null);
    const sessionId = safeString(session?.session_id).trim();
    if (!sessionId || !session || isSessionStale(session))
        return null;
    return { session_id: sessionId };
}
async function persistReboundRalphPaneState(statePath, state, paneId, nowIso) {
...
async function resolveRalphContinuePaneTarget(activeRalph, nowIso) {
...
async function removePidFileIfOwned() {
    if (runOnce)
        return;
    const existingRecord = await readPidFileRecord(pidFilePath).catch(() => null);
    if (existingRecord?.pid !== process.pid)
        return;
    if (existingRecord.owner_token && existingRecord.owner_token !== watcherOwnerToken)
        return;
...
async function runFallbackAutoNudgeTick() {
    ...
    const turnCount = Number.isFinite(hudState.turn_count) ? hudState.turn_count : null;
...
async function enforceLifecycleGuards() {
    ...
    if (parentIsGone()) {
        ...
        if (activeRalph.active) {
            ...
            if (lastParentGuard.reason !== nextParentGuard.reason
...
function sessionDirs() {
    const now = new Date();
    const today = join(homedir(), '.codex', 'sessions', String(now.getUTCFullYear()), String(now.getUTCMonth() + 1).padStart(2, '0'), String(now.getUTCDate()).padStart(2, '0'));
    const yesterdayDate = new Date(now.getTime() - 24 * 60 * 60 * 1000);
    const yesterday = join(homedir(), '.codex', 'sessions', String(yesterdayDate.getUTCFullYear()), String(yesterdayDate.getUTCMonth() + 1).padStart(2, '0'), String(yesterdayDate.getUTCDate()).padStart(2, '0'));
...
async function discoverRolloutFiles() {
    ...
    for (const dir of sessionDirs()) {
        ...
        for (const name of names) {
            ...
            if (!st)
                continue;
            if (st.mtimeMs < startedAt - fileWindowMs)
                continue;
...
async function processLine(meta, line, filePath) {
    ...
    if (!parsed || parsed.type !== 'event_msg' || !parsed.payload)
        return;
    ...
    if (Number.isFinite(evtTs) && evtTs < startedAt - 3000)
        return;
    const key = turnKey(meta.threadId, turnId);
    if (seenTurnKeys.has(key))
        return;
    seenTurnKeys.add(key);
    const payload = buildNotifyPayload(meta.threadId, turnId, safeString(parsed.payload.last_agent_message));
...
function splitBufferedLines(partial, delta) {
...
async function readFileDelta(path, offset, currentSize) {
...
//# sourceMappingURL=notify-fallback-watcher.js.map