    ...
    for (let i = 0; i < iterations; i += 1) {
        ...
        try {
            ...
            if (reopened?.status !== 'pending')
                throw new Error('task not reclaimed');
...
async function benchDirtyWorktree(iterations = 10) {
    ...
    for (let i = 0; i < iterations; i += 1) {
        ...
        try {
            ...
            if (!plan.enabled)
                throw new Error('plan disabled');
            const first = ensureWorktree(plan);
            if (!first.enabled)
                throw new Error('ensure disabled');
            ...
            const elapsed = performance.now() - start;
            if (!blocked)
                throw new Error('dirty reuse was not blocked');
            samples.push(elapsed);
        }
        finally {
...
//# sourceMappingURL=team-hardening-benchmark.js.map