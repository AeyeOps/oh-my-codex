...
import { getSetupInstallableSkillNames, isCatalogInstallableStatus, } from "../catalog/installable.js";
...
function assertDeepJsonEqual(actual, expected, label) {
...
async function assertRootSkillCatalogConsistency(root, skillNames) {
    ...
    const unlistedSkillDirs = rootEntries
        ...
        .filter((skillName) => !manifestByName.has(skillName) && !expectedSkillNames.has(skillName))
        .sort();
...
async function buildExpectedPluginManifest(root) {
    const { pluginManifestPath } = getPluginPaths(root);
    const [manifest, pkg] = await Promise.all([
        readJsonFile(pluginManifestPath),
...
async function assertPluginManifestPolicy(root, manifest) {
...
async function assertPluginMetadata(root) {
    const { pluginMcpPath, pluginAppsPath, pluginManifestPath } = getPluginPaths(root);
    const [actualMcp, actualApps, actualManifest] = await Promise.all([
        readJsonFile(pluginMcpPath),
        readJsonFile(pluginAppsPath),
        readJsonFile(pluginManifestPath),
    ]);
...
async function writePluginMetadata(root, verbose = false) {
    const { pluginMcpPath, pluginAppsPath, pluginManifestPath } = getPluginPaths(root);
    ...
    for (const write of writes) {
        ...
        if (existing !== write.content) {
            ...
            changed = true;
            if (verbose)
                console.log(`synced ${write.label}`);
        }
    }
    return changed;
}
export async function syncPluginMirror(options = {}) {
    ...
    const beforeSkillsMatch = (await compareSkillMirror(rootSkillsDir, pluginSkillsDir, skillNames)) ===
        null;
    ...
    for (const skillName of skillNames) {
        ...
        if (options.verbose) {
            console.log(`mirrored skills/${skillName} -> plugins/${PLUGIN_NAME}/skills/${skillName}`);
        }
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
    syncPluginMirror(parseArgs(process.argv.slice(2)))
        ...
        console.log(`[sync-plugin-mirror] ${action} ${result.mirroredSkillNames.length} canonical skill director${result.mirroredSkillNames.length === 1 ? "y" : "ies"} and plugin metadata`);
    })
        .catch((error) => {
...
//# sourceMappingURL=sync-plugin-mirror.js.map