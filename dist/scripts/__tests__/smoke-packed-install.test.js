...
import { ensureRepoDependencies, hasUsableNodeModules, PACKED_INSTALL_SMOKE_CORE_COMMANDS, parseNpmPackJsonOutput, resolveGitCommonDir, resolveReusableNodeModulesSource, } from '../smoke-packed-install.js';
test('packed install smoke stays limited to boot + core commands', () => {
    assert.deepEqual(PACKED_INSTALL_SMOKE_CORE_COMMANDS, [
        ['--help'],
        ['version'],
    ]);
    assert.equal(PACKED_INSTALL_SMOKE_CORE_COMMANDS.some((argv) => argv.includes('explore') || argv.includes('sparkshell')), false);
});
...
//# sourceMappingURL=smoke-packed-install.test.js.map