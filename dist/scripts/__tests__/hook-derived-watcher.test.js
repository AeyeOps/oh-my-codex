...
describe('hook-derived-watcher', () => {
    ...
    it('dispatches needs-input for assistant_message content arrays', async () => {
        ...
        try {
            ...
            await writeFile(join(cwd, '.omx', 'hooks', 'capture-needs-input.mjs'), `import { appendFile, mkdir } from 'node:fs/promises';
...
`);
            ...
            const result = spawnSync(process.execPath, [watcherScript, '--once', '--cwd', cwd, '--poll-ms', '250'], {
                cwd,
                env: {
                    ...process.env,
                    HOME: homeDir,
            ...
            assert.match(String(events[0].context?.preview ?? ''), /Would you like me to continue/i);
        }
        finally {
...
//# sourceMappingURL=hook-derived-watcher.test.js.map