...
import { getInstallableNativeAgentNames, isNativeAgentInstallableStatus, } from "../../agents/policy.js";
...
async function rejectsWithCode(code, action) {
...
describe("verify-native-agents", () => {
    ...
    it("fails when an installable catalog agent lacks a definition", async () => {
        await rejectsWithCode("native_agent_definition_missing", () => verifyNativeAgents({
    ...
    it("fails when an installable catalog agent lacks a prompt", async () => {
        await rejectsWithCode("native_agent_prompt_missing", () => verifyNativeAgents({
    ...
    it("fails when a definition is missing from catalog agents", async () => {
        await rejectsWithCode("native_agent_catalog_out_of_sync", () => verifyNativeAgents({
            manifest: manifest([]),
            definitions: { executor: definition },
            promptNames: new Set(),
            pluginManifest: {},
    ...
    it("fails when a merged canonical target is missing or non-installable", async () => {
        await rejectsWithCode("native_agent_canonical_invalid", () => verifyNativeAgents({
            manifest: manifest([
                {
                    name: "style-reviewer",
                    category: "review",
    ...
    it("fails for unclassified prompt assets", async () => {
        await rejectsWithCode("native_agent_prompt_unclassified", () => verifyNativeAgents({
            manifest: manifest([{ name: "executor", category: "build", status: "active" }]),
            definitions: { executor: definition },
    ...
    it("fails if the plugin manifest declares setup-owned native-agent fields", async () => {
        await rejectsWithCode("native_agent_plugin_boundary_violation", () => verifyNativeAgents({
            manifest: manifest([{ name: "executor", category: "build", status: "active" }]),
            definitions: { executor: definition },
            promptNames: new Set(["executor"]),
...
//# sourceMappingURL=verify-native-agents.test.js.map