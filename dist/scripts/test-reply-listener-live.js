...
export function resolveReplyListenerLiveEnv(env = process.env) {
...
export async function runReplyListenerLiveSmoke(config, deps = {}) {
    const fetchImpl = deps.fetchImpl ?? fetch;
    const log = deps.log ?? console.log;
    const stamp = new Date().toISOString();
    const discordSendResponse = await fetchImpl(`https://discord.com/api/v10/channels/${config.discordChannelId}/messages`, {
        method: 'POST',
        headers: {
    ...
    try {
        await fetchImpl(`https://discord.com/api/v10/channels/${config.discordChannelId}/messages/${discordMessageId}`, {
            method: 'DELETE',
    ...
    const telegramSendResponse = await fetchImpl(`https://api.telegram.org/bot${config.telegramBotToken}/sendMessage`, {
        method: 'POST',
    ...
    try {
        await fetchImpl(`https://api.telegram.org/bot${config.telegramBotToken}/deleteMessage`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                chat_id: config.telegramChatId,
...
//# sourceMappingURL=test-reply-listener-live.js.map