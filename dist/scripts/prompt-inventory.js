...
export function listPromptSurfacePaths(root = process.cwd()) {
    const paths = new Set();
...
function countAbsoluteDirectives(text) {
    return text
        .split(/\r?\n/)
        .filter((line) => ABSOLUTE_DIRECTIVE_PATTERN.test(line))
        .length;
}
function inventorySurface(root, path) {
...
function duplicateFragmentFamilies(root, paths) {
    ...
    for (const path of paths) {
        ...
        for (const line of text.split(/\r?\n/)) {
            ...
            if (!normalized)
                continue;
            const pathsWithLine = occurrences.get(normalized) ?? new Set();
...
export function buildPromptInventory(root = process.cwd(), generatedAt = new Date().toISOString()) {
...
export function renderPromptInventoryMarkdown(report) {
    ...
    const duplicates = report.duplicateFragmentFamilies.length === 0
        ? ['- None detected.']
        : report.duplicateFragmentFamilies.map((family) => `- ${family.count} files: ${family.text}\n  - ${family.paths.join(', ')}`);
    return [
        '# Prompt Inventory',
        '',
...
//# sourceMappingURL=prompt-inventory.js.map