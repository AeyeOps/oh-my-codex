...
export function tmuxHookExplicitlyDisablesInjection(raw) {
...
export function evaluateInjectionGuards({ config, mode, sourceText, assistantMessage, threadId, turnId, paneKey, sessionKey, skipQuotaChecks, now, state, }) {
    if (!config.enabled)
        return { allow: false, reason: 'disabled' };
    if (!config.valid || !config.target)
        return { allow: false, reason: 'invalid_config' };
    if (!mode)
        return { allow: false, reason: 'mode_not_allowed' };
...
export function isPaneRunningShell(paneCurrentCommand) {
    if (typeof paneCurrentCommand !== 'string')
        return false;
...
function isHudStartCommand(startCommand) {
 ...
 */
export function resolveCodexPane() {
...
export function paneLooksReady(captured) {
    ...
    if (hasCodexWelcomePrompt)
        return true;
    return lines.some((line) => /^\s*(?:[›>❯]\s*)?[A-Z][A-Z0-9]+-\d+\s+only(?:\s*(?:…|\.{3}))?\s*$/iu.test(line));
}
export function paneShowsCodexViewport(captured) {
...
export function paneHasActiveTask(captured) {
    ...
    if (tail.some((line) => /\bbackground terminal running\b/i.test(line)))
        return true;
    if (tail.some((line) => /^•\s.+\(.+•\s*esc to interrupt\)$/i.test(line)))
        return true;
...
export function buildVisibleCapturePaneArgv(paneTarget) {
...
//# sourceMappingURL=tmux-hook-engine.js.map