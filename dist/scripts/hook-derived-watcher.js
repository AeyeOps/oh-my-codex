...
import { buildOperationalContext, classifyExecCommand, parseCommandResult, parseExecCommandArgs, } from './notify-hook/operational-events.js';
...
function collectMessageTextFragments(value, fragments) {
    ...
    if (Array.isArray(value)) {
        for (const item of value)
            collectMessageTextFragments(item, fragments);
        return;
    }
...
async function discoverRolloutFiles() {
    ...
    for (const dir of sessionDirs()) {
        ...
        for (const name of names) {
            ...
            if (!st)
                continue;
            if (now - st.mtimeMs > maxFileAgeMs)
                continue;
...
function inferOperationalCall(parsed, meta) {
    if (!parsed || parsed.type !== 'response_item' || !parsed.payload)
        return null;
    const payload = parsed.payload;
...
async function processLine(meta, line) {
    ...
    if (operational?.phase === 'start') {
        ...
        if (operational.callId)
            pendingCalls.delete(operational.callId);
        if (operational.kind === 'test') {
            if (operational.result?.success !== true && operational.result?.success !== false)
                return;
            await dispatchOperationalEvent({
...
//# sourceMappingURL=hook-derived-watcher.js.map