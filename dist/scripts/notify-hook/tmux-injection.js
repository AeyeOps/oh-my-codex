...
import { readJsonIfExists, normalizeTmuxState, pruneRecentKeys, getScopedStateDirsForCurrentSession, readCurrentSessionId, readdir, } from './state-io.js';
...
import { normalizeTmuxHookConfig, pickActiveMode, evaluateInjectionGuards, buildSendKeysArgv, } from '../tmux-hook-engine.js';
function isHudPaneStartCommand(startCommand) {
    return /\bomx\b.*\bhud\b.*--watch/i.test(safeString(startCommand));
}
async function resolvePaneCwdMismatch(paneId, expectedCwd) {
    if (!expectedCwd)
        return null;
    try {
...
async function finalizeResolvedPane(paneId, reason, expectedCwd) {
...
async function resolveCanonicalPaneFromPaneTarget(paneTarget, expectedCwd) {
...
async function resolvePreferredModePane(stateDir, allowedModes) {
...
async function readVisibleAllowedModes(cwd, stateDir, payload, allowedModes) {
    ...
    for (const sessionId of candidateSessionIds) {
        ...
        if (!canonicalState)
            continue;
        const allowedSet = new Set(listActiveSkills(canonicalState)
    ...
    if (candidateSessionIds.length === 0) {
        ...
        if (rootCanonicalState) {
            const allowedSet = new Set(listActiveSkills(rootCanonicalState)
                .map((entry) => entry.skill)
                .filter((skill) => allowedModes.includes(skill)));
            return {
                canonicalPresent: true,
...
export async function resolveSessionToPane(sessionName) {
    ...
    if (activeNonHud)
        return activeNonHud.paneId;
    return nonHudRows[0]?.paneId || null;
}
export async function resolvePaneTarget(target, expectedCwd, modePane, cwd, payload) {
    ...
    if (!target)
        return { paneTarget: null, reason: 'invalid_target' };
    if (target.type === 'pane') {
        try {
    ...
    try {
        if (!requiresManagedOwnership)
            return { paneTarget: null, reason: 'target_session_requires_managed_context' };
...
export async function handleTmuxInjection({ payload, cwd, stateDir, logsDir, }) {
    ...
    const scanActiveModeStateDirs = async (dirs, preserveExisting = false) => {
        for (const scopedDir of dirs) {
            ...
            if (scannedStateDirs.has(resolvedScopedDir))
                continue;
            scannedStateDirs.add(resolvedScopedDir);
            const files = await readdir(scopedDir).catch(() => []);
            for (const file of files) {
                if (!file.endsWith('-state.json') || file === 'tmux-hook-state.json')
                    continue;
                const path = join(scopedDir, file);
                const parsed = JSON.parse(await readFile(path, 'utf-8'));
                if (parsed && parsed.active) {
                    const modeName = file.replace('-state.json', '');
                    if (canonicalModeState.allowedSet && !canonicalModeState.allowedSet.has(modeName))
                        continue;
    ...
    const preferredModePane = await resolvePreferredModePane(stateDir, canonicalModeState.canonicalPresent
        ? (canonicalModeState.preferredMode ? [canonicalModeState.preferredMode] : [])
        : config.allowed_modes).catch(() => null);
    ...
    let resolution = preferredModePane
        ? await resolveCanonicalPaneFromPaneTarget(preferredModePane.pane, cwd).then((resolved) => (resolved.paneTarget
    ...
    const updateStateForAttempt = (success, reason) => {
        if (guard.dedupeKey)
            state.recent_keys[guard.dedupeKey] = now;
...
//# sourceMappingURL=tmux-injection.js.map