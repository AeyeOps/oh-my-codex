...
async function maybeRecoverStaleLock(lockDir) {
    try {
...
async function withRalphResumeLock(stateDir, fn) {
    ...
    while (true) {
        try {
            ...
            if (err.code !== 'EEXIST')
                throw error;
            if (await maybeRecoverStaleLock(lockDir))
                continue;
...
function isTerminalRalphPhase(value) {
    return RALPH_TERMINAL_PHASES.has(safeString(value).trim().toLowerCase());
}
function isActiveRalphCandidate(state) {
...
async function readCurrentOmxSessionId(stateDir, env = process.env) {
...
function resolveResumePane(env = process.env) {
    const injectedPane = captureTmuxPaneFromEnv(env);
    if (env !== process.env && injectedPane)
        return injectedPane;
    return resolveCodexPane() || injectedPane || '';
}
function bindCurrentPane(state, nowIso, env = process.env) {
...
async function scanMatchingRalphCandidates(stateDir, currentOmxSessionId, payloadSessionId, payloadThreadId) {
    ...
    for (const entry of entries) {
        if (!entry.isDirectory() || !SESSION_ID_PATTERN.test(entry.name) || entry.name === currentOmxSessionId)
            continue;
        const path = join(sessionsRoot, entry.name, 'ralph-state.json');
        if (!existsSync(path))
            continue;
        const state = await readJson(path);
        if (!isActiveRalphCandidate(state))
            continue;
        ...
        if (ownerSessionId) {
            if (!payloadSessionId || ownerSessionId !== payloadSessionId)
                continue;
        }
...
export async function reconcileRalphSessionResume({ stateDir, payloadSessionId, payloadThreadId = '', env = process.env, hooks, }) {
    const lockedResult = await withRalphResumeLock(stateDir, async () => {
        ...
        if (currentRalphState && currentRalphState.active === true) {
            let changed = false;
            const updated = { ...currentRalphState };
            ...
            if (typeof updated.owner_codex_thread_id === 'string'
        ...
        const candidates = await scanMatchingRalphCandidates(stateDir, currentOmxSessionId, normalizedPayloadSessionId, normalizedPayloadThreadId);
...
//# sourceMappingURL=ralph-session-resume.js.map