...
import { buildCapturePaneArgv, normalizeTmuxCapture, paneHasActiveTask, paneLooksReady, } from '../tmux-hook-engine.js';
/**
...
async function readBridgeDispatchRequests(stateDir, teamName) {
    ...
    return parsed.records
        ...
        if (metadataTeam && metadataTeam !== teamName)
            return null;
        return {
            ...
            updated_at: safeString(record.delivered_at).trim()
...
async function withDispatchLock(teamDirPath, fn) {
    return await withLockDirectory(join(teamDirPath, 'dispatch', '.lock'), `Timed out acquiring dispatch lock for ${teamDirPath}`, fn);
}
...
export async function drainPendingTeamDispatch({ cwd, stateDir = resolveBridgeStateDir(cwd), logsDir = join(cwd, '.omx', 'logs'), maxPerTick = 5, injector = injectDispatchRequest, } = {}) {
...
//# sourceMappingURL=team-dispatch.js.map