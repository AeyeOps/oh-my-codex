...
export function buildExpectedManagedTmuxSessionName(cwd, sessionId) {
...
export function resolveInvocationSessionId(payload) {
...
function readNativeSessionId(sessionState) {
...
export async function resolveManagedSessionContext(cwd, payload, { allowTeamWorker = true } = {}) {
    ...
    try {
        ...
        const expectedTmuxSessionName = buildExpectedManagedTmuxSessionName(authoritativeSessionCwd, canonicalSessionId || invocationSessionId);
...
export async function isManagedOmxSession(cwd, payload, options = {}) {
    const context = await resolveManagedSessionContext(cwd, payload, options);
    return context.managed === true;
}
export async function verifyManagedPaneTarget(paneId, cwd, payload, { allowTeamWorker = true } = {}) {
...
async function readManagedPaneCommandState(paneTarget) {
    try {
...
function paneLooksLikeManagedAgent({ currentCommand, startCommand }) {
    if (/\bomx\b.*\bhud\b.*--watch/i.test(startCommand))
        return false;
    if (startCommand.includes('codex'))
        return true;
    return currentCommand === 'codex' || currentCommand === 'node' || currentCommand === 'npx';
}
function paneLooksLikeRetainableManagedAnchor({ currentCommand, startCommand }) {
    if (/\bomx\b.*\bhud\b.*--watch/i.test(startCommand))
        return false;
    if (currentCommand === 'codex')
        return true;
    if ((currentCommand === 'node' || currentCommand === 'npx') && startCommand.includes('codex'))
        return true;
    return false;
}
function paneLooksLikeDetachedManagedWrapperFallback({ currentCommand, startCommand }) {
    if (/\bomx\b.*\bhud\b.*--watch/i.test(startCommand))
        return false;
    return currentCommand === 'node' || currentCommand === 'npx';
}
function parseManagedSessionPaneRows(stdout) {
...
function selectManagedSessionPane(rows, { allowWrapperFallback = false } = {}) {
    ...
    if (activeCanonical)
        return activeCanonical.paneId;
    if (canonicalRows[0]?.paneId)
        return canonicalRows[0].paneId;
    if (!allowWrapperFallback)
        return '';
    ...
    if (activeWrapperFallback)
        return activeWrapperFallback.paneId;
    return wrapperFallbackRows[0]?.paneId || '';
}
export async function resolveManagedCurrentPane(cwd, payload, { allowTeamWorker = false } = {}) {
...
export async function resolveManagedSessionPane(cwd, payload) {
    ...
    if (!expectedSession)
        return '';
    try {
        const panesResult = await runProcess('tmux', ['list-panes', '-s', '-t', expectedSession, '-F', '#{pane_id}\t#{pane_active}\t#{pane_current_command}\t#{pane_start_command}'], 2000);
...
export async function resolveManagedPaneFromAnchor(anchorPane, cwd, payload, { allowTeamWorker = false } = {}) {
    ...
    if (!verdict.ok)
        return '';
    const commandState = await readManagedPaneCommandState(paneTarget);
    if (commandState.lookupFailed)
        return paneTarget;
    if (paneLooksLikeRetainableManagedAnchor(commandState))
        return paneTarget;
    try {
        const sessionName = safeString(verdict.paneSessionName || verdict.managedContext?.expectedTmuxSessionName).trim();
        if (!sessionName)
            return '';
        const panesResult = await runProcess('tmux', ['list-panes', '-s', '-t', sessionName, '-F', '#{pane_id}\t#{pane_active}\t#{pane_current_command}\t#{pane_start_command}'], 2000);
...
//# sourceMappingURL=managed-tmux.js.map