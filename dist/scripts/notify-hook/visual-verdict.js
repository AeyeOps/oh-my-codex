...
export async function maybePersistVisualVerdict({ cwd, payload, stateDir, logsDir, sessionId, turnId }) {
    const output = safeString(payload?.['last-assistant-message'] || payload?.last_assistant_message || '');
    if (!output)
        return;
    ...
    try {
        ...
        await writeFile(join(verdictDir, 'latest-verdict.json'), JSON.stringify(entry, null, 2));
        await logNotifyHookEvent(logsDir, {
            ...entry,
            level: 'info',
...
//# sourceMappingURL=visual-verdict.js.map