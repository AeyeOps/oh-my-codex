...
import { buildCapturePaneArgv, buildPaneInModeArgv, buildPaneCurrentCommandArgv, buildSendKeysArgv, isPaneRunningShell, paneHasActiveTask, paneLooksReady, } from '../tmux-hook-engine.js';
...
export async function evaluatePaneInjectionReadiness(paneTarget, { skipIfScrolling = false, captureLines = 80, requireRunningAgent = true, requireReady = true, requireIdle = true, requireObservableState = false, requireCaptureEvidence = undefined, } = {}) {
    ...
    const buildReadinessResult = (ok, reason, paneCapture, readinessEvidence) => ({
        ok,
        sent: false,
        reason,
        paneTarget: target,
...
export async function sendPaneInput({ paneTarget, prompt, submitKeyPresses = 2, submitDelayMs = 0, typePrompt = true, }) {
    const target = safeString(paneTarget).trim();
    if (!target) {
...
//# sourceMappingURL=team-tmux-guard.js.map