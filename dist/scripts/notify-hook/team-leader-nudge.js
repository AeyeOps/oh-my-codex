...
import { resolveLeaderNudgeIntent, } from './orchestration-intent.js';
...
export function resolveFallbackProgressStallThresholdMs() {
    ...
    if (parsed !== null && parsed >= 10_000 && parsed <= 60 * 60_000)
        return parsed;
    return 120_000;
}
export function resolveWorkerTurnStallThresholdMs() {
    ...
    if (parsed !== null && parsed >= 10_000 && parsed <= 10 * 60_000)
        return parsed;
    return 30_000;
}
...
function classifyLeaderActionState({ allWorkersIdle = false, workerPanesAlive = false, taskCounts = {}, teamProgressStalled = false, } = {}) {
    ...
    if (terminalWaitingOnLeader)
        return 'done_waiting_on_leader';
    if (stalledWaitingOnLeader)
        return 'stuck_waiting_on_leader';
    if (pendingFollowUpTasks)
        return 'still_actionable';
    return 'still_actionable';
}
function buildLeaderActionGuidance(teamName, { allWorkersIdle = false, workerPanesAlive = false, taskCounts = {}, leaderActionState = 'still_actionable', } = {}) {
    const pending = Number.isFinite(taskCounts.pending) ? taskCounts.pending : 0;
    const blocked = Number.isFinite(taskCounts.blocked) ? taskCounts.blocked : 0;
    const inProgress = Number.isFinite(taskCounts.in_progress) ? taskCounts.in_progress : 0;
    const pendingFollowUpTasks = allWorkersIdle && pending > 0 && blocked === 0 && inProgress === 0;
...
function buildIdleContextText(teamName, { allWorkersIdle = false, workerPanesAlive = false, taskCounts = {}, leaderActionState = 'still_actionable', } = {}) {
    const pending = Number.isFinite(taskCounts.pending) ? taskCounts.pending : 0;
    const blocked = Number.isFinite(taskCounts.blocked) ? taskCounts.blocked : 0;
    const inProgress = Number.isFinite(taskCounts.in_progress) ? taskCounts.in_progress : 0;
    const pendingFollowUpTasks = allWorkersIdle && pending > 0 && blocked === 0 && inProgress === 0;
...
export async function checkWorkerPanesAlive(tmuxTarget, workerPaneIds = []) {
    ...
    try {
        ...
        const lines = (result.stdout || '')
            ...
            .map(l => l.trim())
            .filter(Boolean);
        const workerPaneIdSet = new Set(Array.isArray(workerPaneIds)
...
async function syncScopedTeamStateFromPhase(teamStatePath, teamName, phaseSnapshot, nowIso) {
    if (!phaseSnapshot || !phaseSnapshot.terminal)
        return false;
    try {
...
async function resolveCurrentSessionId(stateDir) {
    const fromEnv = safeString(process.env.OMX_SESSION_ID
...
async function readWorkerHeartbeatSnapshot(stateDir, teamName, workerName) {
    if (!workerName)
        return { turn_count: null, missing: true };
    ...
    try {
        ...
        for (const entry of taskFiles) {
            try {
                ...
                if (Object.hasOwn(taskCounts, status))
                    taskCounts[status] += 1;
...
async function readTeamProgressSnapshot(stateDir, teamName, workerNames) {
    const [taskSnapshot, workerSnapshot] = await Promise.all([
        ...
        Promise.all(workerNames.map(async (workerName) => {
    ...
    const missingSignalWorkers = workerSnapshot.filter(({ status_missing, heartbeat_missing }) => status_missing || heartbeat_missing).length;
    return {
        ...taskSnapshot,
        workerSnapshot,
...
async function workerHasOwnedStartedTask(stateDir, teamName, workerName) {
    ...
    try {
        ...
        for (const entry of taskFiles) {
            try {
                const parsed = JSON.parse(await readFile(join(tasksDir, entry), 'utf-8'));
                if (safeString(parsed?.owner).trim() !== workerName)
                    continue;
...
async function getAckWithoutStartEvidence(stateDir, teamName, msg) {
    ...
    if (!fromWorker || fromWorker === 'leader-fixed')
        return null;
    if (!isAckLikeMailboxBody(msg.body))
        return null;
...
export async function maybeNudgeTeamLeader({ cwd, stateDir, logsDir, preComputedLeaderStale, allowFreshMailboxNudges = true, source = 'notify_hook', }) {
    ...
    for (const teamName of candidateTeamNames) {
        ...
        try {
            ...
            if (existsSync(srcPath)) {
                ...
                if (Array.isArray(raw && raw.workers))
                    workers = raw.workers;
            }
        }
        catch {
        ...
        if (currentSessionId && ownerSessionId && ownerSessionId !== currentSessionId)
            continue;
        let mailbox = null;
        try {
        ...
        if (!tmuxSession && !canonicalLeaderPaneId)
            continue;
        let tmuxTarget = canonicalLeaderPaneId;
        if (canonicalLeaderPaneId) {
            const resolvedLeaderTarget = await resolvePaneTarget({ type: 'pane', value: canonicalLeaderPaneId }, '', canonicalLeaderPaneId, '', {}).catch(() => null);
        ...
        const teamProgressStalled = progressSnapshot.workRemaining
            && paneStatus.alive
            && !allWorkersIdle
            && !progressChanged
            && stalledForMs >= stallThresholdMs;
        const hasFreshProgressEvidence = progressSnapshot.workRemaining
            && stalledForMs < stallThresholdMs;
