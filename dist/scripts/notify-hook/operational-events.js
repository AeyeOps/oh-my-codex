...
export function classifyExecCommand(command) {
    ...
    for (const segment of shellSegments(source)) {
        if (SEARCH_SEGMENT_RE.test(segment))
            continue;
...
export function buildOperationalContext({ cwd, normalizedEvent, sessionId = '', sessionName = '', text = '', output = '', command = '', toolName = '', status = '', issueNumber, prNumber, prUrl, errorSummary, extra = {}, }) {
