...
async function initWorkerFixture() {
...
describe('handleTeamWorkerPostToolUseSuccess', () => {
    it('creates a safe worker checkpoint, ledger entries, leader signal, and dedupe marker', async () => {
        ...
        const dedupe = JSON.parse(await readFile(join(fixture.stateRoot, 'team', 'demo-team', 'workers', 'worker-1', 'posttooluse-dedupe.json'), 'utf-8'));
        assert.equal(dedupe.keys.includes(result.dedupeKey), true);
        const ledger = JSON.parse(await readFile(join(fixture.cwd, '.omx', 'reports', 'team-commit-hygiene', 'demo-team.ledger.json'), 'utf-8'));
...
//# sourceMappingURL=team-worker-posttooluse.test.js.map