...
import { appendTeamCommitHygieneEntries } from '../../team/commit-hygiene.js';
...
function readToolUseId(payload) {
...
async function hasGitOperationInProgress(cwd) {
    ...
    for (const path of paths) {
        if (existsSync(join(gitDir, path)))
            return path;
    }
    return null;
}
...
function isProtectedCheckpointPath(path) {
    const normalized = path.replace(/\\/g, '/');
    if (PROTECTED_PATH_EXACT.has(normalized))
        return true;
    if (normalized.includes('/posttooluse-dedupe.json') || normalized.endsWith('/posttooluse-dedupe.json'))
        return true;
    if (PROTECTED_PATH_PREFIXES.some((prefix) => normalized.startsWith(prefix)))
        return true;
    if (PROTECTED_PATH_SUFFIXES.some((suffix) => normalized.endsWith(suffix)))
        return true;
    return false;
}
async function readTeamConfig(stateRoot, teamName) {
    return readJsonIfExists(join(stateRoot, 'team', teamName, 'config.json'));
}
function readLeaderArtifactCwd(config, fallbackCwd) {
...
async function readLeaderHeadObserved(config, fallbackCwd) {
    const leaderCwd = safeString(config?.leader_cwd ?? config?.leaderCwd).trim();
    if (leaderCwd) {
...
async function readTeamPhaseTerminal(stateRoot, teamName) {
...
async function writeHeartbeat(stateRoot, teamName, workerName, nowIso) {
...
async function readDedupeMarker(path) {
    const parsed = await readJsonIfExists(path);
    const keys = Array.isArray(parsed?.keys) ? parsed.keys.filter((key) => typeof key === 'string') : [];
    const entries = Array.isArray(parsed?.entries)
        ? parsed.entries.filter((entry) => !!entry && typeof entry === 'object' && typeof entry.dedupe_key === 'string')
        : [];
    return {
        version: 1,
...
async function writeDedupeMarker(path, params) {
...
async function classifyCleanScaffolding(cwd, leaderHeadObserved, workerHeadAfter) {
    if (!leaderHeadObserved)
        return { status: 'skipped', reason: 'no_leader_head_observed' };
    if (!workerHeadAfter)
        return { status: 'skipped', reason: 'no_worker_head' };
    if (leaderHeadObserved === workerHeadAfter)
        return { status: 'noop', reason: 'worker_already_at_leader' };
    const leaderAncestor = await gitMaybe(cwd, ['merge-base', '--is-ancestor', leaderHeadObserved, workerHeadAfter]);
    if (leaderAncestor.ok)
        return { status: 'noop', reason: 'leader_is_worker_ancestor' };
    const workerAncestor = await gitMaybe(cwd, ['merge-base', '--is-ancestor', workerHeadAfter, leaderHeadObserved]);
    if (workerAncestor.ok)
        return { status: 'skipped', reason: 'worker_behind_leader_defer_to_leader_integration' };
    return { status: 'skipped', reason: 'non_fast_forward_defer_to_leader_integration' };
}
async function unstageCheckpointablePaths(cwd, paths) {
    if (paths.length === 0)
        return;
...
async function checkpointIfEligible(cwd, workerName) {
...
export async function handleTeamWorkerPostToolUseSuccess(payload, cwd, env = process.env) {
    try {
        if (readHookEvent(payload) !== 'PostToolUse')
            return { handled: false, status: 'skipped', reason: 'not_posttooluse', operationKinds: [] };
        if (readToolName(payload) !== 'Bash')
            return { handled: false, status: 'skipped', reason: 'not_bash', operationKinds: [] };
        if (readExitCode(payload) !== 0)
            return { handled: false, status: 'skipped', reason: 'nonzero_exit', operationKinds: [] };
        const parsedWorker = parseTeamWorkerEnv(env.OMX_TEAM_WORKER);
        if (!parsedWorker)
            return { handled: false, status: 'skipped', reason: 'missing_worker_env', operationKinds: [] };
        ...
        const operationKinds = ['auto_checkpoint'];
        await appendLedger({
            teamName,
            workerName,
            cwd,
        ...
        if (workerHeadAfter)
            operationKinds.push('leader_integration_attempt');
...
//# sourceMappingURL=team-worker-posttooluse.js.map