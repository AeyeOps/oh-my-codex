...
export function extractLimitPct(limit) {
    ...
    if (typeof limit === 'number' || typeof limit === 'string')
        return clampPct(asNumber(limit));
...
export function injectLanguageReminder(prompt, sourceText) {
    ...
    if (!hasNonLatinScript(sourceText))
        return basePrompt;
    if (basePrompt.includes(LANGUAGE_REMINDER_MARKER))
        return basePrompt;
