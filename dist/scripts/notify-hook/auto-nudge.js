...
import { getScopedStateDirsForCurrentSession, getScopedStatePath, readJsonIfExists, readScopedJsonIfExists, readdir, writeScopedJson, } from './state-io.js';
...
import { isManagedOmxSession, resolveManagedCurrentPane, resolveManagedPaneFromAnchor, resolveManagedSessionPane, resolveInvocationSessionId, verifyManagedPaneTarget, } from './managed-tmux.js';
...
export function isBlockedAutoApprovalInput(text, blockedInputs = DEEP_INTERVIEW_BLOCKED_APPROVAL_INPUTS) {
    ...
    if (exactMatches.has(normalized))
        return true;
    if (prefixedMatches.some((prefix) => normalized.startsWith(`${prefix} `)))
        return true;
...
function matchesNormalizedPatterns(normalizedText, normalizedPatterns) {
    if (!normalizedText || normalizedPatterns.length === 0)
        return false;
    const tail = normalizedText.slice(-800);
    const lines = tail.split('\n').filter((line) => line.trim());
    const hotZone = lines.slice(-3).join('\n');
    if (normalizedPatterns.some((pattern) => hotZone.includes(pattern)))
        return true;
...
export function resolveEffectiveAutoNudgeResponse(response) {
    ...
    if (!normalized)
        return DEFAULT_AUTO_NUDGE_RESPONSE;
...
export async function loadAutoNudgeConfig() {
    ...
    if (!raw || typeof raw !== 'object')
        return normalizeAutoNudgeConfig(null);
...
function detectStallPatternWithOptions(text, patterns, currentPhase = '', options = {}) {
    ...
    if (!normalized)
        return false;
    const normalizedPatterns = normalizePatternList(patterns);
    if (!matchesNormalizedPatterns(normalized, normalizedPatterns))
        return false;
    if (!usesDefaultStallPatterns(patterns))
        return true;
    if (options.allowPermissionSeeking !== true && looksLikePermissionSeekingContinuation(normalized))
        return false;
    if (safeString(currentPhase).trim().toLowerCase() === 'planning')
        return false;
...
export async function resolveNudgePaneTarget(stateDir, cwd = '', payload = undefined) {
    ...
    if (managedCurrentPane)
        return managedCurrentPane;
    const invocationSessionId = resolveInvocationSessionId(payload);
...
export async function maybeAutoNudge({ cwd, stateDir, logsDir, payload }) {
    ...
    if (!managedSession) {
        if (sourceName === 'notify-fallback-watcher-stall')
            return;
        await logTmuxHookEvent(logsDir, {
            timestamp: new Date().toISOString(),
            type: 'auto_nudge_skipped',
    ...
    try {
        ...
        if (skillState?.phase === 'completing' && !detected)
            return;
        if (!detected || !paneId)
            return;
...
//# sourceMappingURL=auto-nudge.js.map