...
import { getSessionTokenUsage, getQuotaUsage, normalizeInputMessages, } from './notify-hook/payload-parser.js';
import { getScopedStatePath, readCurrentSessionId, readScopedJsonIfExists, getScopedStateDirsForCurrentSession, normalizeNotifyState, pruneRecentTurns, readdir, } from './notify-hook/state-io.js';
...
import { maybeAutoNudge, resolveNudgePaneTarget, isDeepInterviewStateActive, isDeepInterviewInputLockActive, syncSkillStateFromTurn, } from './notify-hook/auto-nudge.js';
...
import { buildOperationalContext, deriveAssistantSignalEvents, readRepositoryMetadata, resolveOperationalSessionName, } from './notify-hook/operational-events.js';
import { parseTeamWorkerEnv, resolveTeamStateDirForWorker, updateWorkerHeartbeat, maybeNotifyLeaderAllWorkersIdle, maybeNotifyLeaderWorkerIdle, } from './notify-hook/team-worker.js';
...
function summarizeIdleNotificationMessage(message) {
...
function classifyIdleNotificationPhase(message) {
...
function buildIdleNotificationFingerprint(payload) {
...
function isTurnCompletePayload(payload) {
...
async function main() {
    ...
    try {
        if (!workerStateRootResolved)
            throw new Error('worker_state_root_unresolved');
        ...
        if (turnId) {
            ...
            const dedupeState = normalizeNotifyState(await readScopedJsonIfExists(stateDir, 'notify-hook-state.json', dedupeSessionId, null));
    ...
    const latestInputPreview = safeString(normalizedInputMessages.length > 0
                    ...
                    if (state.active) {
                        ...
                        if (maxIterations !== null && maxIterations > 0 && nextIteration >= maxIterations) {
                            const currentPhase = typeof state.current_phase === 'string'
                                ...
                                : '';
                            const isActiveRalphProgress = ((f === 'ralph-state.json' || state.mode === 'ralph')
    ...
    if (!isTeamWorker) {
        ...
        try {
            ...
            if (quotaUsage) {
                if (quotaUsage.fiveHourLimitPct !== null)
                    metrics.five_hour_limit_pct = quotaUsage.fiveHourLimitPct;
                if (quotaUsage.weeklyLimitPct !== null)
                    metrics.weekly_limit_pct = quotaUsage.weeklyLimitPct;
            }
    ...
    if (!isTeamWorker) {
        try {
            ...
            const { shouldSendIdleNotification, recordIdleNotificationSent, shouldSendSessionIdleHookEvent, recordSessionIdleHookEventSent, } = await import('../notifications/idle-cooldown.js');
...
//# sourceMappingURL=notify-hook.js.map