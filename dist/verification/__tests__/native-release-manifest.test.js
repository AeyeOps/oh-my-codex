...
describe('native release manifest generator', () => {
    it('annotates Linux libc variants and sorts musl assets before glibc fallbacks', async () => {
        ...
        try {
            ...
            const manifest = JSON.parse(await readFile(outputPath, 'utf-8'));
            assert.deepEqual(manifest.assets.map((asset) => asset.archive), [muslArchive, glibcArchive]);
            assert.deepEqual(manifest.assets.map((asset) => asset.libc), ['musl', 'glibc']);
            assert.deepEqual(manifest.assets.map((asset) => asset.target), ['x86_64-unknown-linux-musl', 'x86_64-unknown-linux-gnu']);
        }
        finally {
...
//# sourceMappingURL=native-release-manifest.test.js.map