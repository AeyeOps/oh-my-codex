...
const { buildMaintainerCloseComment, collectLinkedLocalIssueNumbers, } = require(join(process.cwd(), '.github', 'scripts', 'dev-merge-issue-close.cjs'));
describe('dev merge issue close workflow', () => {
    ...
    it('extracts only explicitly linked local issues from PR title/body close keywords', () => {
        assert.deepEqual(collectLinkedLocalIssueNumbers({
    ...
    it('dedupes repeated references and provides a standard maintainer close comment', () => {
        ...
        assert.equal(buildMaintainerCloseComment({ prNumber: 1550 }), 'Closing automatically because PR #1550 was merged into `dev` and explicitly referenced this issue in the PR title or body.');
    });
});
//# sourceMappingURL=dev-merge-issue-close-workflow.test.js.map