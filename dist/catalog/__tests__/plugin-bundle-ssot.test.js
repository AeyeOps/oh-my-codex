...
describe("plugin bundle SSOT contract", () => {
    ...
    it("fails check mode when plugin MCP metadata drifts from canonical first-party specs", async () => {
        ...
        try {
            await writeFile(join(fixtureRoot, "plugins", "oh-my-codex", ".mcp.json"), `${JSON.stringify({ mcpServers: {} }, null, 2)}\n`);
            await assert.rejects(() => syncPluginMirror({ root: fixtureRoot, check: true }), /plugin_bundle_metadata_out_of_sync[\s\S]*kind=mcp-manifest/);
        }
        finally {
    ...
    it("fails when a root skill directory is not represented by the catalog policy", async () => {
        const fixtureRoot = await copyBundleFixture();
        try {
            await cp(join(fixtureRoot, "skills", "help"), join(fixtureRoot, "skills", "uncataloged-skill"), { recursive: true });
            await assert.rejects(() => syncPluginMirror({ root: fixtureRoot, check: true }), /canonical_skill_catalog_out_of_sync[\s\S]*uncataloged-skill/);
        }
        finally {
            await rm(fixtureRoot, { recursive: true, force: true });
        }
...
//# sourceMappingURL=plugin-bundle-ssot.test.js.map