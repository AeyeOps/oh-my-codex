...
async function listRelativeFiles(dir, base = dir) {
    ...
    const files = await Promise.all(entries.map(async (entry) => {
        ...
        if (entry.isDirectory())
            return listRelativeFiles(fullPath, base);
        if (entry.isFile())
...
export async function compareDirectoryMirror(expectedDir, actualDir, options = {}) {
...
export async function compareSkillMirror(expectedSkillsDir, actualSkillsDir, expectedSkillNames, options = {}) {
    ...
    for (const skillName of sortedExpectedSkillNames) {
        const mismatch = await compareDirectoryMirror(join(expectedSkillsDir, skillName), join(actualSkillsDir, skillName), options);
        if (mismatch) {
            return {
...
export async function assertSkillMirror(expectedSkillsDir, actualSkillsDir, expectedSkillNames, options = {}) {
    const mismatch = await compareSkillMirror(expectedSkillsDir, actualSkillsDir, expectedSkillNames, options);
    if (!mismatch)
        return;
    throw new Error([
...
//# sourceMappingURL=skill-mirror.js.map