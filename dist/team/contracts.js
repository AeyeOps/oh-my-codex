...
export const TEAM_DISPATCH_REQUEST_STATUSES = ['pending', 'notified', 'delivered', 'failed'];
export const TEAM_TERMINAL_DISPATCH_REQUEST_STATUSES = new Set(['delivered', 'failed']);
export const TEAM_DISPATCH_REQUEST_STATUS_TRANSITIONS = {
    pending: ['notified', 'failed'],
    notified: ['delivered', 'failed'],
    delivered: [],
    failed: [],
};
export function isTeamDispatchRequestStatus(status) {
    return TEAM_DISPATCH_REQUEST_STATUSES.includes(status);
}
export function isTerminalTeamDispatchRequestStatus(status) {
    return TEAM_TERMINAL_DISPATCH_REQUEST_STATUSES.has(status);
}
export function canTransitionTeamDispatchRequestStatus(from, to) {
...
export function isTeamWorkerIntegrationStatus(status) {
    return TEAM_WORKER_INTEGRATION_STATUSES.includes(status);
}
