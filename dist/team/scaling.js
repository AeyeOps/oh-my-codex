...
import { sanitizeTeamName, isTmuxAvailable, waitForWorkerReady, dismissTrustPromptIfPresent, sendToWorker, isWorkerAlive, getWorkerPanePid, teardownWorkerPanes, buildWorkerStartupCommand, resolveTeamWorkerCliPlan, } from './tmux-session.js';
import { execFileSync, spawnSync } from 'child_process';
import { teamReadConfig as readTeamConfig, teamSaveConfig as saveTeamConfig, teamWriteWorkerIdentity as writeWorkerIdentity, teamReadManifest as readTeamManifestV2, teamNormalizePolicy as normalizeTeamPolicy, teamReadWorkerStatus as readWorkerStatus, teamWriteWorkerStatus as writeWorkerStatus, teamWithScalingLock as withScalingLock, teamAppendEvent as appendTeamEvent, teamCreateTask as createStateTask, teamListTasks as listTasks, teamMarkDispatchRequestNotified as markDispatchRequestNotified, teamReadDispatchRequest as readDispatchRequest, teamTransitionDispatchRequest as transitionDispatchRequest, } from './team-ops.js';
import { queueInboxInstruction, waitForDispatchReceipt, } from './mcp-comm.js';
import { generateInitialInbox, buildTriggerDirective, writeWorkerRoleInstructionsFile, writeWorkerWorktreeRootAgentsFile, removeWorkerWorktreeRootAgentsFile, } from './worker-bootstrap.js';
import { loadRolePrompt } from './role-router.js';
import { composeRoleInstructionsForRole } from '../agents/native-config.js';
import { codexPromptsDir } from '../utils/paths.js';
import { parseTeamWorkerLaunchArgs, resolveTeamWorkerLaunchArgs, resolveAgentDefaultModel, resolveAgentReasoningEffort, } from './model-contract.js';
...
function assertScalingEnabled(env = process.env) {
    if (!isScalingEnabled(env)) {
        throw new Error(`Dynamic scaling is disabled. Set ${OMX_TEAM_SCALING_ENABLED_ENV}=1 to enable.`);
    }
}
function resolveInstructionStateRoot(worktreePath) {
    return worktreePath ? WORKTREE_TRIGGER_STATE_ROOT : undefined;
}
function resolveLegacyScaledTeamWorktreeMode(config) {
    if (config.worktree_mode)
        return config.worktree_mode;
    if (config.workspace_mode !== 'worktree')
        return { enabled: false };
    const workersWithMetadata = config.workers.filter((worker) => worker.worktree_path || worker.worktree_branch || typeof worker.worktree_detached === 'boolean');
    ...
    const branchPrefixes = new Set(workersWithMetadata
...
function resolveScaleUpWorktreeMode(config) {
    if (config.workspace_mode !== 'worktree')
        return { enabled: false };
    try {
...
async function notifyWorkerPaneOutcome(sessionName, workerIndex, message, paneId, workerCli) {
    try {
...
export async function scaleUp(teamName, count, agentType, tasks, cwd, env = process.env) {
    ...
    return await withScalingLock(sanitized, leaderCwd, async () => {
        ...
        const rollbackScaleUp = async (error, context = {}) => {
        ...
        for (let i = 0; i < count; i++) {
            ...
            const workerTaskRoles = persistedTasks.filter(t => t.owner === workerName).map(t => t.role).filter(Boolean);
            ...
            const cmd = buildWorkerStartupCommand(sanitized, workerIndex, workerLaunchArgs, workerCwd, extraEnv, workerCliPlan[i], undefined, runtimeRole);
            ...
            if (result.status !== 0) {
                return await rollbackScaleUp(`Failed to create tmux pane for ${workerName}: ${(result.stderr || '').trim()}`, { workerName, worktreePath: workerWorkspace?.worktreePath });
            }
            ...
            if (dispatchPolicy.dispatch_mode === 'hook_preferred_with_fallback' && queued.request_id) {
                ...
                if (receipt && (receipt.status === 'notified' || receipt.status === 'delivered')) {
                    ...
                    if (receipt?.status === 'failed') {
                        if (fallback.ok) {
                            await transitionDispatchRequest(sanitized, queued.request_id, 'failed', 'failed', { last_reason: `fallback_confirmed_after_failed_receipt:${fallback.reason}` }, leaderCwd).catch(() => { });
                            outcome = {
                                ok: true,
                            ...
                            await transitionDispatchRequest(sanitized, queued.request_id, 'failed', 'failed', { last_reason: `fallback_attempted_but_unconfirmed:${fallback.reason}` }, leaderCwd).catch(() => { });
                            outcome = {
                                ok: false,
                                transport: fallback.transport,
                        ...
                        const marked = await markDispatchRequestNotified(sanitized, queued.request_id, { last_reason: `fallback_confirmed:${fallback.reason}` }, leaderCwd);
                        if (!marked) {
                            await transitionDispatchRequest(sanitized, queued.request_id, 'failed', 'failed', { last_reason: `fallback_confirmed_after_failed_receipt:${fallback.reason}` }, leaderCwd).catch(() => { });
                        }
                        outcome = {
                        ...
                        if (current) {
                            await transitionDispatchRequest(sanitized, queued.request_id, current.status, 'failed', { last_reason: `fallback_attempted_but_unconfirmed:${fallback.reason}` }, leaderCwd).catch(() => { });
                        }
                        outcome = {
                            ok: false,
                            transport: fallback.transport,
...
export async function scaleDown(teamName, cwd, options = {}, env = process.env) {
    ...
    return await withScalingLock(sanitized, leaderCwd, async () => {
        ...
        const detachedWorktreesToRollback = targetWorkers
            .filter((worker) => worker.worktree_created === true
            ...
            worktreePath: resolve(worker.worktree_path),
            detached: true,
            branchName: null,
            created: true,
            reused: false,
...
function resolveWorkerLaunchArgsForScaling(env, agentType, preferredReasoning) {
    const inheritedArgs = [];
