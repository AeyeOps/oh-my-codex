import { createTeamState, isTerminalPhase, transitionPhase, } from './orchestrator.js';
export function inferPhaseTargetFromTaskCounts(taskCounts, options = {}) {
...
function defaultPersistedPhaseState() {
    return {
...
function toTeamState(phaseState) {
...
function buildTransitionPath(from, to) {
    if (from === to)
        return [];
    if (to === 'team-verify') {
        if (from === 'team-plan')
...
export function reconcilePhaseStateForMonitor(persisted, target) {
