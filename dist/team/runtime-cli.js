...
async function promptStaleCleanup(summary) {
...
export function resolveRuntimeCliStateRoot(cwd, env = process.env) {
    return resolveCanonicalTeamStateRoot(cwd, env);
}
export function collectTaskResults(stateRoot, teamName) {
...
export function buildTerminalCliResult(stateRoot, teamName, phase, workerCount, startTimeMs) {
    ...
    return {
        ...
        notice: `[runtime-cli] phase=${phase} reached terminal state; preserving team state for inspection. `
...
export function normalizeAgentTypes(raw, workerCount) {
...
async function main() {
    ...
    try {
        ...
        try {
            ...
            runtime = await startTeam(teamName, tasks.map(t => t.subject).join('; '), agentType, workerCount, tasks, cwd, { confirmStaleCleanup: promptStaleCleanup });
        }
        finally {
            if (typeof previousCliMap === 'string')
                process.env.OMX_TEAM_WORKER_CLI_MAP = previousCliMap;
            else
                delete process.env.OMX_TEAM_WORKER_CLI_MAP;
        }
    }
    catch (err) {
