...
function isLocalExplorationTask(text) {
...
function isDependencyEvaluationTask(text) {
...
function inferLaneIntent(text) {
    if (BUILD_FIX_INTENT.test(text) && /\b(?:fix|resolve|repair)\b|(?:수정|해결)/i.test(text))
        return 'build-fix';
    if (DEBUG_INTENT.test(text))
        return 'debug';
    if (REVIEW_INTENT.test(text))
        return 'review';
    if (PRIMARY_TEST_INTENT.test(text))
        return 'verification';
    if (PRIMARY_DOCS_INTENT.test(text) || DOCS_INTENT.test(text))
        return 'docs';
    if (DESIGN_INTENT.test(text))
        return 'design';
    if (CLEANUP_INTENT.test(text))
        return 'cleanup';
