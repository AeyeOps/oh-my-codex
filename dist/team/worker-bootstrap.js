...
function buildWorkerRootAgentsBackupPath(teamStateRoot, teamName, workerName, worktreePath) {
    ...
    return gitPath
        ? gitPath
        : join(teamStateRoot, "team", teamName, "workers", workerName, "root-agents-backup.json");
}
export function generateWorkerRootAgentsContent(options) {
...
function isTracked(worktreePath, fileName) {
    try {
...
async function ensureGitInfoExcludePattern(worktreePath, pattern) {
...
export async function writeWorkerWorktreeRootAgentsFile(options) {
    ...
    const backupPath = buildWorkerRootAgentsBackupPath(options.teamStateRoot, options.teamName, options.workerName, options.worktreePath);
    await mkdir(dirname(backupPath), { recursive: true });
    await writeFile(backupPath, JSON.stringify(backup, null, 2), "utf-8");
    await writeFile(agentsPath, generateWorkerRootAgentsContent(options), "utf-8");
    return agentsPath;
}
export async function removeWorkerWorktreeRootAgentsFile(teamName, workerName, teamStateRoot, worktreePath) {
    const agentsPath = join(worktreePath, "AGENTS.md");
    const backupPath = buildWorkerRootAgentsBackupPath(teamStateRoot, teamName, workerName, worktreePath);
    let backup = null;
    try {
...
function buildVerificationSection(taskDescription) {
 ...
 */
export function generateWorkerOverlay(teamName) {
...
function dropShadowedSkillReferenceLines(content, shadowedSkillNames) {
    if (shadowedSkillNames.size === 0)
        return content;
 ...
 */
export async function writeTeamWorkerInstructionsFile(teamName, cwd, overlay) {
    const baseParts = [];
    ...
    const projectSkillNames = new Set(installedSkills
 ...
 */
export async function writeWorkerRoleInstructionsFile(teamName, workerName, cwd, baseInstructionsPath, workerRole, rolePromptContent) {
 ...
 */
export async function removeTeamWorkerInstructionsFile(teamName, cwd) {
    const outPath = join(cwd, ".omx", "state", "team", teamName, "worker-agents.md");
...
function renderDelegationContract(task) {
...
function renderDelegationContracts(tasks) {
 ...
 */
export function generateInitialInbox(workerName, teamName, agentType, tasks, options = {}) {
...
export function generateTaskAssignmentInbox(workerName, teamName, taskOrId, taskDescriptionArg) {
    const task = typeof taskOrId === "string"
        ? { id: taskOrId, description: taskDescriptionArg ?? "" }
        : taskOrId;
    const taskId = task.id;
    const taskDescription = task.description;
    const delegationSection = renderDelegationContracts([task]);
 ...
 */
export function generateShutdownInbox(teamName, workerName) {
 ...
 */
export function generateTriggerMessage(workerName, teamName, teamStateRoot = ".omx/state") {
    return buildTriggerDirective(workerName, teamName, teamStateRoot).text;
}
export function buildTriggerDirective(workerName, teamName, teamStateRoot = ".omx/state") {
 ...
 * Always < 200 characters, ASCII-safe.
 */
export function generateMailboxTriggerMessage(workerName, teamName, count, teamStateRoot = ".omx/state") {
    return buildMailboxTriggerDirective(workerName, teamName, count, teamStateRoot).text;
}
export function buildMailboxTriggerDirective(workerName, teamName, count, teamStateRoot = ".omx/state") {
...
export function generateLeaderMailboxTriggerMessage(teamName, fromWorker, teamStateRoot = ".omx/state") {
    return buildLeaderMailboxTriggerDirective(teamName, fromWorker, teamStateRoot).text;
}
export function buildLeaderMailboxTriggerDirective(teamName, fromWorker, teamStateRoot = ".omx/state") {
