...
export declare const TEAM_DISPATCH_REQUEST_STATUSES: readonly ["pending", "notified", "delivered", "failed"];
export type TeamDispatchRequestStatus = (typeof TEAM_DISPATCH_REQUEST_STATUSES)[number];
export declare const TEAM_TERMINAL_DISPATCH_REQUEST_STATUSES: ReadonlySet<TeamDispatchRequestStatus>;
export declare const TEAM_DISPATCH_REQUEST_STATUS_TRANSITIONS: Readonly<Record<TeamDispatchRequestStatus, readonly TeamDispatchRequestStatus[]>>;
export declare function isTeamDispatchRequestStatus(status: unknown): status is TeamDispatchRequestStatus;
export declare function isTerminalTeamDispatchRequestStatus(status: TeamDispatchRequestStatus): boolean;
export declare function canTransitionTeamDispatchRequestStatus(from: TeamDispatchRequestStatus, to: TeamDispatchRequestStatus): boolean;
export declare const TEAM_WORKER_INTEGRATION_STATUSES: readonly ["idle", "integrated", "integration_failed", "cherry_pick_conflict", "rebase_conflict"];
export type TeamWorkerIntegrationStatus = (typeof TEAM_WORKER_INTEGRATION_STATUSES)[number];
export declare function isTeamWorkerIntegrationStatus(status: unknown): status is TeamWorkerIntegrationStatus;
