...
async function markLatestMailboxDispatchDelivered(teamName, worker, messageId, cwd) {
...
function parseOptionalMetadata(value) {
    if (value === undefined)
        return undefined;
...
function summarizeEvent(event) {
    ...
    return {
        ...
        intent: typeof event.intent === 'string'
            ? event.intent
...
async function readLatestLeaderProgressEvidenceMs(cwd, teamName) {
...
function buildStallState(teamName, summary, snapshot, recentEvents, phaseState, leaderAttention, authoritativeLeaderState) {
    ...
    const terminalLeaderDecisionPending = pendingTaskCount === 0
        && idleState.all_workers_idle === true
        && liveWorkers.length > 0;
    const stuckLeaderDecisionPending = blockedCount > 0 && pendingCount === 0 && inProgressCount === 0 && idleState.all_workers_idle === true;
    ...
    const leaderAttentionPending = !teamTerminal && (unreadLeaderMessageCount > 0
        || pendingLeaderDispatchCount > 0
        || leaderSessionStopped);
    const leaderStale = !teamTerminal && leaderSessionStopped;
    const teamStalled = stalledWorkers.length > 0
...
function readLegacyMailboxMessage(teamName, workerName, messageId, cwd) {
        ...
        switch (operation) {
            case 'send-message': {
                ...
                const matching = outcome.message_id
                    ...
                    : [...messages].reverse().find((message) => !beforeIds.has(message.message_id)
                ...
                const legacySenderFallback = !matching && !legacyMatching
                    ? [...readLegacyMailboxMessages(teamName, toWorker, cwd)].reverse().find((message) => !beforeIds.has(String(message.message_id || ''))
                        && String(message.from_worker || '') === fromWorker
                        && String(message.to_worker || '') === toWorker) ?? null
                    : null;
            ...
            case 'read-stall-state': {
                ...
                const recentEvents = selectRecentEvents(events);
                const unreadLeaderMessageCount = leaderMailboxMessages.filter((message) => {
                    const deliveredAt = typeof message?.delivered_at === 'string' ? message.delivered_at.trim() : '';
                    return deliveredAt.length === 0;
                }).length;
                const pendingLeaderDispatchCount = leaderDispatchRequests.filter((request) => request.status === 'pending' || request.status === 'notified').length;
                const leaderStoppedAtMs = leaderAttention?.leader_session_stopped_at
                    ? Date.parse(leaderAttention.leader_session_stopped_at)
                    : Number.NaN;
                const leaderActivityAfterStop = Number.isFinite(leaderStoppedAtMs)
                    && Number.isFinite(latestLeaderActivityMs)
                    && latestLeaderActivityMs > leaderStoppedAtMs;
                const leaderStopObserved = leaderAttention?.leader_session_active === false
