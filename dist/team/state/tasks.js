...
function isClaimLeaseExpired(claim, now = new Date()) {
    if (!claim?.leased_until)
        return false;
...
export async function claimTask(taskId, workerName, expectedVersion, deps) {
    ...
    const lock = await deps.withTaskClaimLock(deps.teamName, taskId, deps.cwd, async () => {
        ...
        if (v.status === 'in_progress') {
            if (!isClaimLeaseExpired(v.claim))
                return { ok: false, error: 'claim_conflict' };
            v.owner = undefined;
            v.claim = undefined;
            v.status = 'pending';
...
export async function reclaimExpiredTaskClaim(taskId, deps) {
    const lock = await deps.withTaskClaimLock(deps.teamName, taskId, deps.cwd, async () => {
        const current = await deps.readTask(deps.teamName, taskId, deps.cwd);
        if (!current)
            return { ok: false, error: 'task_not_found' };
        ...
        if (v.status === 'completed' || v.status === 'failed')
            return { ok: false, error: 'already_terminal' };
        if (v.status !== 'in_progress' || !v.claim)
            return { ok: true, task: v, reclaimed: false };
        if (!isClaimLeaseExpired(v.claim))
            return { ok: false, error: 'lease_active' };
        const updated = {
            ...v,
            status: 'pending',
            owner: undefined,
        ...
        await deps.writeAtomic(deps.taskFilePath(deps.teamName, taskId, deps.cwd), JSON.stringify(updated, null, 2));
        return { ok: true, task: updated, reclaimed: true };
    });
    if (!lock.ok)
        return { ok: false, error: 'claim_conflict' };
    return lock.value;
