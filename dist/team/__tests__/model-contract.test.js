...
import { collectInheritableTeamWorkerArgs, isLowComplexityAgentType, resolveAgentDefaultModel, resolveAgentReasoningEffort, resolveTeamWorkerLaunchArgs, resolveTeamLowComplexityDefaultModel, } from '../model-contract.js';
function expectedLowComplexityModel() {
...
describe('team model contract', () => {
    ...
    it('keeps exactly one canonical model flag with precedence env > inherited > fallback', () => {
        assert.deepEqual(resolveTeamWorkerLaunchArgs({
...
describe('resolveTeamWorkerLaunchArgs - teammate reasoning allocation', () => {
    it('injects preferred reasoning when explicit reasoning is absent', () => {
        ...
        assert.deepEqual(result, ['-c', 'model_reasoning_effort="low"', '--model', expectedLowComplexityModel()]);
    });
