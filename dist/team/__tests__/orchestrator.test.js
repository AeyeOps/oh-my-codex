...
import { canResumeTeamState, createTeamState, isTerminalPhase, isValidTransition, transitionPhase, } from '../orchestrator.js';
function moveToVerify(initial) {
...
describe('transitionPhase', () => {
    ...
    it('enforces bounded team-fix loop and transitions to failed with reason', () => {
        ...
        assert.match(overflow.phase_transitions.at(-1)?.reason ?? '', /team-fix loop limit reached \(2\)/);
    });
    it('prevents transitions from terminal phases', () => {
        ...
        assert.throws(() => transitionPhase(complete, 'team-fix'), /Cannot transition from terminal phase: complete/);
    });
