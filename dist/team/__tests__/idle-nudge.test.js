...
async function withFakeTmux(run) {
    ...
    try {
        ...
        await run({
            tmuxLogPath,
            setCaptureSequence: async (tokens) => {
        ...
        if (typeof prevCaptureSeq === 'string')
            ...
            delete process.env.OMX_CAPTURE_SEQ_FILE;
        if (typeof prevCaptureToken === 'string')
            process.env.OMX_CAPTURE_TOKEN = prevCaptureToken;
        else
            delete process.env.OMX_CAPTURE_TOKEN;
        if (typeof prevFailSendKeys === 'string')
            process.env.OMX_FAIL_SEND_KEYS = prevFailSendKeys;
        else
            delete process.env.OMX_FAIL_SEND_KEYS;
        if (typeof prevFailCapture === 'string')
            process.env.OMX_FAIL_CAPTURE = prevFailCapture;
        else
            delete process.env.OMX_FAIL_CAPTURE;
        await rm(root, { recursive: true, force: true });
    }
}
async function withMockedNow(initialNow, run) {
...
describe('idle-nudge', () => {
    it('uses an explicit next-action default nudge message', () => {
        assert.equal(DEFAULT_NUDGE_CONFIG.message, 'Next: read your inbox/mailbox, continue your assigned task now, and if blocked send the leader a concrete status update.');
