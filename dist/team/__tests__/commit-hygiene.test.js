...
import { buildTeamCommitHygieneContext, renderTeamCommitHygieneMarkdown, resolveTeamCommitHygieneArtifactCwd, TEAM_OPERATIONAL_COMMIT_KINDS, TEAM_OPERATIONAL_COMMIT_STATUSES, } from '../commit-hygiene.js';
describe('team commit hygiene vocabulary', () => {
    it('includes canonical operation and status vocabulary in structured context', () => {
        ...
        assert.deepEqual(context.vocabulary.operational_commit_kinds.map((term) => term.value), [...TEAM_OPERATIONAL_COMMIT_KINDS]);
        assert.deepEqual(context.vocabulary.operational_commit_statuses.map((term) => term.value), [...TEAM_OPERATIONAL_COMMIT_STATUSES]);
        assert.match(context.vocabulary.operational_commit_kinds.find((term) => term.value === 'auto_checkpoint')?.description ?? '', /worker-local checkpoint commit/i);
        assert.match(context.vocabulary.operational_commit_statuses.find((term) => term.value === 'conflict')?.description ?? '', /reconciliation/i);
    });
    it('renders the vocabulary before runtime ledger details in markdown', () => {
        ...
        assert.ok(markdown.indexOf('## Commit Hygiene Vocabulary') < markdown.indexOf('## Runtime Operational Ledger'), 'vocabulary should explain ledger terms before ledger entries are shown');
    });
});
describe('team commit hygiene artifact root', () => {
    it('derives leader-facing artifacts from persisted team metadata instead of worker cwd', () => {
        ...
        assert.equal(resolveTeamCommitHygieneArtifactCwd({
            workers: [],
