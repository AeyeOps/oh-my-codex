...
import { resolveCanonicalTeamStateRoot, resolveWorkerNotifyTeamStateRoot, resolveWorkerNotifyTeamStateRootPath, resolveWorkerTeamStateRoot, resolveWorkerTeamStateRootPath, } from '../state-root.js';
...
describe('state-root', () => {
    ...
    async function writeTeamMetadata(stateRoot, teamName, filename, workers, extra = {}) {
    ...
    async function writeIdentity(stateRoot, teamName, workerName, worktreePath, teamStateRoot = stateRoot) {
    ...
    it('resolves worker root from OMX_TEAM_STATE_ROOT only when identity validates', async () => {
        ...
        assert.equal(await resolveWorkerTeamStateRootPath(worktree, { teamName: 'team-a', workerName: 'worker-1' }, {
    ...
    it('rejects non-git worker notify roots without a matching canonical marker', async () => {
        ...
        await mkdir(worktree, { recursive: true });
        assert.equal(await resolveWorkerNotifyTeamStateRootPath(worktree, { teamName: 'team-a', workerName: 'worker-1' }, {
            OMX_TEAM_STATE_ROOT: stateRoot,
        }), null);
    ...
    it('notify-hook worker state resolution reuses the non-git resolver', async () => {
        ...
        try {
            ...
            const stateRoot = join(worktree, '.omx', 'state');
            await writeIdentity(stateRoot, 'team-a', 'worker-1', worktree);
            assert.equal(await resolveTeamStateDirForWorker(worktree, { teamName: 'team-a', workerName: 'worker-1' }), null);
        }
        finally {
