...
import { buildFollowupStaffingPlan, isApprovedExecutionFollowupShortcut, resolveAvailableAgentTypes, } from '../followup-planner.js';
describe('followup-planner', () => {
    ...
    it('builds concrete team staffing guidance from the available roster', () => {
        const plan = buildFollowupStaffingPlan('team', 'Fix flaky integration tests and update README', ['executor', 'test-engineer', 'writer'], { workerCount: 3, fallbackRole: 'executor' });
        ...
        assert.ok(plan.allocations.some((allocation) => allocation.reason.includes('specialist') || allocation.reason.includes('verification')));
    ...
    it('builds concrete ralph staffing guidance from the available roster', () => {
        const plan = buildFollowupStaffingPlan('ralph', 'Investigate auth regression and verify the fix', ['architect', 'debugger', 'executor', 'test-engineer']);
    ...
    it('allocates explore plus researcher lanes for mixed local+official-doc follow-up', () => {
        const plan = buildFollowupStaffingPlan('team', 'Find which files implement session refresh in the repo, then research the official docs and version compatibility notes for the auth SDK', ['executor', 'explore', 'researcher', 'verifier'], { workerCount: 3, fallbackRole: 'executor' });
    ...
    it('allocates explore plus dependency-expert lanes for mixed local+dependency evaluation follow-up', () => {
        const plan = buildFollowupStaffingPlan('team', 'Map which local packages currently handle logging, then compare replacement npm packages for license risk, maintenance, and migration path', ['dependency-expert', 'executor', 'explore', 'verifier'], { workerCount: 3, fallbackRole: 'executor' });
        assert.ok(plan.allocations.some((allocation) => allocation.role === 'explore' && allocation.reason.includes('primary')));
    ...
    it('allocates explore first plus dependency-expert for local-usage-plus-upgrade follow-up phrasing', () => {
        const plan = buildFollowupStaffingPlan('team', 'Check how we use this SDK today and whether we should upgrade it', ['dependency-expert', 'executor', 'explore', 'verifier'], { workerCount: 3, fallbackRole: 'executor' });
        assert.ok(plan.allocations.some((allocation) => allocation.role === 'explore' && allocation.reason.includes('primary')));
        assert.ok(plan.allocations.some((allocation) => allocation.role === 'dependency-expert' && allocation.reason.includes('specialist')));
    });
    it('recognizes short approved team follow-up shortcuts in English and Korean', () => {
        assert.equal(isApprovedExecutionFollowupShortcut('team', 'team', { planningComplete: true }), true);
        assert.equal(isApprovedExecutionFollowupShortcut('team', 'team으로 해줘', { planningComplete: true }), true);
    });
    it('rejects short follow-up shortcuts when the prior execution context conflicts', () => {
        assert.equal(isApprovedExecutionFollowupShortcut('team', 'team', {
