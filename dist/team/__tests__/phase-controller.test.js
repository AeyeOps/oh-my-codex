...
describe('phase-controller', () => {
    it('infers complete when all tasks terminal and none failed', () => {
        assert.equal(inferPhaseTargetFromTaskCounts({ pending: 0, blocked: 0, in_progress: 0, failed: 0 }), 'complete');
    });
    it('infers team-fix when all tasks terminal but failures exist', () => {
        assert.equal(inferPhaseTargetFromTaskCounts({ pending: 0, blocked: 0, in_progress: 0, failed: 2 }), 'team-fix');
    });
    it('infers team-verify when terminal tasks still need verification evidence', () => {
        assert.equal(inferPhaseTargetFromTaskCounts({ pending: 0, blocked: 0, in_progress: 0, failed: 0 }, { verificationPending: true }), 'team-verify');
    });
