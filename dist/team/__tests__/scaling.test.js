...
describe('Monotonic worker index counter', () => {
    it('initTeamState sets next_worker_index to workerCount + 1', async () => {
        ...
        try {
            ...
            // Verify on disk
            const diskCfg = JSON.parse(readFileSync(join(cwd, '.omx', 'state', 'team', 'idx-test', 'config.json'), 'utf8'));
    ...
    it('next_worker_index is present in manifest.v2.json', async () => {
        ...
        try {
            ...
            const manifest = JSON.parse(readFileSync(join(cwd, '.omx', 'state', 'team', 'manif-test', 'manifest.v2.json'), 'utf8'));
...
describe('scaleUp', () => {
    ...
    it('returns error for invalid count', async () => {
        const result = await scaleUp('test', 0, 'executor', [], '/tmp', { OMX_TEAM_SCALING_ENABLED: '1' });
    ...
    it('returns error for negative count', async () => {
        const result = await scaleUp('test', -1, 'executor', [], '/tmp', { OMX_TEAM_SCALING_ENABLED: '1' });
        assert.equal(result.ok, false);
        if (!result.ok) {
            assert.match(result.error, /count must be a positive integer/);
        }
    ...
    it('persists scaled-up task roles in canonical task state and inbox ids', async () => {
        ...
        try {
            ...
            const result = await scaleUp('scale-up-role', 1, 'executor', [{ subject: 'document routing report only', description: 'document routing report only', owner: 'worker-2', role: 'writer' }], cwd, { OMX_TEAM_SCALING_ENABLED: '1', OMX_TEAM_SKIP_READY_WAIT: '1' });
            ...
            const workerIdentity = JSON.parse(await readFile(join(cwd, '.omx', 'state', 'team', 'scale-up-role', 'workers', 'worker-2', 'identity.json'), 'utf-8'));
...
`);
            ...
            await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
            const result = await scaleUp('rollback-worktree', 1, 'executor', [{ subject: 'write docs', description: 'write docs', owner: 'worker-2', role: 'writer' }], cwd, { OMX_TEAM_SCALING_ENABLED: '1', OMX_TEAM_SKIP_READY_WAIT: '1' });
            assert.equal(result.ok, false);
            if (result.ok)
                return;
...
`);
            ...
            await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
            const result = await scaleUp('frontier-role', 1, 'executor', [{ subject: 'test routing report only', description: 'test routing report only', owner: 'worker-2', role: 'test-engineer' }], cwd, { OMX_TEAM_SCALING_ENABLED: '1', OMX_TEAM_SKIP_READY_WAIT: '1' });
            assert.equal(result.ok, true);
            if (!result.ok)
                return;
...
`);
            ...
            await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
            const result = await scaleUp('scale-up-layout', 1, 'executor', [], cwd, { OMX_TEAM_SCALING_ENABLED: '1', OMX_TEAM_SKIP_READY_WAIT: '1' });
            assert.equal(result.ok, true);
            if (!result.ok)
                return;
...
describe('scaleDown worktree AGENTS cleanup', () => {
    it('removes generated worktree-root AGENTS during scale-down', async () => {
        ...
        try {
            ...
            await writeFile(join(cwd, '.omx', 'state', 'team', 'scale-down-worktree', 'workers', 'worker-2', 'root-agents-backup.json'), JSON.stringify({ existed: true, tracked: false, previousContent: '# Tracked root instructions\n' }, null, 2), 'utf8');
            ...
            await saveTeamConfig(config, cwd);
            const result = await scaleDown('scale-down-worktree', cwd, { workerNames: ['worker-2'], force: true }, { OMX_TEAM_SCALING_ENABLED: '1' });
            assert.equal(result.ok, true);
            if (!result.ok)
                return;
