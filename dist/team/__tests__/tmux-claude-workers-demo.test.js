...
import { buildWorkerProcessLaunchSpec, resolveTeamWorkerCliPlan, translateWorkerLaunchArgsForCli, } from '../tmux-session.js';
describe('tmux claude workers demo', () => {
    ...
    describe('claude worker launch spec', () => {
        it('claude worker uses --dangerously-skip-permissions flag', () => {
            const spec = buildWorkerProcessLaunchSpec('demo-team', 4, ['--model', 'claude-sonnet-4-6'], '/tmp/workspace', {}, 'claude');
            assert.equal(spec.workerCli, 'claude');
            assert.ok(spec.args.includes('--dangerously-skip-permissions'), 'claude worker must include --dangerously-skip-permissions flag');
        });
        it('claude worker drops codex-specific flags', () => {
            ...
            const spec = buildWorkerProcessLaunchSpec('demo-team', 4, codexArgs, '/tmp/workspace', {}, 'claude');
        ...
        it('codex worker preserves launch args', () => {
            ...
            const spec = buildWorkerProcessLaunchSpec('demo-team', 1, launchArgs, '/tmp/workspace', {}, 'codex');
    ...
    describe('full demo scenario with 6 mixed workers', () => {
        it('creates correct launch specs for all workers in demo', () => {
            ...
            const cliMap = Array.from({ length: workerCount }, (_, i) => i + 1 <= pivot ? 'codex' : 'claude').join(',');
            ...
            for (let i = 1; i <= workerCount; i++) {
                ...
                const spec = buildWorkerProcessLaunchSpec(teamName, i, launchArgs, cwd, { OMX_TEAM_STATE_ROOT: `${cwd}/.omx/state` }, workerCli);
                ...
                if (workerCli === 'claude') {
                    assert.deepEqual(spec.args, ['--dangerously-skip-permissions'], `worker-${i} (claude) should only have skip-permissions flag`);
                }
                else {
                    assert.ok(spec.args.includes('--model'), `worker-${i} (codex) should have --model flag`);
                }
            }
        });
    ...
    describe('edge cases for mixed worker teams', () => {
        ...
        it('rejects invalid CLI map with empty entries', () => {
            assert.throws(() => resolveTeamWorkerCliPlan(3, [], { OMX_TEAM_WORKER_CLI_MAP: 'codex,,claude' }), /Empty entries are not allowed/);
        });
        it('rejects CLI map with wrong length', () => {
            assert.throws(() => resolveTeamWorkerCliPlan(3, [], { OMX_TEAM_WORKER_CLI_MAP: 'codex,claude' }), /expected 1 or 3/);
        });
