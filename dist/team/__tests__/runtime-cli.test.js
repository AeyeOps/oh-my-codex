...
describe('runtime-cli helpers', () => {
    it('normalizes per-worker providers and validates supported values', async () => {
        ...
        assert.deepEqual(runtimeCli.normalizeAgentTypes(['codex', 'gemini'], 2), ['codex', 'gemini']);
        assert.deepEqual(runtimeCli.normalizeAgentTypes(['gemini'], 3), ['gemini']);
        assert.throws(() => runtimeCli.normalizeAgentTypes(['codex', 'invalid'], 2), /Expected codex\\|claude\\|gemini/);
    });
    ...
    it('does not auto-force failed-task shutdown without explicit issue confirmation', async () => {
        ...
        try {
            ...
            assert.equal(existsSync(teamRoot), true);
            const runtimeCli = await loadRuntimeCliModule();
            await assert.rejects(() => runtimeCli.shutdownWithForceFallback('shutdown-confirm-required', cwd), /shutdown_confirm_issues_required:failed=1/);
            assert.equal(existsSync(teamRoot), true);
        }
        finally {
            if (typeof previousTeamStateRoot === 'string')
    ...
    it('preserves team state when building terminal output for completed teams', async () => {
        ...
        try {
            ...
            assert.equal(existsSync(teamRoot), true);
            const runtimeCli = await loadRuntimeCliModule();
            const result = runtimeCli.buildTerminalCliResult(runtimeCli.resolveRuntimeCliStateRoot(cwd), 'runtime-cli-preserve-complete', 'complete', 1, Date.now() - 1_000);
            assert.equal(existsSync(teamRoot), true);
    ...
    it('preserves team state when building terminal output for failed teams', async () => {
        ...
        try {
            ...
            assert.equal(existsSync(teamRoot), true);
            const runtimeCli = await loadRuntimeCliModule();
            const result = runtimeCli.buildTerminalCliResult(runtimeCli.resolveRuntimeCliStateRoot(cwd), 'runtime-cli-preserve-failed', 'failed', 1, Date.now() - 1_000);
            assert.equal(existsSync(teamRoot), true);
    ...
    it('reports cancelled terminal phases without deleting team state', async () => {
        ...
        try {
            ...
            assert.equal(existsSync(teamRoot), true);
            const runtimeCli = await loadRuntimeCliModule();
            const result = runtimeCli.buildTerminalCliResult(runtimeCli.resolveRuntimeCliStateRoot(cwd), 'runtime-cli-preserve-cancelled', 'cancelled', 1, Date.now() - 1_000);
            assert.equal(existsSync(teamRoot), true);
            assert.equal(result.exitCode, 1);
            assert.equal(result.output.status, 'failed');
