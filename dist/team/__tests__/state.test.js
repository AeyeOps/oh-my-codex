...
describe('team state', () => {
    ...
    it('dispatch bridge queue uses the same request id as the TS store', async () => {
        ...
        try {
            ...
            await writeFile(join(fakeBinDir, 'omx-runtime'), `#!/usr/bin/env bash
set -eu
printf '%s\n' "$*" >> "${runtimeLogPath}"
if [[ "\${1:-}" == "schema" ]]; then
...
`);
    ...
    it('resolves task/mailbox/approval paths under explicit OMX_TEAM_STATE_ROOT from a worker cwd (worker-env contamination regression)', async () => {
        ...
        try {
            ...
            if (typeof prevRoot === 'string')
                process.env.OMX_TEAM_STATE_ROOT = prevRoot;
            else
                delete process.env.OMX_TEAM_STATE_ROOT;
    ...
    it('sendDirectMessage recreates mailbox directory when missing', async () => {
        ...
        try {
            ...
            assert.equal(dm.to_worker, 'worker-2');
            assert.equal(existsSync(join(cwd, '.omx', 'state', 'team', 'team-msg-recreate-mailbox', 'mailbox', 'worker-2.json')), true);
        }
        finally {
            await rm(cwd, { recursive: true, force: true });
        }
    ...
    it('listTasks ignores malformed and id-mismatched task payloads', async () => {
        ...
        try {
            ...
            await writeFile(join(cwd, '.omx', 'state', 'team', 'team-list-validate', 'tasks', 'task-2.json'), JSON.stringify({
                id: '999',
                subject: 'mismatch',
                description: 'bad',
                status: 'pending',
            ...
            await writeFile(join(cwd, '.omx', 'state', 'team', 'team-list-validate', 'tasks', 'task-3.json'), JSON.stringify({ nope: true }, null, 2));
    ...
    it('getTeamSummary aggregates task counts correctly', async () => {
        ...
        try {
            ...
            const statusPath = join(cwd, '.omx', 'state', 'team', 'team-11', 'workers', 'worker-1', 'status.json');
