...
import { generateWorkerOverlay, applyWorkerOverlay, stripWorkerOverlay, generateWorkerRootAgentsContent, writeTeamWorkerInstructionsFile, writeWorkerRoleInstructionsFile, writeWorkerWorktreeRootAgentsFile, removeWorkerWorktreeRootAgentsFile, removeTeamWorkerInstructionsFile, generateInitialInbox, generateTaskAssignmentInbox, generateShutdownInbox, generateTriggerMessage, buildTriggerDirective, generateMailboxTriggerMessage, buildMailboxTriggerDirective, generateLeaderMailboxTriggerMessage, buildLeaderMailboxTriggerDirective, } from "../worker-bootstrap.js";
...
describe("worker bootstrap", () => {
    ...
    it("generateTaskAssignmentInbox includes delegation contract for follow-up task object", () => {
        const inbox = generateTaskAssignmentInbox("worker-3", "team-followup", {
            id: "42",
    ...
    it("writeWorkerRoleInstructionsFile preserves precomposed mini guidance as wrapper-only content", async () => {
        ...
        try {
            ...
            const basePath = await writeTeamWorkerInstructionsFile("mini-role-team", cwd, overlay);
            const composedRoleInstructions = composeRoleInstructionsForRole("writer", "---\ndescription: demo\n---\n\n<identity>You are Writer.</identity>", "gpt-5.4-mini");
            const outPath = await writeWorkerRoleInstructionsFile("mini-role-team", "worker-2", cwd, basePath, "writer", composedRoleInstructions);
            const content = await readFile(outPath, "utf8");
            assert.match(content, /<identity>You are Writer\.<\/identity>/);
    ...
    it("generateInitialInbox omits duplicated specialization when root AGENTS is canonical", () => {
        ...
        const inbox = generateInitialInbox("worker-1", "team-root-canonical", "writer", tasks, {
            workerRole: "writer",
            rolePromptContent: "<identity>You are Writer.</identity>",
