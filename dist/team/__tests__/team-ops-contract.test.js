...
function parseStateReExports(source) {
    const map = {};
...
describe('team/team-ops module contract', () => {
    ...
    it('re-exported bindings resolve to the exact state-layer implementations', () => {
        const teamOpsModule = teamOps;
        const stateModule = state;
        for (const [alias, stateName] of Object.entries(EXPECTED_STATE_RE_EXPORTS)) {
            assert.equal(teamOpsModule[alias], stateModule[stateName], `Expected team-ops export ${alias} to re-export state.${stateName}`);
        }
    });
});
//# sourceMappingURL=team-ops-contract.test.js.map