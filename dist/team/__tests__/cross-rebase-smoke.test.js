...
import { initTeamState, listMailboxMessages, readMonitorSnapshot, readTeamConfig, saveTeamConfig, writeWorkerStatus, } from '../state.js';
...
async function addWorktree(repo, branchName, prefix) {
...
async function configureWorkers(teamName, repo, workers) {
...
describe('cross-rebase smoke regression', () => {
    ...
    it('surfaces cross-rebase conflicts as wakeable and preserves cleanup/report evidence', async () => {
        ...
        try {
            ...
            assert.equal(leaderMailbox.some((message) => /rebase onto .* with -X ours failed/i.test(message.body)), true);
...
//# sourceMappingURL=cross-rebase-smoke.test.js.map