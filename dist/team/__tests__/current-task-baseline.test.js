...
import { assertCurrentTaskBranchAvailable, findActiveCurrentTaskByBranch, upsertCurrentTaskBaseline, } from '../current-task-baseline.js';
...
describe('current-task-baseline', () => {
    ...
    it('blocks duplicate branch creation when baseline points at another worktree path', async () => {
        ...
        try {
            ...
            assert.throws(() => assertCurrentTaskBranchAvailable(repo, 'feature/already-active', join(repo, '..', 'new-worktree')), /current_task_branch_guard:feature\/already-active:/);
        }
        finally {
            await rm(repo, { recursive: true, force: true });
        }
