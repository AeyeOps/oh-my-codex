...
async function addWorktree(repo, branchName, pathPrefix) {
...
async function withPromptModeCodexEnv(binDir, extraEnv, run) {
...
async function withMockTmuxFixture(options, run) {
...
async function withNativeWindowsPlatform(run) {
...
describe('runtime', () => {
    ...
    it('resolveWorkerLaunchArgsFromEnv preserves explicit model in either syntax', () => {
        assert.deepEqual(resolveWorkerLaunchArgsFromEnv({ OMX_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5' }, 'explore'), ['--model', 'gpt-5']);
        assert.deepEqual(resolveWorkerLaunchArgsFromEnv({ OMX_TEAM_WORKER_LAUNCH_ARGS: '--model=gpt-5.3' }, 'explore'), ['--model', 'gpt-5.3']);
    });
    it('resolveWorkerLaunchArgsFromEnv uses inherited leader model for all agent types', () => {
        const args = resolveWorkerLaunchArgsFromEnv({ OMX_TEAM_WORKER_LAUNCH_ARGS: '--no-alt-screen' }, 'executor', 'gpt-4.1');
    ...
    it('resolveWorkerLaunchArgsFromEnv uses inherited leader model over low-complexity default', () => {
        const args = resolveWorkerLaunchArgsFromEnv({ OMX_TEAM_WORKER_LAUNCH_ARGS: '--no-alt-screen' }, 'explore', 'gpt-4.1');
        assert.deepEqual(args, ['--no-alt-screen', '--model', 'gpt-4.1']);
    });
    it('resolveWorkerLaunchArgsFromEnv prefers explicit env model over inherited leader model', () => {
        assert.deepEqual(resolveWorkerLaunchArgsFromEnv({ OMX_TEAM_WORKER_LAUNCH_ARGS: '--model gpt-5' }, 'explore', 'gpt-4.1'), ['--model', 'gpt-5']);
    });
    ...
    it('resolveWorkerLaunchArgsFromEnv preserves explicit reasoning and logs source=explicit', () => {
        ...
        console.log = (...args) => { logs.push(args.join(' ')); };
        try {
            const args = resolveWorkerLaunchArgsFromEnv({ OMX_TEAM_WORKER_LAUNCH_ARGS: '-c model_reasoning_effort=\"high\" --no-alt-screen' }, 'explore');
            assert.deepEqual(args, ['--no-alt-screen', '-c', 'model_reasoning_effort="high"', '--model', expectedLowComplexityModel()]);
        }
        finally {
            console.log = originalLog;
        }
    ...
    it('waitForClaudeStartupEvidence requires first-start ACK/task progress before startup dispatch is treated as settled', async () => {
        ...
        try {
            ...
            await writeAtomic(join(cwd, '.omx', 'state', 'team', 'claude-startup', 'workers', 'worker-1', 'status.json'), JSON.stringify({
                state: 'working',
    ...
    it('waitForWorkerStartupEvidence ignores Codex ACK-only startup replies until work is claimed', async () => {
        ...
        try {
            ...
            await writeAtomic(join(cwd, '.omx', 'state', 'team', 'codex-startup', 'workers', 'worker-1', 'status.json'), JSON.stringify({
                state: 'working',
                current_task_id: 'task-1',
                updated_at: new Date().toISOString(),
            }, null, 2));
...
`,
    ...
    it('startTeam rejects nested team invocation inside worker context', async () => {
        ...
        try {
            await assert.rejects(() => startTeam('nested-a', 'task', 'executor', 1, [{ subject: 's', description: 'd' }], cwd), /nested_team_disallowed/);
        }
        finally {
            if (typeof prev === 'string')
                process.env.OMX_TEAM_WORKER = prev;
...
`, { mode: 0o755 });
    ...
    it('startTeam rejects duplicate active same-name team state without mutating existing files', async () => {
        ...
        try {
            ...
            await initTeamState('dup-team', 'existing task', 'executor', 1, cwd, undefined, { ...process.env, OMX_SESSION_ID: 'sess-existing-team' });
            ...
            await assert.rejects(() => withoutTeamWorkerEnv(() => startTeam('dup-team', 'replacement task', 'executor', 1, [{ subject: 'new subject', description: 'new description', owner: 'worker-1' }], cwd)), /team_name_conflict: active team state already exists/);
...
`,
                ...
                process.env.OMX_TEAM_SKIP_READY_WAIT = '1';
                const events = [];
                runtime = await withoutTeamWorkerEnv(() => startTeam('team-interactive-cleanup', 'interactive cleanup before worker spawn', 'executor', 1, [{ subject: 's', description: 'd', owner: 'worker-1' }], cwd, {
...
`,
    ...
    it('startTeam rejects dirty leader workspace before provisioning worker worktrees', async () => {
        ...
        try {
            process.env.OMX_TEAM_WORKER_LAUNCH_MODE = 'prompt';
            await assert.rejects(() => withoutTeamWorkerEnv(() => startTeam('team-dirty-preflight', 'reject dirty leader workspace', 'executor', 1, [{ subject: 's', description: 'd', owner: 'worker-1' }], repo, { worktreeMode: { enabled: true, detached: true, name: null } })), /leader_workspace_dirty_for_worktrees:.*M README\.md.*\?\? notes\.txt.*commit_or_stash_before_omx_team/s);
    ...
    it('startTeam runs worker MCP orphan cleanup before prompt worker spawn', async () => {
        ...
        await mkdir(binDir, { recursive: true });
        await writeFakePromptWorkerBinary(fakeCodexPath, `const capturePath = process.env.OMX_PROMPT_CLEANUP_CAPTURE_PATH;
...
setTimeout(() => {}, 5000);`);
        ...
        try {
            ...
            process.env.OMX_TEST_ALLOW_NONTTY_CODEX_PROMPT = '1';
            const started = await withoutTeamWorkerEnv(() => startTeam('team-prompt-cleanup', 'prompt cleanup before worker spawn', 'executor', 1, [{ subject: 's', description: 'd', owner: 'worker-1' }], cwd, {
                cleanupLaunchOrphanedMcpProcesses: async () => {
            ...
            if (typeof prevCapture === 'string')
                process.env.OMX_PROMPT_CLEANUP_CAPTURE_PATH = prevCapture;
            else
                delete process.env.OMX_PROMPT_CLEANUP_CAPTURE_PATH;
            if (typeof prevAllowNonTty === 'string')
                process.env.OMX_TEST_ALLOW_NONTTY_CODEX_PROMPT = prevAllowNonTty;
            else
                delete process.env.OMX_TEST_ALLOW_NONTTY_CODEX_PROMPT;
            await rm(cwd, { recursive: true, force: true });
        }
...
`, { mode: 0o755 });
        ...
        try {
            ...
            if (typeof prevCapture === 'string')
                ...
                delete process.env.OMX_GEMINI_ARGV_CAPTURE_PATH;
            await rm(cwd, { recursive: true, force: true });
        }
    });
...
`, { mode: 0o755 });
        ...
        try {
            ...
            let worker1Args = null;
            let worker2Args = null;
            for (let attempt = 0; attempt < 50; attempt += 1) {
            ...
            if (typeof prevStandardModel === 'string')
                process.env.OMX_DEFAULT_STANDARD_MODEL = prevStandardModel;
            else
                delete process.env.OMX_DEFAULT_STANDARD_MODEL;
            await rm(cwd, { recursive: true, force: true });
        }
...
`, { mode: 0o755 });
        ...
        try {
            ...
            if (typeof prevLogDir === 'string')
                ...
                delete process.env.OMX_TEST_LOG_DIR;
...
`, { mode: 0o755 });
        ...
        try {
            ...
            const identityPath = join(repo, '.omx', 'state', 'team', runtime.teamName, 'workers', 'worker-1', 'identity.json');
            const identity = JSON.parse(await readFile(identityPath, 'utf-8'));
    ...
    it('shutdownTeam force-kills prompt workers that ignore SIGTERM', async () => {
        ...
        const fakeCodexPath = join(binDir, 'codex');
        await mkdir(binDir, { recursive: true });
        await writeFakePromptWorkerBinary(fakeCodexPath, `
process.stdin.resume();
setInterval(() => {}, 1000);
process.on('SIGTERM', () => {
...
`);
        ...
        let workerPid = 0;
        try {
            runtime = await withPromptModeCodexEnv(binDir, {}, () => withoutTeamWorkerEnv(() => startTeam('team-prompt-stubborn', 'prompt-mode stubborn worker teardown', 'executor', 1, [{ subject: 's', description: 'd', owner: 'worker-1' }], cwd)));
            ...
            assert.ok(shutdownDurationMs < 10_000, `forced prompt-worker shutdown should skip the 15s ack wait (actual ${shutdownDurationMs}ms)`);
        }
        finally {
            if (runtime) {
                await shutdownTeam(runtime.teamName, cwd, { force: true }).catch(() => { });
...
`);
        ...
        let helperPid = 0;
        try {
            runtime = await withPromptModeCodexEnv(binDir, { OMX_HELPER_PID_PATH: helperPidPath }, () => withoutTeamWorkerEnv(() => startTeam('team-prompt-descendants', 'prompt-mode detached descendant teardown', 'executor', 1, [{ subject: 's', description: 'd', owner: 'worker-1' }], cwd)));
            ...
            assert.ok(shutdownDurationMs < 10_000, `forced descendant teardown should skip the 15s ack wait (actual ${shutdownDurationMs}ms)`);
        }
        finally {
            if (runtime) {
                await shutdownTeam(runtime.teamName, cwd, { force: true }).catch(() => { });
    ...
    it('monitorTeam returns correct task counts from state files', async () => {
        ...
        try {
            ...
            await updateWorkerHeartbeat('team-counts', 'worker-1', { pid: 111, last_turn_at: new Date().toISOString(), turn_count: 7, alive: true }, cwd);
            const statusPath = join(cwd, '.omx', 'state', 'team', 'team-counts', 'workers', 'worker-1', 'status.json');
    ...
    it('monitorTeam keeps phase in team-verify when completed code tasks lack verification evidence', async () => {
        ...
        try {
            ...
            const task = await createTask('team-verify-gate', {
                subject: 'code change',
            ...
            assert.equal(first?.recommendations.some((r) => r.includes(`task-${task.id}`) && r.includes('Verification evidence missing')), true);
            const taskPath = join(cwd, '.omx', 'state', 'team', 'team-verify-gate', 'tasks', `task-${task.id}.json`);
            const fromDisk = JSON.parse(await readFile(taskPath, 'utf-8'));
            fromDisk.result = [
                'Summary: done',
                'Verification:',
    ...
    it('monitorTeam deactivates root team-state.json when the local phase becomes terminal', async () => {
        ...
        try {
            ...
            await createTask('team-root-sync', {
                subject: 'code change',
                description: 'implement feature',
                status: 'completed',
                owner: 'worker-1',
            ...
            const rootState = JSON.parse(await readFile(rootStatePath, 'utf-8'));
    ...
    it('monitorTeam emits worker_state_changed, worker_idle, and task_completed events based on transitions', async () => {
        ...
        try {
            ...
            await writeAtomic(join(cwd, '.omx', 'state', 'team', 'team-events', 'tasks', `task-${t.id}.json`), JSON.stringify({ ...t, status: 'completed', owner: 'worker-1' }, null, 2));
            await writeAtomic(join(cwd, '.omx', 'state', 'team', 'team-events', 'workers', 'worker-1', 'status.json'), JSON.stringify({ state: 'idle', updated_at: new Date().toISOString() }, null, 2));
            await monitorTeam('team-events', cwd);
    ...
    it('monitorTeam persists integration ledger and cherry-picks unseen worker HEADs once', async () => {
        ...
        try {
            ...
            const integrationEvents = events.filter((e) => e.type === 'worker_merge_applied' || e.type === 'worker_cherry_pick_applied');
    ...
    it('monitorTeam integrates detached worker worktree by commit sha instead of merging leader HEAD into itself', async () => {
        ...
        try {
            ...
            assert.equal(leaderMailbox.some((message) => message.body.includes(`INTEGRATED: merged worker-1 (${workerHead.slice(0, 12)})`)
                && message.body.includes(leaderHeadAfter.slice(0, 12))), true);
            const ledgerPath = join(repo, '.omx', 'reports', 'team-commit-hygiene', 'team-merge-detached.ledger.json');
            const ledger = JSON.parse(await readFile(ledgerPath, 'utf-8'));
            assert.equal(ledger.entries.some((entry) => entry.operation === 'integration_merge'
                && entry.status === 'applied'
                && entry.source_commit === workerHead
                && entry.leader_head_before !== entry.leader_head_after), true);
        }
        finally {
            if (workerPath) {
                await rm(workerPath, { recursive: true, force: true });
...
`, { mode: 0o755 });
    ...
    it('monitorTeam rebases idle workers after integration lands on leader (cross-worker rebase)', async () => {
        ...
        try {
            ...
            const ledger = JSON.parse(await readFile(ledgerPath, 'utf-8'));
            assert.equal(ledger.entries.some((entry) => entry.operation === 'cross_rebase' && entry.worker_name === 'worker-2' && entry.status === 'applied'), true);
        }
        finally {
            if (worker1Path) {
    ...
    it('monitorTeam auto-resolves conflicts with -X theirs (worker wins on leader)', async () => {
        ...
        try {
            ...
            assert.notEqual(snapshot?.integrationByWorker?.['worker-1']?.status, 'cherry_pick_conflict', 'integration should not be permanently blocked');
    ...
    it('shutdownTeam clean fast path ignores worker shutdown ack files', async () => {
        ...
        try {
            ...
            const ackPath = join(cwd, '.omx', 'state', 'team', 'team-shutdown-clean-fast', 'workers', 'worker-1', 'shutdown-ack.json');
            await writeFile(ackPath, JSON.stringify({ status: 'reject', reason: 'stale ack', updated_at: '9999-01-01T00:00:00.000Z' }));
    ...
    it('shutdownTeam blocks when pending tasks remain (shutdown gate)', async () => {
        ...
        try {
            ...
            await createTask('team-shutdown-gate-pending', { subject: 'pending', description: 'd', status: 'pending' }, cwd);
            await assert.rejects(() => shutdownTeam('team-shutdown-gate-pending', cwd), /shutdown_gate_blocked:pending=1,blocked=0,in_progress=0,failed=0/);
    ...
    it('shutdownTeam honors legacy policy cleanup override after governance hydration', async () => {
        ...
        try {
            ...
            await createTask('team-shutdown-gate-legacy', { subject: 'pending', description: 'd', status: 'pending' }, cwd);
    ...
    it('shutdownTeam requires explicit issue confirmation when failed tasks remain', async () => {
        ...
        try {
            ...
            await createTask('team-shutdown-gate-failed', { subject: 'failed', description: 'd', status: 'failed' }, cwd);
            await assert.rejects(() => shutdownTeam('team-shutdown-gate-failed', cwd), /shutdown_confirm_issues_required:failed=1:rerun=omx team shutdown team-shutdown-gate-failed --confirm-issues/);
    ...
    it('shutdownTeam force=true bypasses shutdown gate and cleans up', async () => {
        ...
        try {
            ...
            await createTask('team-shutdown-gate-force', { subject: 'pending', description: 'd', status: 'pending' }, cwd);
    ...
    it('shutdownTeam force=true emits shutdown_gate_forced audit event', async () => {
        ...
        try {
            ...
            await createTask('team-gate-forced-event', { subject: 'pending', description: 'd', status: 'pending' }, cwd);
...
`,
    ...
    it('shutdownTeam returns rejection error when worker rejects shutdown and force is false', async () => {
        ...
        try {
            ...
            const ackPath = join(cwd, '.omx', 'state', 'team', 'team-reject', 'workers', 'worker-1', 'shutdown-ack.json');
            await writeFile(ackPath, JSON.stringify({ status: 'reject', reason: 'still working', updated_at: '9999-01-01T00:00:00.000Z' }));
    ...
    it('shutdownTeam emits shutdown_ack event when worker ack is received', async () => {
        ...
        try {
            ...
            const ackPath = join(cwd, '.omx', 'state', 'team', 'team-ack-evt', 'workers', 'worker-1', 'shutdown-ack.json');
            await writeFile(ackPath, JSON.stringify({ status: 'reject', reason: 'busy', updated_at: '9999-01-01T00:00:00.000Z' }));
            ...
            const ackEvents = events.filter((e) => e.type === 'shutdown_ack');
    ...
    it('shutdownTeam emits shutdown_ack event with accept reason for accepted acks', async () => {
        ...
        try {
            ...
            const ackPath = join(cwd, '.omx', 'state', 'team', 'team-ack-accept', 'workers', 'worker-1', 'shutdown-ack.json');
            await writeFile(ackPath, JSON.stringify({ status: 'accept', updated_at: '9999-01-01T00:00:00.000Z' }));
    ...
    it('shutdownTeam force=true ignores rejection and cleans up team state', async () => {
        ...
        try {
            ...
            const ackPath = join(cwd, '.omx', 'state', 'team', 'team-force', 'workers', 'worker-1', 'shutdown-ack.json');
            await writeFile(ackPath, JSON.stringify({ status: 'reject', reason: 'still working', updated_at: '9999-01-01T00:00:00.000Z' }));
    ...
    it('shutdownTeam ignores stale rejection ack from a prior request', async () => {
        ...
        try {
            ...
            const ackPath = join(cwd, '.omx', 'state', 'team', 'team-stale-ack', 'workers', 'worker-1', 'shutdown-ack.json');
            await writeFile(ackPath, JSON.stringify({ status: 'reject', reason: 'old ack', updated_at: '2000-01-01T00:00:00.000Z' }));
    ...
    it('shutdownTeam confirmIssues=true allows failed-task shutdown without worker ack handshake', async () => {
        ...
        try {
            ...
            await createTask('team-confirm-issues', { subject: 'failed', description: 'd', status: 'failed' }, cwd);
            const ackPath = join(cwd, '.omx', 'state', 'team', 'team-confirm-issues', 'workers', 'worker-1', 'shutdown-ack.json');
            await writeFile(ackPath, JSON.stringify({ status: 'reject', reason: 'should be ignored', updated_at: '9999-01-01T00:00:00.000Z' }));
...
`,
    ...
    it('shutdownTeam still requires confirm-issues on failed tasks', async () => {
        ...
        try {
            ...
            await createTask('team-normal-gate', { subject: 'failed', description: 'd', status: 'failed' }, cwd);
            await assert.rejects(() => shutdownTeam('team-normal-gate', cwd), /shutdown_confirm_issues_required:failed=1/);
    ...
    it('assignTask does not claim task when worker does not exist', async () => {
        ...
        try {
            ...
            const task = await createTask('team-missing-worker', { subject: 'x', description: 'd', status: 'pending', requires_code_change: false }, cwd);
            await assert.rejects(() => assignTask('team-missing-worker', 'worker-404', task.id, cwd), /Worker worker-404 not found in team/);
    ...
    it('assignTask rolls back claim when notification transport fails', async () => {
        ...
        try {
            ...
            const task = await createTask('team-notify-fail', { subject: 'x', description: 'd', status: 'pending', requires_code_change: false }, cwd);
            // Force notification transport to fail by clearing PATH so tmux is unavailable.
            await assert.rejects(() => withEmptyPath(() => assignTask('team-notify-fail', 'worker-1', task.id, cwd)), /worker_notify_failed/);
    ...
    it('assignTask rolls back claim when inbox write fails after claim', async () => {
        ...
        try {
            ...
            const task = await createTask('team-inbox-fail', { subject: 'x', description: 'd', status: 'pending', requires_code_change: false }, cwd);
            ...
            await assert.rejects(() => assignTask('team-inbox-fail', 'worker-1', task.id, cwd), /worker_assignment_failed:/);
    ...
    it('monitorTeam does not re-notify already-notified mailbox messages (issue #116)', async () => {
        ...
        try {
            ...
            assert.ok(diskSnap1.mailboxNotifiedByMessageId['msg-already-notified'], 'already-notified message must be preserved in snapshot after first poll');
            ...
            assert.ok(diskSnap2.mailboxNotifiedByMessageId['msg-already-notified'], 'already-notified message must remain in snapshot after second poll (no reset)');
        }
        finally {
            await rm(cwd, { recursive: true, force: true });
        }
    ...
    it('monitorTeam only notifies once per new message even without notified_at (issue #116)', async () => {
        ...
        try {
            ...
            assert.ok(diskSnap2);
            assert.ok(diskSnap2.mailboxNotifiedByMessageId['msg-unnotified'], 'notified message must be captured in snapshot after second poll');
            ...
            assert.ok(diskSnap3);
            assert.ok(diskSnap3.mailboxNotifiedByMessageId['msg-unnotified'], 'notified message must remain in snapshot on third poll (no duplicate notification)');
        }
        finally {
            await rm(cwd, { recursive: true, force: true });
        }
...
`,
    ...
    it('sendWorkerMessage hook-preferred path for leader waits for receipt then falls back to mailbox persistence', async () => {
        ...
        try {
            ...
            const notifiedMsg = mailbox.find((m) => m.notified_at);
