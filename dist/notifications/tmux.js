 ...
 */
export function sanitizeTmuxAlertText(raw) {
...
export function getCurrentTmuxSession() {
    ...
    if (process.env.TMUX) {
        try {
            ...
            const sessionName = execTmux(paneTargetSafe
 ...
 */
function detectTmuxSessionByPid() {
...
export function getCurrentTmuxPaneId() {
    ...
    if (process.env.TMUX && envPane && /^%\d+$/.test(envPane))
        return envPane;
