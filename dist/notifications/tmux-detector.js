 ...
 */
export function buildCapturePaneArgv(paneId, lines) {
...
export function analyzePaneContent(content) {
    ...
    const hasRateLimitMessage = lower.includes('rate limit') ||
        lower.includes('rate-limit') ||
        lower.includes('429');
    const isBlocked = lower.includes('waiting') ||
    ...
    if (lower.includes('>') || lower.includes('$'))
        confidence += 0.1;
    if (lower.includes('agent') || lower.includes('task'))
        confidence += 0.1;
 ...
 */
export function buildSendPaneArgvs(paneId, text, pressEnter = true) {
    ...
    const argvs = [['send-keys', '-t', paneId, '-l', '--', safe]];
    if (pressEnter) {
 ...
 */
export function getSubmitPresses(workerCli) {
...
export function sendToPane(paneId, text, pressEnter = true, deps = {}) {
