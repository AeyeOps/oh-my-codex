...
import { getIdleNotificationCooldownSeconds, shouldSendIdleNotification, recordIdleNotificationSent, shouldSendSessionIdleHookEvent, recordSessionIdleHookEventSent, shouldIncludeSessionIdleTmuxTail, recordSessionIdleTmuxTailSent, } from '../idle-cooldown.js';
...
describe('shouldSendIdleNotification', () => {
    ...
    it('allows a changed summary fingerprint immediately', () => {
        ...
        assert.equal(shouldSendIdleNotification(stateDir, sessionId, '{"phase":"idle","summary":"Waiting on user input"}'), true);
    });
...
describe('recordIdleNotificationSent', () => {
    ...
    it('writes a cooldown file with lastSentAt timestamp', () => {
        ...
        const content = JSON.parse(readFileSync(join(stateDir, 'idle-notif-cooldown.json'), 'utf-8'));
...
describe('session-idle hook event dedupe', () => {
    ...
    it('re-emits when the hook fingerprint changes', () => {
        ...
        assert.equal(shouldSendSessionIdleHookEvent(stateDir, sessionId, '{"phase":"idle","summary":"Waiting on user input"}'), true);
    });
});
