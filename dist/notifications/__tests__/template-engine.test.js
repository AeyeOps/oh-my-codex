...
import { computeTemplateVariables, interpolateTemplate, validateTemplate, getDefaultTemplate, } from '../template-engine.js';
...
describe('interpolateTemplate', () => {
    ...
    it('includes {{#if}} block content when var is truthy', () => {
        const result = interpolateTemplate('before{{#if reason}}\nreason: {{reason}}{{/if}}\nafter', makePayload({ reason: 'done' }));
        assert.ok(result.includes('reason: done'));
    });
    it('removes {{#if}} block when var is falsy', () => {
        const result = interpolateTemplate('before{{#if reason}}\nreason: {{reason}}{{/if}}\nafter', makePayload({ reason: undefined }));
...
describe('getDefaultTemplate', () => {
    const EVENTS = ['session-start', 'session-stop', 'session-end', 'session-idle', 'ask-user-question'];
    ...
    it('returns fallback for unknown event', () => {
        const tmpl = getDefaultTemplate('unknown-event');
