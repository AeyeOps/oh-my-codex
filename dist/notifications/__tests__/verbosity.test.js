...
import { getVerbosity, isEventAllowedByVerbosity, shouldIncludeTmuxTail, isEventEnabled, } from '../config.js';
...
describe('getVerbosity', () => {
    ...
    it('ignores invalid config value and falls back to default', () => {
        delete process.env.OMX_NOTIFY_VERBOSITY;
        assert.equal(getVerbosity(makeConfig({ verbosity: 'bogus' })), 'session');
    });
