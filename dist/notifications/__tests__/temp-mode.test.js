...
let tempCodexHome;
async function writeCodexConfig(contents) {
...
describe('notification temp mode', () => {
    ...
    it('shouldDispatchOpenClaw enforces temp-mode explicit selection and gateway matching', async () => {
        ...
        assert.equal(await shouldDispatchOpenClaw('session-end', activeNoOpenClaw, process.env), false);
        assert.equal(await shouldDispatchOpenClaw('session-end', activeWithOpenClaw, process.env), true);
        assert.equal(await shouldDispatchOpenClaw('session-end', activeWithCustomGateway, process.env), true);
        assert.equal(await shouldDispatchOpenClaw('session-end', activeWithWrongGateway, process.env), false);
        assert.equal(await shouldDispatchOpenClaw('session-end', null, process.env), true);
        assert.equal(await shouldDispatchOpenClaw('session-end', activeWithOpenClaw, { OMX_OPENCLAW: '0', CODEX_HOME: tempCodexHome }), false);
    });
});
//# sourceMappingURL=temp-mode.test.js.map