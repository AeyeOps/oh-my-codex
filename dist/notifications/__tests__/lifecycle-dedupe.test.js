...
import { shouldSendLifecycleNotification, recordLifecycleNotificationSent, shouldSendLifecycleHookBroadcast, recordLifecycleHookBroadcastSent, } from '../lifecycle-dedupe.js';
...
describe('lifecycle notification dedupe', () => {
    ...
    it('tracks hook dedupe separately from notification dedupe state', async () => {
        ...
        try {
            ...
            recordLifecycleNotificationSent(stateDir, payload);
            assert.equal(shouldSendLifecycleHookBroadcast(stateDir, payload.sessionId, 'keyword-detector', hookFingerprint), true);
            recordLifecycleHookBroadcastSent(stateDir, payload.sessionId, 'keyword-detector', hookFingerprint);
            assert.equal(shouldSendLifecycleHookBroadcast(stateDir, payload.sessionId, 'keyword-detector', hookFingerprint), false);
            assert.equal(shouldSendLifecycleNotification(stateDir, payload), false);
        }
        finally {
            await rm(stateDir, { recursive: true, force: true });
...
//# sourceMappingURL=lifecycle-dedupe.test.js.map