...
import { RateLimiter, captureReplyAcknowledgementSummary, formatReplyAcknowledgement, redactSensitiveTokens, sanitizeReplyInput, isReplyListenerProcess, normalizeReplyListenerConfig, pollDiscordOnce, pollTelegramOnce, resetReplyListenerTransientState, } from '../reply-listener.js';
...
function createMapping(platform) {
    return {
        platform,
...
function createHttpsRequestMock(routes) {
                ...
                queueMicrotask(() => {
                    try {
                        ...
                        if (result.body !== undefined) {
                            response.write(typeof result.body === 'string'
                                ? result.body
...
describe('isReplyListenerProcess', () => {
    ...
    it('returns false on native Windows instead of shelling out to ps', (_, done) => {
        ...
        const child = spawn(process.execPath, ['-e', 'const pollLoop = () => {}; setInterval(pollLoop, 60000);'], { stdio: 'ignore' });
        child.once('spawn', () => {
            const pid = child.pid;
...
describe('formatReplyAcknowledgement', () => {
    it('includes recent output when a summary is available', () => {
        ...
        assert.equal(message, 'Injected into Codex CLI session.\n\nRecent output:\nLine 1\nLine 2');
    });
...
describe('redactSensitiveTokens', () => {
    it('redacts OpenAI-style API keys', () => {
        assert.equal(redactSensitiveTokens('export OPENAI_API_KEY=sk-proj-abc123def456'), 'export OPENAI_API_KEY=[REDACTED]');
    });
    it('redacts GitHub PAT tokens', () => {
        assert.equal(redactSensitiveTokens('token: ghp_1234567890abcdefABCDEF'), 'token: [REDACTED]');
    });
    ...
    it('redacts multi-part authorization header values', () => {
        assert.equal(redactSensitiveTokens('authorization: Bearer mysecrettoken'), 'authorization: [REDACTED]');
    });
    it('redacts quoted JSON secret fields', () => {
        assert.equal(redactSensitiveTokens('{"api_key":"mysecret","safe":true}'), '{"api_key":"[REDACTED]","safe":true}');
    });
...
describe('pollDiscordOnce', () => {
    it('treats exact-match status replies as read-only Discord session lookups', async () => {
        ...
        await pollDiscordOnce(config, state, new RateLimiter(10), {
            fetchImpl,
    ...
    it('injects authorized replies and posts a threaded acknowledgement with recent output', async () => {
        ...
        assert.equal(acknowledgementBody.content, 'Injected into Codex CLI session.\n\nRecent output:\nRecent pane output');
    ...
    it('ignores unauthorized Discord replies while still advancing the last message id', async () => {
        resetReplyListenerTransientState();
        const state = createBaseState();
        await pollDiscordOnce(createBaseConfig(), state, new RateLimiter(10), {
    ...
    it('drops mapped Discord replies when the rate limiter rejects them', async () => {
        ...
        const state = createBaseState();
        let injectCalled = false;
        await pollDiscordOnce(createBaseConfig(), state, { canProceed: () => false, reset: () => { } }, {
            fetchImpl: async () => jsonResponse([
                {
                    id: 'discord-reply-3',
                    author: { id: 'discord-user-1' },
...
describe('pollTelegramOnce', () => {
    it('injects Telegram replies and sends a reply acknowledgement', async () => {
        ...
        const writes = [];
        let sendMessageBody = '';
        await pollTelegramOnce(config, state, new RateLimiter(10), {
        ...
        assert.ok(writes.length >= 1);
        const parsedBody = JSON.parse(sendMessageBody);
    ...
    it('ignores Telegram replies from the wrong chat', async () => {
        ...
        await pollTelegramOnce(config, createBaseState(), new RateLimiter(10), {
            httpsRequestImpl: createHttpsRequestMock({
                [`GET /bot${config.telegramBotToken}/getUpdates?offset=0&timeout=0`]: () => ({
                    statusCode: 200,
                    body: {
