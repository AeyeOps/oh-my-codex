...
import { STATUS_DATA_UNAVAILABLE_MESSAGE, buildDiscordSessionStatusReply, isDiscordStatusCommand, } from '../session-status.js';
function createMapping(projectPath, sessionId = 'sess-1') {
    return {
...
describe('session-status helper', () => {
    ...
    it('renders stale partial summaries with explicit unknown fields', async () => {
        ...
        try {
            ...
            await writeFile(join(wd, '.omx', 'logs', 'session-history.jsonl'), `${JSON.stringify({
                session_id: 'sess-old',
