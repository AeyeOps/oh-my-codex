...
describe('_sendJsonHttpsRequest', () => {
    async function withMockedHttpsRequest(mockRequest, run) {
        const httpsModule = await import('https');
        const originalRequest = httpsModule.default.request;
        httpsModule.default.request = mockRequest;
        try {
            await run();
        }
        finally {
            httpsModule.default.request = originalRequest;
        }
    }
    it('rejects non-2xx HTTP responses', async () => {
        await withMockedHttpsRequest((_options, callback) => {
            ...
            await assert.rejects(_sendJsonHttpsRequest({
