...
import { getSelectedOpenClawGatewayNames, isOpenClawSelectedInTempContract, readNotifyTempContractFromEnv, } from "./temp-contract.js";
import { formatNotification } from "./formatter.js";
import { dispatchNotifications } from "./dispatcher.js";
import { getCurrentTmuxSession, sanitizeTmuxAlertText } from "./tmux.js";
import { basename } from "path";
import { omxStateDir } from "../utils/paths.js";
import { shouldSendLifecycleNotification, recordLifecycleNotificationSent, } from "./lifecycle-dedupe.js";
import { parseTmuxTail } from "./formatter.js";
import { shouldIncludeSessionIdleTmuxTail, recordSessionIdleTmuxTailSent, } from "./idle-cooldown.js";
 ...
 */
function toOpenClawEvent(event) {
    switch (event) {
...
export async function shouldDispatchOpenClaw(event, tempContract, env = process.env) {
    if (env.OMX_OPENCLAW !== "1")
        return false;
    if (!tempContract?.active)
        return true;
    if (!isOpenClawSelectedInTempContract(tempContract))
        return false;
    const selectedGatewayNames = getSelectedOpenClawGatewayNames(tempContract);
    if (selectedGatewayNames.size === 0)
        return false;
    try {
...
export async function notifyLifecycle(event, data, profileName) {
    try {
        ...
        let dispatchOpenClawLater = null;
