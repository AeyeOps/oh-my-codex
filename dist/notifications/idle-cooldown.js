 ...
 */
function getScopedStatePath(stateDir, fileName, sessionId) {
...
function getCooldownStatePath(stateDir, sessionId) {
    return getScopedStatePath(stateDir, IDLE_NOTIFICATION_STATE_FILE, sessionId);
}
function getSessionIdleHookStatePath(stateDir, sessionId) {
    return getScopedStatePath(stateDir, SESSION_IDLE_HOOK_STATE_FILE, sessionId);
}
function normalizeIdleFingerprint(fingerprint) {
...
function readIdleNotificationState(cooldownPath) {
    try {
...
function writeIdleNotificationState(cooldownPath, patch) {
    try {
 ...
 */
export function shouldSendIdleNotification(stateDir, sessionId, fingerprint) {
 ...
 */
export function recordIdleNotificationSent(stateDir, sessionId, fingerprint) {
    const cooldownPath = getCooldownStatePath(stateDir, sessionId);
    const normalizedFingerprint = normalizeIdleFingerprint(fingerprint);
 ...
 */
export function shouldSendSessionIdleHookEvent(stateDir, sessionId, fingerprint) {
    const normalizedFingerprint = normalizeIdleFingerprint(fingerprint);
    if (!normalizedFingerprint)
        return true;
 ...
 */
export function recordSessionIdleHookEventSent(stateDir, sessionId, fingerprint) {
    const normalizedFingerprint = normalizeIdleFingerprint(fingerprint);
 ...
 */
export function shouldIncludeSessionIdleTmuxTail(stateDir, sessionId, tmuxTailFingerprint) {
 ...
 */
export function recordSessionIdleTmuxTailSent(stateDir, sessionId, tmuxTailFingerprint) {
    const normalizedFingerprint = normalizeIdleFingerprint(tmuxTailFingerprint);
    const cooldownPath = getCooldownStatePath(stateDir, sessionId);
    writeIdleNotificationState(cooldownPath, {
...
//# sourceMappingURL=idle-cooldown.js.map