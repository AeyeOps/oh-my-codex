...
`), /keep_policy must be one of/i);
    ...
    it('rejects mission directories outside a git repository', async () => {
        ...
        try {
            ...
            await assert.rejects(() => loadAutoresearchMissionContract(missionDir), /not a git repository|mission-dir must be inside a git repository/i);
        }
        finally {
    ...
    it('rejects mission directories missing sandbox.md', async () => {
        ...
        try {
            ...
            await writeFile(join(missionDir, 'mission.md'), '# Mission\nShip it\n', 'utf-8');
            await assert.rejects(() => loadAutoresearchMissionContract(missionDir), /sandbox\.md is required inside mission-dir/i);
        }
        finally {
