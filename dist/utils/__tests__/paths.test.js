...
import { codexHome, codexConfigPath, codexPromptsDir, userSkillsDir, projectSkillsDir, legacyUserSkillsDir, listInstalledSkillDirectories, detectLegacySkillRootOverlap, omxStateDir, omxProjectMemoryPath, omxNotepadPath, omxPlansDir, omxAdaptersDir, omxLogsDir, packageRoot, canonicalizeComparablePath, OMX_ENTRY_PATH_ENV, OMX_STARTUP_CWD_ENV, rememberOmxLaunchContext, resolveOmxCliEntryPath, resolveOmxEntryPath, } from "../paths.js";
describe("codexHome", () => {
...
describe("listInstalledSkillDirectories", () => {
    ...
    it("deduplicates by skill name and prefers project skills over user skills", async () => {
        ...
        try {
            ...
            const projectOnlyDir = join(projectRoot, ".codex", "skills", "project-only");
...
describe("omxProjectMemoryPath", () => {
    it("uses provided projectRoot", () => {
        assert.equal(omxProjectMemoryPath("/my/project"), join("/my/project", ".omx", "project-memory.json"));
    });
    it("defaults to cwd when no projectRoot given", () => {
        assert.equal(omxProjectMemoryPath(), join(process.cwd(), ".omx", "project-memory.json"));
    });
});
...
describe("OMX launcher path resolution", () => {
    ...
    it("canonicalizes symlinked startup cwd launcher paths to their real path", async () => {
        ...
        try {
            ...
            await mkdir(launcherDir, { recursive: true });
            await writeFile(launcherPath, "#!/usr/bin/env node\n", "utf-8");
            await symlink(realStartupCwd, linkedStartupCwd, process.platform === "win32" ? "junction" : "dir");
            const resolved = resolveOmxEntryPath({
                argv1: "dist/cli/omx.js",
                cwd: laterCwd,
                env: {
