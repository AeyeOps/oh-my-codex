...
import { buildPlatformCommandSpec, classifySpawnError, resolveCommandPathForPlatform, resolveTmuxBinaryForPlatform, spawnPlatformCommandSync, } from '../platform-command.js';
describe('buildPlatformCommandSpec', () => {
    it('wraps .cmd shims through ComSpec on Windows', async () => {
        ...
        try {
            ...
            const spec = buildPlatformCommandSpec('codex', ['--version'], 'win32', {
                PATH: fakeBin,
                PATHEXT: '.EXE;.CMD;.PS1',
    ...
    it('launches .exe binaries directly on Windows', async () => {
        ...
        try {
            ...
            const spec = buildPlatformCommandSpec('tmux', ['-V'], 'win32', {
                PATH: fakeBin,
                PATHEXT: '.EXE;.CMD;.PS1',
            });
    ...
    it('skips directory PATH candidates when resolving the PowerShell executable for .ps1 shims', async () => {
        ...
        try {
            ...
            const spec = buildPlatformCommandSpec(ps1Path, ['--version'], 'win32', {
...
describe('resolveCommandPathForPlatform', () => {
    it('prefers PATHEXT candidates on Windows', async () => {
        ...
        try {
            const exePath = join(fakeBin, 'tmux.exe');
            await writeFile(exePath, '');
            assert.equal(resolveCommandPathForPlatform('tmux', 'win32', {
                PATH: fakeBin,
                PATHEXT: '.EXE;.CMD',
            }), exePath);
        }
    ...
    it('falls back to psmux on Windows when tmux is absent', async () => {
        ...
        try {
            ...
            assert.equal(resolveTmuxBinaryForPlatform('win32', {
                PATH: fakeBin,
                PATHEXT: '.EXE;.CMD',
            }), psmuxPath);
        }
    ...
    it('ignores directory candidates on Windows even when they appear before a real executable', async () => {
        ...
        try {
            ...
            await mkdir(powershellDirectory, { recursive: true });
            await writeFile(powershellExePath, '');
            assert.equal(resolveCommandPathForPlatform('powershell', 'win32', {
                PATH: [badPowerShellBin, goodPowerShellBin].join(delimiter),
                PATHEXT: '.EXE;.CMD;.PS1',
            }), powershellExePath);
        }
    ...
    it('resolves PATH entries to absolute paths on POSIX', async () => {
        ...
        try {
            ...
            assert.equal(resolveCommandPathForPlatform('node', 'linux', { PATH: fakeBin }), nodePath);
        }
        finally {
            await rm(fakeBin, { recursive: true, force: true });
        }
    ...
    it('returns null on POSIX when the command is not present on PATH', () => {
        assert.equal(resolveCommandPathForPlatform('missing-binary', 'linux', { PATH: '/tmp/does-not-exist' }), null);
    });
});
describe('classifySpawnError', () => {
    it('classifies ENOENT as missing', () => {
        assert.equal(classifySpawnError({ code: 'ENOENT' }), 'missing');
    });
    it('classifies EPERM as blocked', () => {
        assert.equal(classifySpawnError({ code: 'EPERM' }), 'blocked');
    });
    it('classifies other errors as generic error', () => {
        assert.equal(classifySpawnError({ code: 'EIO' }), 'error');
    });
});
describe('spawnPlatformCommandSync', () => {
    it('passes the Windows-resolved spec into the spawn implementation', async () => {
        ...
        try {
            ...
            await writeFile(cmdPath, '@echo off\r\n');
            const calls = [];
            const probed = spawnPlatformCommandSync('codex', ['--version'], { encoding: 'utf-8' }, 'win32', {
                PATH: fakeBin,
                PATHEXT: '.EXE;.CMD',
                ComSpec: 'C:\\Windows\\System32\\cmd.exe',
    ...
    it('does not force verbatim arguments for direct Windows executables', async () => {
        ...
        try {
            ...
            await writeFile(exePath, '');
            const calls = [];
            spawnPlatformCommandSync('tmux', ['-V'], { encoding: 'utf-8' }, 'win32', {
                PATH: fakeBin,
                PATHEXT: '.EXE;.CMD',
            }, undefined, ((command, args, options) => {
                calls.push({ command, args, options });
    ...
    it('retries blocked node-hosted scripts through process.execPath on non-Windows', () => {
        ...
        const calls = [];
        const probed = spawnPlatformCommandSync(scriptPath, ['--prompt', 'find auth'], { encoding: 'utf-8' }, 'linux', process.env, undefined, ((command, args) => {
...
//# sourceMappingURL=platform-command.test.js.map