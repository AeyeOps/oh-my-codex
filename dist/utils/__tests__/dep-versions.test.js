...
function readInstalledVersions(pkg) {
...
describe('transitive dependency minimum safe versions (issue #170)', () => {
    it('ajv is at least 8.18.0 (fixes GHSA-2g4f-4pwh-qvx6 ReDoS)', () => {
        ...
        assert.ok(versions.every((version) => semverGte(version, '8.18.0')), `ajv versions [${versions.join(', ')}] include a version below the minimum safe version 8.18.0`);
    });
    it('hono is at least 4.11.10 (fixes GHSA-gq3j-xvxp-8hrf timing attack)', () => {
        ...
        assert.ok(versions.every((version) => semverGte(version, '4.11.10')), `hono versions [${versions.join(', ')}] include a version below the minimum safe version 4.11.10`);
    });
});
//# sourceMappingURL=dep-versions.test.js.map