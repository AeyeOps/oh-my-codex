import { OMX_MODELS_END_MARKER, OMX_MODELS_START_MARKER, } from './agents-model-table.js';
...
export function hasOmxManagedAgentsSections(content) {
    return (isOmxGeneratedAgentsMd(content) ||
        (content.includes(OMX_MODELS_START_MARKER) &&
            content.includes(OMX_MODELS_END_MARKER)));
}
export function addGeneratedAgentsMarker(content) {
    if (content.includes(OMX_GENERATED_AGENTS_MARKER))
        return content;
