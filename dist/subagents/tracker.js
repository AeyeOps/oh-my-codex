...
export function normalizeSubagentTrackingState(input) {
    ...
    for (const [sessionId, rawSession] of Object.entries(parsed.sessions ?? {})) {
        if (!rawSession || typeof rawSession !== 'object')
            continue;
        const threads = {};
        for (const [threadId, rawThread] of Object.entries(rawSession.threads ?? {})) {
            if (!rawThread || typeof rawThread !== 'object')
                continue;
            const candidate = rawThread;
        ...
        const sessionCandidate = rawSession;
...
export async function readSubagentTrackingState(cwd) {
...
export function recordSubagentTurn(state, input) {
    const sessionId = input.sessionId.trim();
    const threadId = input.threadId.trim();
    if (!sessionId || !threadId)
        return normalizeSubagentTrackingState(state);
...
export function summarizeSubagentSession(state, sessionId, options = {}) {
    const normalized = normalizeSubagentTrackingState(state);
...
export async function readSubagentSessionSummary(cwd, sessionId, options = {}) {
