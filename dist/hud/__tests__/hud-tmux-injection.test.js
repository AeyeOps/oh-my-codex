...
describe('VULNERABILITY – old string-interpolation approach', () => {
     ...
     */
    function buildOldCommand(cwd, omxBin, preset) {
    ...
    it('cwd containing $() is injectable via double-quote context', () => {
        ...
        assert.ok(shellCmd.includes('-c "/tmp/foo"$(touch /tmp/pwned)"bar"'), `Old code passes command substitution unescaped: ${shellCmd}`);
    });
    it('cwd containing backticks is injectable via double-quote context', () => {
        const maliciousCwd = '/tmp/`id>/tmp/leak`';
        const shellCmd = buildOldCommand(maliciousCwd, '/usr/bin/omx.js');
        assert.ok(shellCmd.includes('`id>/tmp/leak`'), `Old code passes backtick command unescaped: ${shellCmd}`);
    });
    it("omxBin containing single quote breaks out of the '-quoted command", () => {
        ...
        assert.ok(shellCmd.includes("'node /tmp/it';touch /tmp/pwned;echo '/omx.js"), `Old code allows single-quote breakout: ${shellCmd}`);
    });
});
...
describe('buildTmuxSplitArgs – shell injection hardening', () => {
    ...
    it("omxBin with single quote is properly escaped in command string", () => {
        ...
        assert.ok(cmd.includes("'\\''"), `Expected escaped single quote in: ${cmd}`);
    ...
    it("omxBin with ';command' breakout attempt is neutralised", () => {
        ...
        assert.equal(cmd, "node '/tmp/x'\\'';touch /tmp/pwned;echo '\\''/omx.js' hud --watch");
    ...
    it('invalid preset is dropped (defense in depth)', () => {
        const args = buildTmuxSplitArgs('/home/user', '/usr/bin/omx.js', 'minimal;touch /tmp/pwned');
        const cmd = args[6];
...
//# sourceMappingURL=hud-tmux-injection.test.js.map