...
function runNotifyHook(payload) {
...
describe('notify-hook session-scoped iteration updates', () => {
    ...
    it('persists visual-verdict feedback from runtime assistant output', async () => {
        ...
        try {
            ...
            assert.equal((progress.visual_feedback?.[0]?.qualitative_feedback?.next_actions?.length || 0) <= VISUAL_NEXT_ACTIONS_LIMIT, true);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
...
//# sourceMappingURL=notify-hook-session-scope.test.js.map