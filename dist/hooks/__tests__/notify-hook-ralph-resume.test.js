...
function notifyHookRepoRoot() {
    const testDir = dirname(fileURLToPath(import.meta.url));
    return join(testDir, '..', '..', '..');
}
function runNotifyHook(payload, envOverrides = {}) {
...
function buildPayload(cwd, overrides = {}) {
    return {
        cwd,
...
function buildResumeFakeTmux(currentPaneId, cwd, sessionName = 'devsess') {
...
async function setupTmuxFixture(wd, currentPaneId) {
...
async function withPatchedEnv(overrides, run) {
...
describe('notify-hook Ralph session resume', () => {
    it('resumes a matching prior Ralph into the current OMX session and rebinds the pane', async () => {
        ...
        try {
            ...
            const currentState = JSON.parse(await readFile(join(currentSessionDir, 'ralph-state.json'), 'utf-8'));
            ...
            const priorState = JSON.parse(await readFile(join(priorSessionDir, 'ralph-state.json'), 'utf-8'));
    ...
    it('adopts a same-thread Ralph across OMX session turnover even when Codex session id is absent', async () => {
        ...
        try {
            ...
            const firstResult = runNotifyHook(buildPayload(wd, {
            ...
            const updatedPriorState = JSON.parse(await readFile(join(priorSessionDir, 'ralph-state.json'), 'utf-8'));
            ...
            await writeJson(join(stateDir, 'session.json'), { session_id: currentOmxSessionId });
            const secondResult = runNotifyHook(buildPayload(wd, {
                thread_id: 'thread-turnover-1',
        ...
        for (const scenario of scenarios) {
            await t.test(scenario.name, async () => {
                ...
                try {
                    ...
                    for (const priorSessionId of scenario.priorSessions) {
                        const priorState = JSON.parse(await readFile(join(stateDir, 'sessions', priorSessionId, 'ralph-state.json'), 'utf-8'));
                        assert.equal(priorState.active, true);
    ...
    it('rolls back the target state when transfer fails after writing the current session', async () => {
        ...
        try {
            ...
            await assert.rejects(() => reconcileRalphSessionResume({
                stateDir,
                payloadSessionId: 'codex-session-1',
...
//# sourceMappingURL=notify-hook-ralph-resume.test.js.map