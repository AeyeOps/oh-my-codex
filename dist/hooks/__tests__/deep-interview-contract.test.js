...
function readProjectAgents(startDir) {
...
describe("deep-interview Ouroboros contract", () => {
    ...
    it("strengthens questioning pressure on all four analysis axes", () => {
        assert.match(deepInterviewSkill, /Treat every answer as a claim to pressure-test before moving on/i);
        assert.match(deepInterviewSkill, /demand evidence or examples, expose a hidden assumption, force a tradeoff or boundary, or reframe root cause vs symptom/i);
        assert.match(deepInterviewSkill, /Do not rotate to a new clarity dimension just for coverage/i);
        assert.match(deepInterviewSkill, /Prefer staying on the same thread for multiple rounds when it has the highest leverage/i);
        assert.match(deepInterviewSkill, /Do not offer early exit before the first explicit assumption probe and one persistent follow-up have happened/i);
        assert.match(deepInterviewSkill, /Round 4\+: allow explicit early exit with risk warning/i);
    });
    it("moves challenge modes and preserved evidence discipline earlier", () => {
        assert.match(deepInterviewSkill, /Contrarian.*round 2\+.*untested assumption/i);
        assert.match(deepInterviewSkill, /Simplifier.*round 4\+.*scope expands faster than outcome clarity/i);
        assert.match(deepInterviewSkill, /Ontologist.*round 5\+.*ambiguity > 0\.25.*describing symptoms/i);
        assert.match(deepInterviewSkill, /Brownfield evidence vs inference notes/i);
    });
    ...
    it("documents surface-aware omx question handling and fallback boundaries", () => {
        ...
        assert.match(deepInterviewSkill, /required `AskUserQuestion` equivalent/i);
        assert.match(deepInterviewSkill, /attached-tmux Codex CLI, deep-interview uses `omx question`/i);
        assert.match(deepInterviewSkill, /OMX_QUESTION_RETURN_PANE=\$TMUX_PANE/i);
        assert.match(deepInterviewSkill, /outside tmux and cannot render `omx question`, use (the )?native structured (question tool|input) when available/i);
        assert.match(deepInterviewSkill, /ask exactly one concise plain-text question and wait for the answer/i);
        assert.doesNotMatch(deepInterviewSkill, /else, use `request_user_input` to present concise multiple-choice options/i);
        assert.match(deepInterviewSkill, /wait for that background terminal to finish and read its JSON answer before scoring ambiguity, asking another round, or handing off/i);
    });
    it("teaches canonical single-choice vs multi-answerable omx question payloads", () => {
        assert.match(deepInterviewSkill, /Use canonical `type` values instead of authoring raw `multi_select` flags by hand/i);
        assert.match(deepInterviewSkill, /type: "single-answerable"/i);
        assert.match(deepInterviewSkill, /type: "multi-answerable"/i);
        assert.match(deepInterviewSkill, /Use `single-answerable` when exactly one answer should drive the next branch/i);
        assert.match(deepInterviewSkill, /Use `multi-answerable` when multiple options may all be true at once/i);
        assert.match(deepInterviewSkill, /If one selected option would immediately require a follow-up question to disambiguate the others, prefer a `single-answerable` round now/i);
        assert.match(deepInterviewSkill, /Keep interview options bounded and concrete\./i);
        assert.match(deepInterviewSkill, /Canonical bounded single-choice payload:/i);
        assert.match(deepInterviewSkill, /Which execution lane should own this once the interview is complete\?/i);
        ...
        assert.match(deepInterviewSkill, /Canonical bounded multi-select payload:/i);
        assert.match(deepInterviewSkill, /Which non-goals must stay out of scope for the first pass\?/i);
    ...
    it("locks canonical omx question answer shapes for single and multi rounds", () => {
        ...
        assert.match(deepInterviewSkill, /"value": \["no-new-dependencies", "no-api-contract-changes"\]/i);
        assert.match(deepInterviewSkill, /"selected_values": \["no-new-dependencies", "no-api-contract-changes"\]/i);
        assert.match(deepInterviewSkill, /For `multi-answerable`, treat `answer\.selected_values` as the source of truth/i);
    });
    ...
    it("documents the autoresearch specialization contract", () => {
        ...
        assert.match(deepInterviewSkill, /\.omx\/specs\/deep-interview-autoresearch-\{slug\}\.md/i);
        ...
        assert.match(deepInterviewSkill, /do not run direct CLI launch or detached\/split tmux launch, and only hand off to `\$autoresearch` after explicit confirmation/i);
...
//# sourceMappingURL=deep-interview-contract.test.js.map