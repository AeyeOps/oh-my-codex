...
import { readTriageState, writeTriageState, shouldSuppressFollowup, promptSignature, } from '../triage-state.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeTmp() {
...
function expectedPath(cwd, sessionId) {
    if (sessionId) {
...
describe('readTriageState — valid file returns parsed object', () => {
    ...
    it('valid state with last_triage=null parses correctly', () => {
        const nullState = { version: 1, last_triage: null, suppress_followup: false };
...
describe('writeTriageState — round-trip', () => {
    ...
    it('round-trip works with last_triage=null', () => {
        ...
        const state = { version: 1, last_triage: null, suppress_followup: false };
        writeTriageState({ cwd: tmp, sessionId, state });
        const result = readTriageState({ cwd: tmp, sessionId });
        assert.ok(result !== null);
        assert.deepEqual(result, state);
...
describe('writeTriageState — overwrite behaviour', () => {
    ...
    it('second write replaces first write cleanly', () => {
        ...
        const second = { version: 1, last_triage: null, suppress_followup: false };
...
describe('shouldSuppressFollowup — clarifying short prompts only', () => {
    const previous = validState({ suppress_followup: true });
    it('"yes" (1 word) → true', () => {
        assert.equal(shouldSuppressFollowup({ previous, currentPrompt: 'yes', currentHasKeyword: false }), true);
    });
    it('"the auth helper" (3 words) → true', () => {
        assert.equal(shouldSuppressFollowup({ previous, currentPrompt: 'the auth helper', currentHasKeyword: false }), true);
    });
    it('"settings page" (2 words) → false', () => {
        assert.equal(shouldSuppressFollowup({ previous, currentPrompt: 'settings page', currentHasKeyword: false }), false);
    });
    it('"yeah do that" (3 words) → true', () => {
        assert.equal(shouldSuppressFollowup({ previous, currentPrompt: 'yeah do that', currentHasKeyword: false }), true);
    });
    it('"one two three four five six" (6 words, boundary) → false', () => {
        assert.equal(shouldSuppressFollowup({
            previous,
...
//# sourceMappingURL=triage-state.test.js.map