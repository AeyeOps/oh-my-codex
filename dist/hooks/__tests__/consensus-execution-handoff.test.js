...
const architectPrompt = readFileSync(join(__dirname, '../../../prompts/architect.md'), 'utf-8');
const criticPrompt = readFileSync(join(__dirname, '../../../prompts/critic.md'), 'utf-8');
/**
...
describe('Consensus mode execution handoff (plan/SKILL.md)', () => {
    it('should mandate AskUserQuestion for the approval step', () => {
        ...
        assert.ok(consensusSection.includes('AskUserQuestion'), 'Consensus mode should mandate AskUserQuestion');
    });
    it('should mandate $ralph invocation for execution on user approval', () => {
        const consensusSection = extractSection(planSkill, 'Consensus Mode');
        assert.ok(consensusSection, 'Consensus Mode section should exist');
        assert.ok(consensusSection.includes('$ralph'), 'Consensus mode should reference $ralph invocation');
    });
    it('should use MUST language for execution handoff', () => {
        const consensusSection = extractSection(planSkill, 'Consensus Mode');
        assert.ok(consensusSection, 'Consensus Mode section should exist');
        assert.ok(/MUST.*\$ralph/s.test(consensusSection) || /\$ralph.*MUST/s.test(consensusSection), 'Consensus mode should use MUST language around $ralph invocation');
    });
    it('should prohibit direct implementation from the planning agent', () => {
        const consensusSection = extractSection(planSkill, 'Consensus Mode');
        assert.ok(consensusSection, 'Consensus Mode section should exist');
        assert.ok(/Do NOT implement directly/i.test(consensusSection), 'Consensus mode should prohibit direct implementation');
    });
    ...
    it('should reference $ralph in Escalation section', () => {
        assert.ok(planSkill.includes('$ralph'), 'plan/SKILL.md should reference $ralph for execution handoff');
    });
...
describe('User feedback step between Planner and Architect/Critic (plan/SKILL.md)', () => {
    ...
    it('should mandate AskUserQuestion for the user feedback step', () => {
        const consensusSection = extractSection(planSkill, 'Consensus Mode');
        assert.ok(consensusSection, 'Consensus Mode section should exist');
        assert.ok(/User feedback.*MUST.*AskUserQuestion/s.test(consensusSection), 'User feedback step should mandate AskUserQuestion');
    });
    ...
    it('should require architect antithesis and critic rejection gates in consensus flow', () => {
        const consensusSection = extractSection(planSkill, 'Consensus Mode');
        assert.ok(consensusSection, 'Consensus Mode section should exist');
        assert.ok(consensusSection.includes('steelman counterargument (antithesis)') ||
            consensusSection.includes('steelman counterargument'), 'Architect step should require steelman counterargument');
        assert.ok(consensusSection.includes('tradeoff tension'), 'Should require tradeoff tension');
        assert.ok(/Critic.*MUST.*explicitly reject shallow alternatives/s.test(consensusSection) ||
            consensusSection.includes('Critic **MUST** explicitly reject shallow alternatives'), 'Critic should explicitly reject shallow alternatives');
        assert.ok(consensusSection.includes('driver contradictions') ||
            consensusSection.includes('driver contradiction'), 'Should mention driver contradictions');
        assert.ok(consensusSection.includes('weak verification'), 'Should mention weak verification');
    });
});
...
describe('RALPLAN-DR in ralplan/SKILL.md', () => {
    it('should contain RALPLAN-DR structured deliberation description', () => {
        assert.ok(ralplanSkill.includes('RALPLAN-DR'), 'ralplan/SKILL.md should mention RALPLAN-DR');
    });
    it('should document the --deliberate flag', () => {
        assert.ok(ralplanSkill.includes('--deliberate'), 'ralplan/SKILL.md should document --deliberate flag');
    });
    it('should contain Pre-Execution Gate section', () => {
        assert.ok(ralplanSkill.includes('Pre-Execution Gate'), 'ralplan/SKILL.md should contain Pre-Execution Gate section');
    });
    it('should document gate bypass prefixes force: and !', () => {
        assert.ok(ralplanSkill.includes('force:') && ralplanSkill.includes('! '), 'ralplan/SKILL.md should document force: and ! bypass prefixes');
    });
    it('should document sequential Architect then Critic execution', () => {
        assert.ok(/step[s]? 3 and 4 MUST run sequentially|Do NOT.*parallel/i.test(ralplanSkill) ||
            ralplanSkill.includes('await completion before step 4'), 'ralplan/SKILL.md should require sequential Architect/Critic execution');
    });
    it('should document ADR requirement', () => {
        assert.ok(ralplanSkill.includes('ADR'), 'ralplan/SKILL.md should reference ADR requirement');
    });
...
describe('Architect prompt RALPLAN-DR sections', () => {
    it('should have steelman antithesis requirement', () => {
        assert.ok(architectPrompt.includes('antithesis') || architectPrompt.includes('steelman'), 'architect.md should require steelman antithesis');
    });
    it('should have tradeoff tension requirement', () => {
        assert.ok(architectPrompt.includes('tradeoff tension'), 'architect.md should require tradeoff tension');
    });
    it('should have synthesis requirement', () => {
        assert.ok(architectPrompt.includes('synthesis'), 'architect.md should mention synthesis');
    });
    it('should reference ralplan consensus reviews', () => {
        assert.ok(architectPrompt.includes('ralplan'), 'architect.md should reference ralplan consensus reviews');
    });
});
...
describe('Critic prompt RALPLAN-DR sections', () => {
    it('should have gate checks for ralplan reviews', () => {
        assert.ok(criticPrompt.includes('ralplan') && criticPrompt.includes('gate'), 'critic.md should mention ralplan gate checks');
    });
    it('should explicitly REJECT shallow alternatives', () => {
        assert.ok(/REJECT.*shallow alternatives/i.test(criticPrompt) ||
            criticPrompt.includes('REJECT shallow alternatives'), 'critic.md should explicitly REJECT shallow alternatives');
    });
    it('should enforce deliberate mode requirements', () => {
        assert.ok(criticPrompt.includes('deliberate') &&
            (criticPrompt.includes('pre-mortem') || criticPrompt.includes('expanded test plan')), 'critic.md should enforce deliberate mode requirements');
    });
});
