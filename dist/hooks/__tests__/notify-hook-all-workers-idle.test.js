...
function writeWorkerIdentityFixture(cwd, workerEnv) {
...
function runNotifyHookAsWorker(cwd, fakeBinDir, workerEnv, extraEnv = {}) {
...
describe('notify-hook all-workers-idle notification', () => {
    it('sends notification to leader when all workers are idle', async () => {
        await withTempWorkingDir(async (cwd) => {
            ...
            const deferredEvent = events.find((e) => e.type === 'leader_notification_deferred' && e.reason === 'leader_pane_missing_no_injection');
            ...
            const warn = logLines
                .map((line) => JSON.parse(line))
                .find((entry) => entry.type === 'leader_notification_deferred' && entry.reason === 'leader_pane_missing_no_injection');
    ...
    it('writes deferred visibility once per cooldown window when leader pane is missing', async () => {
        await withTempWorkingDir(async (cwd) => {
            ...
            const deferredEvents = events.filter((event) => event.type === 'leader_notification_deferred' && event.reason === 'leader_pane_missing_no_injection');
...
`;
            ...
            const events = (await readFile(eventsPath, 'utf-8')).trim().split('\n').filter(Boolean).map((line) => JSON.parse(line));
            const deferred = events.find((event) => event.type === 'leader_notification_deferred' && event.reason === 'leader_pane_shell_no_injection');
...
`;
    ...
    it('writes all_workers_idle event to events.ndjson', async () => {
        await withTempWorkingDir(async (cwd) => {
            ...
            const idleEvent = events.find((e) => e.type === 'all_workers_idle');
...
//# sourceMappingURL=notify-hook-all-workers-idle.test.js.map