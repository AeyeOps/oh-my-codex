...
describe("generateOverlay", () => {
    ...
    it("adds advisory explore routing guidance by default and hides it only on explicit opt-out", async () => {
        ...
        try {
            ...
            const defaultOverlay = await generateOverlay(tempDir, "explore-routing-default");
            assert.match(defaultOverlay, /\*\*Explore Command Preference:\*\*/);
    ...
    it("does not inherit stale root active modes into a fresh session overlay", async () => {
        await writeFile(join(tempDir, ".omx", "state", "ralph-state.json"), JSON.stringify({
            active: true,
            iteration: 9,
            max_iterations: 10,
            current_phase: "executing",
    ...
    it("lists both approved combined workflow members from canonical skill state", async () => {
        ...
        const sessionDir = join(tempDir, ".omx", "state", "sessions", sessionId);
        await mkdir(sessionDir, { recursive: true });
        await writeFile(join(tempDir, ".omx", "state", "session.json"), JSON.stringify({ session_id: sessionId }));
        ...
        await writeFile(join(sessionDir, "team-state.json"), JSON.stringify({ active: true, current_phase: "running" }));
        await writeFile(join(sessionDir, "ralph-state.json"), JSON.stringify({ active: true, iteration: 2, max_iterations: 5, current_phase: "executing" }));
        const overlay = await generateOverlay(tempDir, sessionId);
...
describe("resolveSessionOrchestrationMode", () => {
    ...
    it("does not inherit root skill-active orchestration mode into a fresh session", async () => {
        await writeFile(join(tempDir, ".omx", "state", "skill-active-state.json"), JSON.stringify({
            active: true,
            skill: "team",
        }));
        const mode = await resolveSessionOrchestrationMode(tempDir, "fresh-session-isolation");
    ...
    it("active mode summary follows canonical session skill state instead of stale root mode files", async () => {
        ...
        await mkdir(sessionDir, { recursive: true });
        await writeFile(join(rootStateDir, "ralph-state.json"), JSON.stringify({ active: true, iteration: 9, max_iterations: 10, current_phase: "stale-root" }));
        await writeFile(join(sessionDir, "skill-active-state.json"), JSON.stringify({
            active: true,
            skill: "team",
            phase: "running",
        ...
        await writeFile(join(sessionDir, "team-state.json"), JSON.stringify({ active: true, team_name: "delta" }));
        const overlay = await generateOverlay(tempDir, sessionId);
    ...
    it("active mode summary suppresses stale autoresearch mode files when canonical session skill state excludes it", async () => {
        ...
        const sessionDir = join(rootStateDir, "sessions", sessionId);
        await mkdir(sessionDir, { recursive: true });
        await writeFile(join(rootStateDir, "autoresearch-state.json"), JSON.stringify({ active: true, current_phase: "running" }));
        await writeFile(join(sessionDir, "skill-active-state.json"), JSON.stringify({
            active: true,
            skill: "team",
            phase: "running",
