...
describe('notify-hook/operational-events – buildOperationalContext', () => {
    ...
    it('writes PR lifecycle details into the repo baseline registry as best effort state', async () => {
        ...
        try {
            ...
            const entry = baseline.tasks.find((item) => item.branch_name === 'feature/opctx-pr');
...
describe('notify-hook/operational-events – deriveAssistantSignalEvents', () => {
    it('detects handoff-needed and retry-needed from assistant text', async () => {
        ...
        assert.equal(signals.some((signal) => signal.event === 'handoff-needed'), true);
        assert.equal(signals.some((signal) => signal.event === 'retry-needed'), true);
    });
    it('avoids duplicate finished/failed assistant lifecycle signals', async () => {
        const { deriveAssistantSignalEvents } = await loadModule('notify-hook/operational-events.js');
        assert.equal(deriveAssistantSignalEvents('Implementation completed. Final summary ready.').some((signal) => signal.event === 'finished'), false);
        assert.equal(deriveAssistantSignalEvents('The operation failed with error: unable to continue.').some((signal) => signal.event === 'failed'), false);
    });
});
// ---------------------------------------------------------------------------
...
describe('notify-hook/auto-nudge – detectStallPattern', () => {
    ...
    it('ignores orchestration intent tags when normalizing stall text', async () => {
        ...
        assert.equal(normalizeAutoNudgeSignatureText('Team alpha: 1 msg(s) for leader. [OMX_INTENT:pending-mailbox-review]'), 'team alpha 1 msg s for leader');
    });
...
//# sourceMappingURL=notify-hook-modules.test.js.map