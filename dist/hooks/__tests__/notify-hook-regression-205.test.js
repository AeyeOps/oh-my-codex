...
function runNotifyHook(cwd, fakeBinDir, codexHome, payloadOverrides = {}) {
    const payload = {
        cwd,
...
describe('regression-205: DEFAULT_STALL_PATTERNS excludes permission-seeking prompts after #1416', () => {
    it('DEFAULT_STALL_PATTERNS array does not include "if you want"', async () => {
        ...
        assert.ok(Array.isArray(DEFAULT_STALL_PATTERNS), 'DEFAULT_STALL_PATTERNS should be an array');
...
describe('regression-205: detectStallPattern excludes "if you want" after #1416', () => {
    it('does not detect "if you want" pattern', async () => {
        ...
        assert.equal(detectStallPattern('If you want, I can refactor the module.', DEFAULT_STALL_PATTERNS), false);
    });
    it('does not detect "if you want" case-insensitively', async () => {
        const { detectStallPattern, DEFAULT_STALL_PATTERNS } = await loadModule('notify-hook/auto-nudge.js');
        assert.equal(detectStallPattern('IF YOU WANT I can do more.', DEFAULT_STALL_PATTERNS), false);
    });
    it('ignores OMX injection-marker lines when matching patterns', async () => {
        const { detectStallPattern, DEFAULT_STALL_PATTERNS } = await loadModule('notify-hook/auto-nudge.js');
        assert.equal(detectStallPattern('keep going [OMX_TMUX_INJECT]', DEFAULT_STALL_PATTERNS), false);
    });
    it('does not false-positive on unrelated text', async () => {
        const { detectStallPattern, DEFAULT_STALL_PATTERNS } = await loadModule('notify-hook/auto-nudge.js');
        assert.equal(detectStallPattern('Build succeeded. All tests pass.', DEFAULT_STALL_PATTERNS), false);
    });
});
// ---------------------------------------------------------------------------
...
describe('regression-205: notify-hook ignores "if you want" for default auto-nudge', () => {
    let originalTeamWorker;
    let originalTeamStateRoot;
    before(() => {
    ...
    it('does not record pending stall state for permission-seeking "if you want" text', async () => {
        await withTempWorkingDir(async (cwd) => {
            ...
            assert.doesNotMatch(tmuxLog, /send-keys -t %99 -l yes, proceed \[OMX_TMUX_INJECT\]/, 'default notify-hook path should not inject for permission-seeking prompts');
...
describe('regression-205: resolveTeamStateDirForWorker is exported from team-worker.js', () => {
    it('exports resolveTeamStateDirForWorker as a function', async () => {
        ...
        assert.equal(typeof mod.resolveTeamStateDirForWorker, 'function', 'resolveTeamStateDirForWorker should be an exported function');
    });
    it('fails closed for OMX_TEAM_STATE_ROOT when worker identity is missing', async () => {
        ...
        try {
            const result = await resolveTeamStateDirForWorker('/some/cwd', { teamName: 'fix-ts', workerName: 'worker-1' });
    ...
    it('does not guess {cwd}/.omx/state when no worker identity exists', async () => {
        ...
        try {
            ...
            const result = await resolveTeamStateDirForWorker(cwd, { teamName: 'fix-ts', workerName: 'worker-1' });
            assert.equal(result, null);
        }
        finally {
...
//# sourceMappingURL=notify-hook-regression-205.test.js.map