...
function buildSessionIdlePlugin(targetPath) {
...
function runNotifyHook(repoRoot, cwd, lastAssistantMessage, turnId, envOverrides = {}) {
    const payload = {
        cwd,
...
describe('notify-hook session-idle dedupe', () => {
    it('suppresses repeated unchanged post_turn_idle_notification hook events once the first hook dispatch succeeds', async () => {
        ...
        try {
            ...
            const pluginState = await readJson(pluginStatePath);
...
//# sourceMappingURL=notify-hook-session-idle-dedupe.test.js.map