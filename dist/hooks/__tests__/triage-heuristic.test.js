...
// Helpers
// ---------------------------------------------------------------------------
function assertLane(prompt, lane) {
    const result = triagePrompt(prompt);
    assert.equal(result.lane, lane, `prompt="${prompt}" expected lane=${lane} got lane=${result.lane} (reason=${result.reason})`);
}
function assertLightDestination(prompt, destination) {
    const result = triagePrompt(prompt);
    assert.equal(result.lane, 'LIGHT', `prompt="${prompt}" expected LIGHT got ${result.lane} (reason=${result.reason})`);
    assert.equal(result.destination, destination, `prompt="${prompt}" expected destination=${destination} got ${result.destination}`);
}
// ---------------------------------------------------------------------------
...
describe('triagePrompt — determinism', () => {
    it('returns identical results across 1000 calls for the same input', () => {
        ...
        for (let i = 1; i < 1000; i++) {
            const result = triagePrompt(prompt);
            assert.equal(result.lane, first.lane, `non-deterministic: call ${i} returned lane=${result.lane}, first was ${first.lane}`);
            assert.equal(result.destination, first.destination, `non-deterministic: call ${i} returned destination=${result.destination}, first was ${first.destination}`);
            assert.equal(result.reason, first.reason, `non-deterministic: call ${i} returned reason=${result.reason}, first was ${first.reason}`);
        }
    });
});
// ---------------------------------------------------------------------------
...
describe('triagePrompt — performance benchmark', () => {
    it('classifies the full canonical corpus 1000x with avg < 1 ms per call', () => {
        ...
        console.log(`[triage-heuristic benchmark] ${totalCalls} calls in ${elapsed.toFixed(2)} ms` +
            ` — avg ${avgPerCall.toFixed(4)} ms/call (corpus size: ${corpusLength})`);
        assert.ok(avgPerCall < 1.0, `avg per call ${avgPerCall.toFixed(4)} ms exceeded 1 ms soft bound (total: ${elapsed.toFixed(2)} ms over ${totalCalls} calls)`);
    });
});
//# sourceMappingURL=triage-heuristic.test.js.map