...
import { codexHome, listInstalledSkillDirectories, omxNotepadPath, omxProjectMemoryPath, packageRoot, } from "../utils/paths.js";
...
import { SKILL_ACTIVE_STATE_FILE, listActiveSkills, readVisibleSkillActiveState, } from "../state/skill-active.js";
...
function capBodyToMax(sections, maxBody) {
    ...
    if (body.length > maxBody) {
        if (maxBody <= 3)
            return ".".repeat(Math.max(0, maxBody));
        body = body.slice(0, maxBody - 3) + "...";
    }
    return body;
}
// ── Overlay generation ───────────────────────────────────────────────────────
async function isRalphActive(cwd, sessionId) {
...
async function readRalphPlanningArtifacts(cwd) {
...
async function readActiveModes(cwd, sessionId) {
    if (sessionId && !existsSync(getStateDir(cwd, sessionId))) {
        return "";
    }
    const refs = await listModeStateFilesWithScopePreference(cwd, sessionId);
    const canonicalState = await readVisibleSkillActiveState(cwd, sessionId);
    const canonicalSkills = new Map(listActiveSkills(canonicalState).map((entry) => [entry.skill, entry]));
    ...
    const modes = [];
    const emittedCanonicalSkills = new Set();
    ...
    if (!useCompatibilityFallback) {
        for (const [skill, entry] of [...canonicalSkills.entries()].sort((a, b) => a[0].localeCompare(b[0]))) {
            if (emittedCanonicalSkills.has(skill))
                continue;
            const details = [];
            if (entry.phase)
                details.push(`phase: ${entry.phase}`);
...
async function readTeamOrchestratorOverlay() {
...
export async function resolveSessionOrchestrationMode(cwd, sessionId, activeSkill) {
    if (activeSkill === "team")
        return "team";
    if (activeSkill)
        return "default";
 ...
 */
export async function generateOverlay(cwd, sessionId, options = {}) {
    ...
    if (ralphActive) {
        ...
        if (!planningArtifacts.hasPrd)
            missing.push("`prd-*.md`");
        if (!planningArtifacts.hasTestSpec)
            missing.push("`test-spec-*.md`");
    ...
    if (overlay.length <= MAX_OVERLAY_SIZE)
        return overlay;
...
function stripOverlayContent(content) {
    ...
    while (iterations < MAX_STRIP_ITERATIONS) {
        ...
        if (endIdx < 0) {
            ...
            const nextMarkerIdx = markerCandidates.length > 0 ? Math.min(...markerCandidates) : -1;
            if (nextMarkerIdx < 0) {
...
export function sessionModelInstructionsPath(cwd, sessionId) {
 ...
 */
export async function writeSessionModelInstructionsFile(cwd, sessionId, overlay) {
    ...
    for (const sourcePath of sourcePaths) {
        if (seenPaths.has(sourcePath) || !existsSync(sourcePath))
            continue;
 ...
 */
export async function removeSessionModelInstructionsFile(cwd, sessionId) {
    const sessionPath = sessionModelInstructionsPath(cwd, sessionId);
