...
export function isExploreCommandRoutingEnabled(env = process.env) {
    const raw = env[OMX_EXPLORE_CMD_ENV];
    if (typeof raw !== 'string')
        return true;
    return !DISABLED_VALUES.has(raw.trim().toLowerCase());
}
export function isSimpleExplorationPrompt(text) {
    const trimmed = text.trim();
    if (!trimmed)
        return false;
    if (NON_EXPLORATION_PATTERNS.some((pattern) => pattern.test(trimmed)))
        return false;
    return SIMPLE_EXPLORATION_PATTERNS.some((pattern) => pattern.test(trimmed));
}
export function buildExploreRoutingGuidance(env = process.env) {
    if (!isExploreCommandRoutingEnabled(env))
        return '';
    return [
...
//# sourceMappingURL=explore-routing.js.map