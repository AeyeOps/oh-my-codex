...
function hashDedupeKey(target, text) {
...
function resolveSessionPaneTarget(sessionName) {
...
function resolveTmuxTarget(options) {
    ...
    if (paneId) {
        ...
        if (pane.ok && pane.stdout)
            return { ok: true, reason: 'ok', target: pane.stdout, paneId: pane.stdout };
...
async function sendTmuxKeys(options, context) {
    ...
    const tmuxState = await readJsonIfExists(tmuxStatePath, {
        last_sent_at: 0,
        recent_keys: {},
    });
    const now = Date.now();
    ...
    tmuxState.recent_keys = Object.fromEntries(Object.entries(tmuxState.recent_keys || {}).filter(([, ts]) => Number.isFinite(ts) && ts >= minTs));
...
export function createHookPluginTmuxApi(options) {
    return {
        sendKeys: (sendOptions) => sendTmuxKeys(sendOptions, options),
    };
}
