...
describe('dispatchHookEvent', () => {
    ...
    it('does not execute plugin top-level code in the parent process', async () => {
        ...
        try {
            const dir = join(cwd, '.omx', 'hooks');
            await mkdir(dir, { recursive: true });
            await writeFile(join(dir, 'top-level-side-effect.mjs'), `import { appendFileSync } from 'node:fs';
import { join } from 'node:path';
...
`);
    ...
    it('returns timeout promptly when plugin ignores SIGTERM', async () => {
        ...
        try {
            const dir = join(cwd, '.omx', 'hooks');
            await mkdir(dir, { recursive: true });
            await writeFile(join(dir, 'ignore-sigterm.mjs'), `export async function onHookEvent() {
          process.on('SIGTERM', () => {});
          setInterval(() => {}, 60_000);
          await new Promise(() => {});
        }`);
    ...
    it('dedupes repeated native lifecycle hook dispatches for the same session/turn fingerprint', async () => {
        ...
        try {
            const dir = join(cwd, '.omx', 'hooks');
            await mkdir(dir, { recursive: true });
            await writeFile(join(dir, 'good.mjs'), 'export async function onHookEvent(event, sdk) { const count = Number((await sdk.state.read("count", 0)) || 0); await sdk.state.write("count", count + 1); }');
