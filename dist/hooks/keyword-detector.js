...
import { SKILL_ACTIVE_STATE_FILE, listActiveSkills, writeSkillActiveStateCopies, } from '../state/skill-active.js';
...
const PLANNING_LIKE_WORKFLOW_SKILLS = new Set([
    'deep-interview',
    'ralplan',
]);
const EXECUTION_LIKE_WORKFLOW_SKILLS = new Set([
    'autopilot',
    'autoresearch',
    'ralph',
    'team',
...
const STATEFUL_SKILL_SEED_CONFIG = {
...
function preserveCompletedDeepInterviewPhase(previousModeState) {
    if (!previousModeState || previousModeState.active !== false)
        return '';
    return safeString(previousModeState.current_phase).trim();
}
function releaseDeepInterviewInputLock(previous, nowIso, reason = 'handoff') {
    if (!previous)
        return undefined;
    return {
        ...previous,
...
async function readExistingSkillState(statePath) {
    try {
...
function buildActiveSkills(state) {
    if (!state.active)
        return undefined;
...
async function readExistingDeepInterviewState(statePath) {
    try {
        const raw = await readFile(statePath, 'utf-8');
        return JSON.parse(raw);
    }
...
export async function persistDeepInterviewModeState(stateDir, nextSkill, nowIso, previousSkill, input) {
    const statePath = resolveSeedStateFilePath(stateDir, 'deep-interview', nextSkill?.session_id ?? previousSkill?.session_id ?? input.sessionId).absolutePath;
    ...
    if (nextSkill?.skill === 'deep-interview' && nextSkill.active) {
        const nextQuestionEnforcement = clearDeepInterviewQuestionObligation(previousModeState?.question_enforcement, 'handoff', new Date(nowIso));
        const nextState = withModeRuntimeContext(previousModeState ?? {}, {
    ...
    if (!previousModeState?.active && !hadActiveDeepInterview)
        return;
    ...
    const nextState = {
        ...
        ...(previousModeState?.question_enforcement
            ? {
                question_enforcement: clearDeepInterviewQuestionObligation(previousModeState.question_enforcement, questionExitReason, new Date(nowIso)),
            }
            : {}),
    };
    await writeFile(statePath, JSON.stringify(nextState, null, 2));
}
function resolveSeedStateFilePath(stateDir, mode, sessionId, scope = 'session') {
...
async function persistStatefulSkillSeedState(stateDir, nextSkill, nowIso, previousSkill) {
    ...
    const startedAt = previousSkill?.skill === nextSkill.skill && previousSkill.active
        ...
        : preserveExistingModeState
            ...
            : nowIso;
    const baseState = withModeRuntimeContext((preserveExistingModeState ? existingModeState : {}) ?? {}, {
...
function parseExplicitSkillInvocations(text) {
    const results = [];
    ...
    while ((match = regex.exec(text)) !== null) {
        ...
        if (!registryEntry)
            continue;
        if (results.some((item) => item.skill === normalizedSkill))
            continue;
        results.push({
            keyword: rawKeyword,
...
function hasIntentContextForKeyword(text, keyword) {
    ...
    if (!KEYWORDS_REQUIRING_INTENT.has(k))
        return true;
    const patterns = KEYWORD_INTENT_PATTERNS[k];
...
export function detectKeywords(text) {
    ...
    for (const { pattern, skill, priority } of KEYWORD_MAP) {
        ...
        if (match) {
            if (!hasIntentContextForKeyword(normalizedText, match[0].toLowerCase()))
                continue;
            implicit.push({
                keyword: match[0],
                skill,
...
function isNamedActiveSkillContinuationPrompt(text, skill) {
...
function initialWorkflowPhaseForMode(mode) {
...
export async function recordSkillActivation(input) {
    ...
    if (hasCancelIntent && hadDeepInterviewLock) {
        ...
        try {
            await writeSkillActiveStateCopies(dirname(dirname(input.stateDir)), state, input.sessionId, selectRootSkillStateCopy(previousRoot, state, input.sessionId));
    ...
    if (isTrackedWorkflowMode(match.skill)) {
        ...
        const { requestedSkills: requestedWorkflowSkills, deferredSkills } = resolveRequestedWorkflowSkills(workflowMatches.length > 0 ? workflowMatches : [match.skill]);
        let nextWorkflowEntries = previousWorkflowEntries.map((entry) => ({ ...entry }));
        const transitionMessages = [];
        for (const requestedMode of requestedWorkflowSkills) {
            const decision = evaluateWorkflowTransition(nextWorkflowEntries.map((entry) => entry.skill), requestedMode);
            if (!decision.allowed) {
                return {
                    ...(previous ?? {}),
                    version: 1,
                    ...
                    transition_error: buildWorkflowTransitionError(nextWorkflowEntries.map((entry) => entry.skill), requestedMode, 'activate'),
                };
            }
            if (decision.autoCompleteModes.length > 0) {
                const transition = await reconcileWorkflowTransition(dirname(dirname(input.stateDir)), requestedMode, {
                    action: 'activate',
        ...
        try {
            ...
            await writeSkillActiveStateCopies(dirname(dirname(input.stateDir)), nextState, input.sessionId, selectRootSkillStateCopy(previousRoot, nextState, input.sessionId));
    ...
    const state = {
        ...
        phase: initialWorkflowPhaseForMode(match.skill),
...
export function applyRalplanGate(keywords, text, options = {}) {
    ...
    const shortFollowupBypasses = executionKeywords.filter((keyword) => {
        ...
        if (normalizedKeyword !== 'team' && normalizedKeyword !== 'ralph')
            return false;
        return isApprovedExecutionFollowupShortcut(normalizedKeyword, text, {
            planningComplete,
