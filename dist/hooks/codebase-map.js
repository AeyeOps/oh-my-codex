 ...
 */
function getTrackedSourceFiles(cwd) {
    try {
 ...
 */
function buildDirLine(dir, files) {
    const names = files
 ...
 */
export async function generateCodebaseMap(cwd) {
    try {
        ...
        for (const dir of sortedDirs.slice(0, MAX_DIRS)) {
            ...
            if (dir === 'src') {
                ...
                for (const f of dirFiles) {
                    ...
                    if (!subGrouped.has(subDir))
                        subGrouped.set(subDir, []);
