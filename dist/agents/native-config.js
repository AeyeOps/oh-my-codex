...
import { getEnvConfiguredStandardDefaultModel, getMainDefaultModel, getSparkDefaultModel, getStandardDefaultModel, } from "../config/models.js";
...
function readConfigTomlContent(codexHomeOverride, provided) {
...
function resolveFrontierModel(options) {
    const configTomlContent = readConfigTomlContent(options.codexHomeOverride, options.configTomlContent);
    return getRootModelName(configTomlContent)
        ?? getMainDefaultModel(options.codexHomeOverride);
}
function resolveStandardModel(options) {
    const explicitStandardModel = getEnvConfiguredStandardDefaultModel(options.env ?? process.env, options.codexHomeOverride);
    if (explicitStandardModel)
        return explicitStandardModel;
    return getStandardDefaultModel(options.codexHomeOverride);
}
function resolveAgentModel(agent, options = {}) {
...
function isExactMiniModel(resolvedModel) {
    return resolvedModel?.trim() === EXACT_GPT_5_4_MINI_MODEL;
}
export function composeRoleInstructions(promptContent, metadata, resolvedModel) {
    const instructions = stripFrontmatter(promptContent);
    const parts = [instructions];
    if (metadata) {
        parts.push("", POSTURE_OVERLAYS[metadata.posture], "", MODEL_CLASS_OVERLAYS[metadata.modelClass]);
    }
    ...
    const metadataLines = [];
    if (metadata) {
        metadataLines.push("## OMX Agent Metadata", `- role: ${metadata.name}`, `- posture: ${metadata.posture}`, `- model_class: ${metadata.modelClass}`, `- routing_role: ${metadata.routingRole}`);
    }
    if (resolvedModel) {
...
export function composeRoleInstructionsForRole(roleName, promptContent, resolvedModel) {
    const agent = AGENT_DEFINITIONS[roleName];
    return composeRoleInstructions(promptContent, agent
        ? {
            name: agent.name,
...
export function generateStandaloneAgentToml(config) {
    ...
    if (typeof config.developerInstructions === "string" &&
 ...
 */
export function generateAgentToml(agent, promptContent, options = {}) {
...
export async function installNativeAgentConfigs(pkgRoot, options = {}) {
    const { force = false, dryRun = false, verbose = false, agentsDir = codexAgentsDir(), catalogManifest, allowUncatalogedDefinitions = false, } = options;
    ...
    for (const name of [...installableAgentNames].sort()) {
        ...
        if (!agent) {
            if (verbose)
                console.log(`  skip ${name} (no agent definition)`);
            continue;
        }
