...
import { listActiveSkills, readVisibleSkillActiveState, syncCanonicalSkillStateForMode, writeSkillActiveStateCopies, } from '../skill-active.js';
...
describe('skill-active state helpers', () => {
    ...
    it('preserves root-scoped team state when a session-scoped ralph overlap is activated', async () => {
        await withTempRepo('omx-skill-active-team-ralph-', async (cwd) => {
            ...
            const rootState = JSON.parse(await readFile(join(cwd, '.omx', 'state', 'skill-active-state.json'), 'utf-8'));
            assert.deepEqual(rootState.active_skills?.map(({ skill, phase, session_id }) => ({
                skill,
                phase,
                session_id,
            ...
            assert.ok(sessionState);
            assert.deepEqual(listActiveSkills(sessionState).map(({ skill, phase, session_id }) => ({
                skill,
                phase,
                session_id,
            })), [
...
//# sourceMappingURL=skill-active.test.js.map