...
describe('state operations directory initialization', () => {
    ...
    it('writes and reads deep-interview state', async () => {
        ...
        try {
            ...
            const readBody = readResponse.payload;
    ...
    it('lists active modes from the explicit session scope without leaking a sibling Ralph session', async () => {
        ...
        try {
            ...
            await writeFile(join(foreignSessionDir, 'ralph-state.json'), JSON.stringify({ active: true, current_phase: 'executing' }, null, 2));
            const response = await executeStateOperation('state_list_active', {
                workingDirectory: wd,
    ...
    it('serializes concurrent state_write calls per mode file and preserves merged fields', async () => {
        ...
        try {
            const writes = Array.from({ length: 16 }, (_, i) => executeStateOperation('state_write', {
                workingDirectory: wd,
                mode: 'team',
                state: { [`k${i}`]: i },
            }));
    ...
    it('syncs canonical skill-active state for tracked mode writes and clears', async () => {
        ...
        try {
            ...
            const cleared = JSON.parse(await readFile(canonicalPath, 'utf-8'));
    ...
    it('denies unsupported overlaps without writing the requested mode state', async () => {
        ...
        try {
            ...
            assert.match(String(denied.payload.error || ''), /Unsupported workflow overlap: team \+ autopilot\./);
    ...
    it('does not auto-complete existing workflow state when tracked write validation fails', async () => {
        ...
        try {
            ...
            await mkdir(sessionDir, { recursive: true });
            await writeFile(join(sessionDir, 'ralplan-state.json'), JSON.stringify({ active: true, mode: 'ralplan', current_phase: 'planning' }, null, 2));
            const denied = await executeStateOperation('state_write', {
                workingDirectory: wd,
            ...
            assert.equal(denied.isError, true);
            assert.match(String(denied.payload.error || ''), /ralph\.current_phase/i);
            const ralplanState = JSON.parse(await readFile(join(sessionDir, 'ralplan-state.json'), 'utf-8'));
    ...
    it('keeps session-scoped tracked state writable after root-state parse fallback on resume', async () => {
        ...
        try {
            ...
            const sessionState = JSON.parse(await readFile(join(sessionDir, 'ralph-state.json'), 'utf-8'));
            ...
            const rootState = JSON.parse(await readFile(join(stateDir, 'ralph-state.json'), 'utf-8'));
