...
import { getAllScopedStatePaths, getReadScopedStateDirs, getReadScopedStatePaths, getStateDir, getStatePath, resolveStateScope, resolveWorkingDirectoryForState, validateSessionId, validateStateModeSegment, } from '../mcp/state-paths.js';
import { ensureCanonicalRalphArtifacts } from '../ralph/persistence.js';
import { RALPH_PHASES, validateAndNormalizeRalphState } from '../ralph/contract.js';
import { applyRunOutcomeContract } from '../runtime/run-outcome.js';
import { SKILL_ACTIVE_STATE_MODE, readSkillActiveState, syncCanonicalSkillStateForMode, writeSkillActiveStateCopies, } from './skill-active.js';
...
async function writeClearedSessionScopedModeState(path, mode, sessionId) {
...
function readModeSupportsStrictValidation(mode) {
    return SUPPORTED_STATE_READ_MODES.includes(mode);
}
function validateStrictReadableMode(mode) {
...
async function initializeStateEnvironment(cwd, effectiveSessionId) {
...
async function listStateSessionIds(cwd) {
...
export async function listStateStatuses(cwd, explicitSessionId, mode) {
    ...
    for (const stateDir of stateDirs) {
        ...
        for (const file of files) {
            ...
            if (!mode && currentMode === SKILL_ACTIVE_STATE_MODE)
                continue;
            if (mode && currentMode !== mode)
                continue;
            if (seenModes.has(currentMode))
                continue;
...
export async function listActiveStateModes(workingDirectory, explicitSessionId) {
...
export async function executeStateOperation(name, rawArgs = {}) {
    let cwd;
    let explicitSessionId;
    try {
        cwd = resolveWorkingDirectoryForState(rawArgs.workingDirectory);
    ...
    try {
        ...
        switch (name) {
            ...
            case 'state_write': {
                ...
                const { mode: _mode, workingDirectory: _workingDirectory, session_id: _sessionId, state: customState, ...fields } = rawArgs;
                ...
                await withStateWriteLock(path, async () => {
                    ...
                    const explicitRunOutcome = Object.prototype.hasOwnProperty.call(fields, 'run_outcome')
                        || (customState != null
                            && Object.prototype.hasOwnProperty.call(customState, 'run_outcome'));
