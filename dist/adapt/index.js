...
import { applyHermesEnvelope, applyHermesProbe, applyHermesStatus, buildHermesBootstrapMetadata, buildHermesRuntimeObservation, collectHermesEvidence, } from "./hermes.js";
import { buildOpenClawDoctorReport, buildOpenClawEnvelope, buildOpenClawProbeReport, buildOpenClawStatusReport, initOpenClawFoundation, } from "./openclaw.js";
...
export function supportedAdaptTargets() {
    return listAdaptTargets().map((descriptor) => descriptor.target);
}
export function buildAdaptPlanningLink(cwd) {
...
export function buildAdaptEnvelope(cwd, target, now = new Date()) {
...
export async function buildAdaptEnvelopeForTarget(cwd, target, now = new Date()) {
...
export function buildAdaptProbeReport(cwd, target, now = new Date()) {
    const descriptor = getAdaptTargetDescriptor(target);
    if (!descriptor) {
        throw new Error(`Unknown adapt target: ${target}`);
    }
    ...
    const planning = buildAdaptPlanningLink(cwd);
    if (target === "openclaw") {
        return buildOpenClawProbeReport(paths, planning, descriptor.capabilities, now);
    }
    return {
        schemaVersion: ADAPT_SCHEMA_VERSION,
...
export async function buildAdaptProbeReportForTarget(cwd, target, now = new Date()) {
...
export function buildAdaptStatusReport(cwd, target, now = new Date()) {
    const descriptor = getAdaptTargetDescriptor(target);
    if (!descriptor) {
        throw new Error(`Unknown adapt target: ${target}`);
    }
    ...
    const planning = buildAdaptPlanningLink(cwd);
    if (target === "openclaw") {
        return buildOpenClawStatusReport(paths, planning, descriptor.capabilities, now);
    }
    return {
        schemaVersion: ADAPT_SCHEMA_VERSION,
        timestamp: toIsoTimestamp(now),
...
export async function buildAdaptStatusReportForTarget(cwd, target, now = new Date()) {
...
export function buildAdaptDoctorReport(cwd, target, now = new Date()) {
    const descriptor = getAdaptTargetDescriptor(target);
    if (!descriptor) {
        throw new Error(`Unknown adapt target: ${target}`);
    }
    ...
    const issues = [];
    if (target === "openclaw") {
        return buildOpenClawDoctorReport(resolveAdaptPaths(cwd, target), planning, now);
    }
...
export async function buildAdaptDoctorReportForTarget(cwd, target, now = new Date()) {
    ...
    const issues = report.issues.filter((issue) => issue.code !== "target_specific_logic_deferred");
    ...
    return {
        ...report,
        summary: "Hermes doctor inspects external ACP, gateway, and session-store evidence plus OMX-owned adapter readiness.",
        issues,
        nextSteps: [
            ...bootstrap.nextSteps,
            ...report.nextSteps.filter((step) => !/follow-on integration gaps/i.test(step)),
        ],
    };
}
export function initAdaptFoundation(cwd, target, write = false, now = new Date()) {
    const descriptor = getAdaptTargetDescriptor(target);
    if (!descriptor) {
        throw new Error(`Unknown adapt target: ${target}`);
    }
    ...
    const planning = buildAdaptPlanningLink(cwd);
    if (target === "openclaw") {
        return initOpenClawFoundation(paths, planning, descriptor.capabilities, write, now);
    }
    const envelope = buildAdaptEnvelope(cwd, target, now);
    ...
    if (write) {
        ...
        writeFileSync(envelopePaths.configPath, `${JSON.stringify(config, null, 2)}\n`, "utf-8");
        writeFileSync(envelopePaths.envelopePath, `${JSON.stringify(envelope, null, 2)}\n`, "utf-8");
...
export async function initAdaptFoundationForTarget(cwd, target, write = false, now = new Date()) {
