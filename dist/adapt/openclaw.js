...
function summarizeObservedState(metadata) {
...
function observeOpenClaw(paths, planning) {
        ...
        for (const event of OPENCLAW_HOOK_EVENTS) {
            ...
            if (!resolved) {
                hooks.push({
                    ...
                    gatewayType: null,
                    status: "blocked",
                    detail: "The hook mapping exists, but the referenced gateway is missing or disabled locally.",
                });
                continue;
            }
            const gatewayType = resolved.gateway.type === "command" ? "command" : "http";
            if (gatewayType === "command" && !inspection.commandGateEnabled) {
                hooks.push({
                    ...
                    gatewayType,
                    status: "blocked",
                    detail: "Mapped to a command gateway, but OMX_OPENCLAW_COMMAND=1 is not set.",
                });
                continue;
            }
            hooks.push({
    ...
    const observedState = inspection.state === "configured" &&
...
export function buildOpenClawEnvelope(paths, planning, capabilities, now) {
...
export function buildOpenClawProbeReport(paths, planning, capabilities, now) {
    const openclaw = observeOpenClaw(paths, planning);
    const blockedHooks = openclaw.hooks.filter((hook) => hook.status === "blocked");
    return {
        schemaVersion: ADAPT_SCHEMA_VERSION,
        ...
        openclaw,
        nextSteps: blockedHooks.length > 0
...
export function buildOpenClawStatusReport(paths, planning, capabilities, now) {
...
export function buildOpenClawDoctorReport(paths, planning, now) {
    const openclaw = observeOpenClaw(paths, planning);
    const issues = [];
    ...
    else if (openclaw.observedState === "missing-config" ||
...
export function initOpenClawFoundation(paths, planning, capabilities, write, now) {
