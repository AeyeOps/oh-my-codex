...
import { buildAdaptDoctorReportForTarget, buildAdaptEnvelopeForTarget, buildAdaptProbeReportForTarget, buildAdaptStatusReportForTarget, initAdaptFoundationForTarget, } from "../index.js";
let tempDir;
let hermesRoot;
let hermesHome;
let processCwdFixtureBase;
let processCwdFixtureRoot;
let suppliedCwd;
let originalCwd;
const originalEnv = {
...
function writeHermesFixture(options = {}) {
    const { withRoot = true, withGatewayRuntime = false, withStateDb = false, } = options;
    if (withRoot) {
    ...
    if (withGatewayRuntime) {
        ...
        writeFileSync(join(hermesHome, "gateway.pid"), JSON.stringify({ pid: 1234, kind: "hermes-gateway", argv: ["hermes", "gateway"] }));
        writeFileSync(join(hermesHome, "gateway_state.json"), JSON.stringify({
...
beforeEach(async () => {
    ...
    processCwdFixtureRoot = join(processCwdFixtureBase, "hermes-codex-skill-omx-aware-prd", "external", "hermes-agent");
...
describe("hermes adapter integration", () => {
    ...
    it("anchors sibling-default Hermes discovery to the supplied cwd for programmatic probes", async () => {
        ...
        const tempSiblingRoot = join(suppliedCwd, "..", "hermes-codex-skill-omx-aware-prd", "external", "hermes-agent");
    ...
    it("reports Hermes capability evidence asymmetrically in the envelope", async () => {
        ...
        assert.deepEqual([...new Set(envelope.capabilities.map((capability) => capability.ownership))].sort(), ["omx-owned", "shared-contract", "target-observed"]);
        assert.equal(envelope.capabilities.find((capability) => capability.id === "persistent-session-observation")?.status, "ready");
        assert.equal(envelope.capabilities.find((capability) => capability.id === "acp-envelope-bridge")?.status, "ready");
    });
    ...
    it("keeps Hermes init writes inside OMX-owned adapter paths", async () => {
        ...
        const persistedEnvelope = JSON.parse(readFileSync(result.envelope.adapterPaths.envelopePath, "utf-8"));
...
//# sourceMappingURL=hermes.test.js.map