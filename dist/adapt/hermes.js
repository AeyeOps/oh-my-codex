...
function resolveRelativeToCwd(cwd, pathValue) {
    return isAbsolute(pathValue) ? pathValue : resolve(cwd, pathValue);
}
function resolveDefaultHermesSiblingRoot(cwd) {
    return resolve(cwd, "..", "hermes-codex-skill-omx-aware-prd", "external", "hermes-agent");
}
function resolveHermesRoot(cwd) {
...
function resolveHermesHome(cwd) {
...
function collectPathEvidence(root, relativePaths) {
    const present = [];
    const missing = [];
...
async function sqliteHasTable(path, table) {
    if (!existsSync(path))
        return false;
    let handle;
    try {
...
function extractConnectedPlatforms(runtime) {
...
function inferGatewayLive(pidRecord, runtimeRecord) {
...
export async function collectHermesEvidence(cwd = process.cwd()) {
    ...
    const [gatewayPidReadable, gatewayStateReadable, stateDbReadable, pidRecord, runtimeRecord, sessionsTablePresent,] = await Promise.all([
        isReadable(gatewayPidPath),
        isReadable(gatewayStatePath),
        isReadable(stateDbPath),
        readJsonFile(gatewayPidPath),
        readJsonFile(gatewayStatePath),
...
export function buildHermesCapabilityOverrides(capabilities, evidence) {
...
export function buildHermesBootstrapMetadata(evidence) {
    const commands = [
        ...ACP_COMMANDS,
        ...STATUS_COMMANDS,
    ];
...
export function buildHermesRuntimeObservation(evidence) {
...
export function applyHermesEnvelope(envelope, evidence) {
    return {
        ...envelope,
...
export function applyHermesProbe(report, evidence) {
    const nextSteps = [
...
export function applyHermesStatus(report, evidence) {
