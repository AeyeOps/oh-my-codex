...
describe('modes/base tmux pane capture', () => {
    ...
    it('blocks exclusive mode startup when another exclusive state file is malformed', async () => {
        ...
        try {
            ...
            await assert.rejects(() => startMode('autopilot', 'test', 1, wd), /repair or clear that workflow state yourself via `omx state clear --mode ralph` or the `omx_state\.\*` MCP tools/i);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
...
//# sourceMappingURL=base-tmux-pane.test.js.map