...
describe('modes/base multi-state compatibility', () => {
    it('allows the approved team + ralph overlap across root and session scopes', async () => {
        ...
        try {
            ...
            await writeFile(join(wd, '.omx', 'state', 'session.json'), JSON.stringify({ session_id: 'sess-team-ralph' }));
            await startMode('ralph', 'complete the approved plan', 5, wd);
            assert.equal(existsSync(join(wd, '.omx', 'state', 'team-state.json')), true);
            assert.equal(existsSync(join(wd, '.omx', 'state', 'sessions', 'sess-team-ralph', 'ralph-state.json')), true);
    ...
    it('rejects standalone autopilot + team overlaps with actionable clearing guidance', async () => {
        ...
        try {
            ...
            await assert.rejects(() => startMode('team', 'attempt invalid overlap', 5, wd), /omx state.*omx_state\.\*/i);
            const autopilotState = JSON.parse(await readFile(join(wd, '.omx', 'state', 'autopilot-state.json'), 'utf-8'));
...
//# sourceMappingURL=base-multi-state-compat.test.js.map