...
describe('modes/base autoresearch contract integration', () => {
    it('startMode auto-completes deep-interview when starting ralplan', async () => {
        ...
        try {
            ...
            const completed = JSON.parse(await readFile(join(wd, '.omx', 'state', 'deep-interview-state.json'), 'utf-8'));
    ...
    it('startMode blocks autoresearch when ralph is active', async () => {
        ...
        try {
            ...
            await assert.rejects(() => startMode('autoresearch', 'demo mission', 1, wd), /Cannot start autoresearch: ralph is already active/i);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it('startMode blocks execution-to-planning rollback auto-complete', async () => {
        ...
        try {
            ...
            await assert.rejects(() => startMode('ralplan', 'plan again', 5, wd), /Execution-to-planning rollback auto-complete is not allowed/i);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it('startMode persists autoresearch state when no exclusive conflict exists', async () => {
        ...
        try {
            ...
            assert.deepEqual(listActiveSkills(canonical ?? {}).map(({ skill, phase }) => ({ skill, phase })), [{ skill: 'autoresearch', phase: 'starting' }]);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
...
//# sourceMappingURL=base-autoresearch-contract.test.js.map