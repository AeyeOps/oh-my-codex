...
describe('modes/base ralph contract integration', () => {
    it('startMode rejects invalid Ralph max_iterations values', async () => {
        ...
        try {
            await assert.rejects(() => startMode('ralph', 'demo', 0, wd), /ralph\.max_iterations must be a finite (number|integer) > 0/);
    ...
    it('updateModeState rejects invalid Ralph phase and keeps previous persisted state', async () => {
        ...
        try {
            ...
            assert.ok(before);
            await assert.rejects(() => updateModeState('ralph', { current_phase: 'bananas', iteration: -1, max_iterations: 0 }, wd), /ralph\.current_phase must be one of:/);
...
//# sourceMappingURL=base-ralph-contract.test.js.map