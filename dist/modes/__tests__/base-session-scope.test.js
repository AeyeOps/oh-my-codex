...
describe('modes/base session-scoped persistence', () => {
    it('writes mode state into the current session scope when session.json exists', async () => {
        ...
        try {
            ...
            const raw = JSON.parse(await readFile(scopedPath, 'utf-8'));
    ...
    it('persists owner_omx_session_id for Ralph when session scope is active', async () => {
        ...
        try {
            ...
            const scoped = JSON.parse(await readFile(join(sessionDir, 'ralph-state.json'), 'utf-8'));
    ...
    it('prefers session-scoped reads over root fallback and writes updates back to the session scope', async () => {
        ...
        try {
            ...
            const scoped = JSON.parse(await readFile(join(sessionDir, 'ralplan-state.json'), 'utf-8'));
            const root = JSON.parse(await readFile(join(stateDir, 'ralplan-state.json'), 'utf-8'));
...
//# sourceMappingURL=base-session-scope.test.js.map