...
function hasExplicitQuestionPaneTarget(env) {
...
function hasWindowsPsmuxReturnBridge(env, platform) {
    if (platform !== 'win32')
        return false;
    if (hasExplicitQuestionPaneTarget(env))
        return true;
...
export function resolveQuestionRendererStrategy(env = process.env, 
...
_tmuxBinary, options) {
    ...
    if (safeString(env.OMX_QUESTION_TEST_RENDERER).trim() === 'noop')
        return 'test-noop';
    if (hasWindowsPsmuxReturnBridge(env, platform))
        return 'windows-console';
    if (safeString(env.TMUX).trim() !== '')
        return 'inside-tmux';
    if (hasExplicitQuestionPaneTarget(env))
        return 'inside-tmux';
    if (options?.cwd && readPersistedQuestionReturnTarget(options.cwd, options.sessionId))
        return 'inside-tmux';
...
function resolveQuestionUiProcessArgs(recordPath, options) {
    ...
    if (!omxBin)
        throw new Error('Unable to resolve OMX CLI entry path for question UI launch.');
    return [omxBin, 'question', '--ui', '--state-path', recordPath];
}
function buildQuestionUiTmuxArgs(recordPath, options) {
    return [
...
function buildQuestionUiProcessEnv(baseEnv, options) {
    return {
        ...baseEnv,
...
function buildWindowsConsoleStartCommand(command, args) {
    return [
        'start',
        '"OMX Question"',
        '/wait',
...
function defaultSpawnDetachedRenderer(command, args, options) {
    return spawn(command, args, options);
}
function defaultExecTmux(args) {
    const tmux = resolveTmuxBinaryForPlatform();
    if (!tmux)
        throw new Error('tmux is unavailable; omx question requires tmux for OMX-owned question UI rendering.');
...
function readPersistedQuestionReturnTarget(cwd, sessionId) {
...
function isCurrentTmuxSessionAttached(execTmux = defaultExecTmux, env = process.env) {
...
export function isLaunchedQuestionPaneAlive(paneId, execTmux) {
    if (!isPaneId(paneId))
        return false;
    try {
...
export function isLaunchedQuestionSessionAlive(sessionName, execTmux) {
    if (!safeString(sessionName).trim())
        return false;
    try {
...
export function isWindowsConsoleRendererAlive(pid) {
...
export function isQuestionRendererAlive(renderer, execTmux = defaultExecTmux) {
    if (!renderer)
        return true;
    if (renderer.renderer === 'tmux-pane')
        return isLaunchedQuestionPaneAlive(renderer.target, execTmux);
    ...
    if (renderer.renderer === 'windows-console')
        return isWindowsConsoleRendererAlive(renderer.pid);
    return true;
}
export function formatQuestionAnswerForInjection(answer) {
...
export function injectQuestionAnswerToPane(paneId, answer, execTmux = defaultExecTmux, sleepImpl = sleepSync) {
    if (!isPaneId(paneId))
        return false;
...
export function launchQuestionRenderer(options, deps = {}) {
    ...
    if (strategy === 'unsupported') {
        throw new Error('omx question cannot open a visible renderer because this process is outside an attached tmux pane and has no explicit tmux return bridge. Codex App/outside-tmux sessions need an attached tmux OMX CLI session or OMX_QUESTION_RETURN_PANE bridge. Run omx question from inside tmux.');
    }
    ...
    if (strategy === 'inside-tmux') {
        ...
        if (splitTarget.length === 0 && !isCurrentTmuxSessionAttached(execTmux, env)) {
            throw new Error('omx question cannot open a visible renderer because this tmux session has no attached client. Run omx question from an attached tmux pane.');
        }
        ...
        if (!paneId)
            throw new Error('Failed to create tmux split pane for omx question UI.');
    ...
    if (strategy === 'windows-console') {
        ...
        const child = spawnDetachedRenderer(env.ComSpec || 'cmd.exe', ['/d', '/s', '/c', buildWindowsConsoleStartCommand(process.execPath, uiArgs)], {
            cwd: options.cwd,
    ...
    const exhaustiveStrategy = strategy;
