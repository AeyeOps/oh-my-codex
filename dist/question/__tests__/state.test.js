...
import { createQuestionRecord, getQuestionRecordPath, markQuestionAnswered, markQuestionPrompting, markQuestionTerminalError, readQuestionRecord, waitForQuestionTerminalState, } from '../state.js';
const tempDirs = [];
...
describe('question state', () => {
    ...
    it('persists explicit terminal errors after prompting begins', async () => {
        ...
        await markQuestionTerminalError(recordPath, 'error', 'question_runtime_failed', 'Question UI pane %42 disappeared immediately after launch.');
        const loaded = await readQuestionRecord(recordPath);
