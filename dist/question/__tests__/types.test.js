...
describe('normalizeQuestionInput', () => {
    ...
    it('rejects conflicting explicit single-answerable type', () => {
        assert.throws(() => normalizeQuestionInput({ question: 'Pick one', options: ['A'], allow_other: false, type: 'single-answerable', multi_select: true }), /type=single-answerable conflicts with multi_select=true/);
    });
    it('rejects empty options when allow_other is false', () => {
        assert.throws(() => normalizeQuestionInput({ question: 'Pick one', options: [], allow_other: false }), /options must be a non-empty array unless allow_other is true/);
    });
});
