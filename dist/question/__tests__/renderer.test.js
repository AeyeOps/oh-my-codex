...
import { formatQuestionAnswerForInjection, injectQuestionAnswerToPane, launchQuestionRenderer, resolveQuestionRendererStrategy, } from '../renderer.js';
...
describe('resolveQuestionRendererStrategy', () => {
    it('prefers inside-tmux when TMUX is present', () => {
        assert.equal(resolveQuestionRendererStrategy({ TMUX: '/tmp/tmux-demo' }, '/usr/bin/tmux'), 'inside-tmux');
    });
    it('fails closed when tmux exists but TMUX is absent', () => {
        assert.equal(resolveQuestionRendererStrategy({}, '/usr/bin/tmux'), 'unsupported');
    });
    it('uses inline-tty on Windows when no tmux bridge exists but the current terminal is interactive', () => {
        assert.equal(resolveQuestionRendererStrategy({}, '/usr/bin/tmux', { platform: 'win32', stdinIsTTY: true, stdoutIsTTY: true }), 'inline-tty');
    });
    it('supports explicit host-pane bridge hints when TMUX is absent', () => {
        assert.equal(resolveQuestionRendererStrategy({ OMX_QUESTION_RETURN_PANE: '%77' }, '/usr/bin/tmux'), 'inside-tmux');
        assert.equal(resolveQuestionRendererStrategy({ OMX_LEADER_PANE_ID: '%88' }, '/usr/bin/tmux'), 'inside-tmux');
    });
    it('uses a detached Windows console for native psmux return bridges', () => {
        assert.equal(resolveQuestionRendererStrategy({ TMUX: 'psmux-session', TMUX_PANE: '%44' }, 'C:/Program Files/psmux/psmux.exe', { platform: 'win32' }), 'windows-console');
        assert.equal(resolveQuestionRendererStrategy({ OMX_QUESTION_RETURN_PANE: '%45' }, 'C:/Program Files/psmux/psmux.exe', { platform: 'win32' }), 'windows-console');
    });
    it('supports persisted workflow pane bridges when TMUX is absent', () => {
        ...
        try {
            ...
            assert.equal(resolveQuestionRendererStrategy({}, '/usr/bin/tmux', { cwd, sessionId: 'sess-stateful' }), 'inside-tmux');
        }
        finally {
    ...
    it('rejects malformed explicit host-pane bridge hints', () => {
        assert.equal(resolveQuestionRendererStrategy({ OMX_QUESTION_RETURN_PANE: 'not-a-pane' }, '/usr/bin/tmux'), 'unsupported');
    });
    it('uses noop test renderer override when requested', () => {
        assert.equal(resolveQuestionRendererStrategy({ OMX_QUESTION_TEST_RENDERER: 'noop' }, '/usr/bin/tmux'), 'test-noop');
    });
    it('fails closed when neither attached tmux nor tmux binary exists', () => {
        assert.equal(resolveQuestionRendererStrategy({}, undefined), 'unsupported');
    });
});
describe('launchQuestionRenderer', () => {
    it('fails before building UI argv or invoking tmux when no visible renderer is available', () => {
        ...
        process.argv[1] = '';
        try {
            assert.throws(() => launchQuestionRenderer({
                cwd: '/repo',
    ...
    it('targets an explicit host pane when launching from a container without TMUX', () => {
        ...
        const result = launchQuestionRenderer({
            ...
            sessionId: 's1',
            nowIso: '2026-04-19T00:00:00.000Z',
            env: { OMX_QUESTION_RETURN_PANE: '%77' },
        }, {
            execTmux: (args) => {
                calls.push(args);
                if (args[0] === 'split-window')
    ...
    it('opens a detached Windows console instead of a psmux split pane when a return bridge is present', () => {
        ...
        const result = launchQuestionRenderer({
            ...
            env: { TMUX: 'psmux-session', TMUX_PANE: '%44' },
            platform: 'win32',
        }, {
            execTmux: (args) => {
                tmuxCalls.push(args);
    ...
    it('resolves the leader return target before opening the question pane', () => {
        ...
        try {
            ...
            if (typeof originalTmuxPane === 'string')
...
describe('question answer injection', () => {
    ...
    it('injects the answered text back into the requester pane and submits with isolated double C-m', () => {
        const calls = [];
        const sleeps = [];
        const ok = injectQuestionAnswerToPane('%11', {
            kind: 'option',
            value: 'proceed',
