...
import { OmxQuestionError } from '../client.js';
import { reconcileDeepInterviewQuestionEnforcementFromAnsweredRecords, runDeepInterviewQuestion, } from '../deep-interview.js';
const tempDirs = [];
async function makeRepo() {
    ...
    await writeFile(join(cwd, '.omx', 'state', 'session.json'), JSON.stringify({ session_id: 'sess-di' }, null, 2));
    await writeFile(join(cwd, '.omx', 'state', 'sessions', 'sess-di', 'deep-interview-state.json'), JSON.stringify({
        active: true,
        mode: 'deep-interview',
...
describe('runDeepInterviewQuestion', () => {
    it('tracks a pending obligation before omx question returns and satisfies it afterward', async () => {
        ...
        const runner = async () => {
            const inFlightState = JSON.parse(await readFile(statePath, 'utf-8'));
    ...
    it('clears the pending obligation when omx question fails after being attempted', async () => {
        const cwd = await makeRepo();
        const statePath = join(cwd, '.omx', 'state', 'sessions', 'sess-di', 'deep-interview-state.json');
        await assert.rejects(runDeepInterviewQuestion({
            question: 'What should happen next?',
            options: [{ label: 'Launch', value: 'launch' }],
            allow_other: false,
        }, {
    ...
    it('reconciles a pending obligation from an already-answered same-session question record', async () => {
        ...
        await writeFile(join(questionsDir, 'question-answered.json'), JSON.stringify({
        ...
        const reconciled = await reconcileDeepInterviewQuestionEnforcementFromAnsweredRecords(cwd, 'sess-di', new Date('2026-04-19T00:00:21.000Z'));
...
//# sourceMappingURL=deep-interview.test.js.map