...
import { compatibilityRunOutcomeFromTerminalLifecycleOutcome, inferTerminalLifecycleOutcome, normalizeTerminalLifecycleOutcome, preferredRunOutcomeForLifecycleOutcome, } from '../terminal-lifecycle.js';
...
describe('run outcome contract', () => {
    ...
    it('keeps terminal lifecycle compatibility helpers aligned with the shared run outcome contract', () => {
        for (const outcome of ['finished', 'blocked', 'failed', 'userinterlude', 'askuserQuestion']) {
            assert.equal(preferredRunOutcomeForLifecycleOutcome(outcome), compatibilityRunOutcomeFromTerminalLifecycleOutcome(outcome));
        }
    });
});
