...
describe("getOpenClawConfig", () => {
    ...
    it("reads config from OMX_OPENCLAW_CONFIG override file", async () => {
        ...
        const config = {
            enabled: true,
            gateways: {
                myGateway: { type: "http", url: "https://example.com/hook" },
            },
            hooks: {
                "session-start": {
                    gateway: "myGateway",
...
describe("resolveGateway", () => {
    it("returns null when event not in hooks", async () => {
        ...
        const config = {
            enabled: true,
            gateways: { gw: { url: "https://example.com", type: "http" } },
            hooks: {},
        };
    ...
    it("resolves an HTTP gateway", async () => {
        ...
        const config = {
            enabled: true,
            gateways: {
                gw: { url: "https://example.com/hook", type: "http" },
            },
            hooks: {
                "session-start": {
                    gateway: "gw",
