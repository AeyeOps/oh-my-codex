...
import { validateGatewayUrl, shellEscapeArg, interpolateInstruction, isCommandGateway, resolveCommandTimeoutMs, } from '../dispatcher.js';
...
describe('interpolateInstruction - reply context variables', () => {
    ...
    it('replaces reply variables with empty string when undefined', () => {
        const result = interpolateInstruction('{{replyChannel}} {{replyTarget}} {{replyThread}}', { replyChannel: undefined, replyTarget: undefined, replyThread: undefined });
        assert.equal(result, '  ');
    });
    it('replaces all reply variables together', () => {
        const result = interpolateInstruction('notify {{replyTarget}} in {{replyChannel}} thread {{replyThread}}', { replyChannel: '#dev', replyTarget: 'user42', replyThread: 'ts-999' });
...
describe('wakeCommandGateway - command gate', () => {
    ...
    it('uses env timeout when gateway timeout is not set', async () => {
        ...
        const result = await wakeCommandGateway('test', { type: 'command', command: "node -e \"setTimeout(() => {}, 250)\"" }, {});
        assert.equal(result.success, false);
    ...
    it('uses gateway timeout over env timeout', async () => {
        ...
        process.env.OMX_OPENCLAW_COMMAND = '1';
        process.env.OMX_OPENCLAW_COMMAND_TIMEOUT_MS = '100';
        const result = await wakeCommandGateway('test', { type: 'command', command: "node -e \"setTimeout(() => {}, 250)\"", timeout: 500 }, {});
        assert.equal(result.success, true);
    });
});
