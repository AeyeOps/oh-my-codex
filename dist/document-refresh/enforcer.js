...
import { DEFAULT_DOCUMENT_REFRESH_RULES } from "./config.js";
...
function isRenameOnly(record) {
    return /^R100$/u.test(record.status.trim());
}
function isTriggerOnlyExcluded(record) {
...
export function hasDocumentRefreshExemption(text) {
    if (!text)
        return false;
...
function changedPathCandidates(record) {
    return unique([record.path, record.previousPath ?? ""].filter(Boolean));
}
function recordMatchesRuleSource(record, rule) {
    if (isRenameOnly(record))
        return false;
    if (isTriggerOnlyExcluded(record))
        return false;
...
function hasRuleRefresh(changes, localFreshTargets, rule) {
...
export function evaluateDocumentRefresh(input) {
    if (hasDocumentRefreshExemption(input.exemptionText))
        return null;
    ...
    for (const rule of rules) {
        ...
        if (changedPaths.length === 0)
            continue;
        if (hasRuleRefresh(changes, localFreshTargets, rule))
            continue;
        warnings.push({
            ruleId: rule.id,
...
export function isFinalHandoffDocumentRefreshCandidate(text) {
    const message = text?.trim() ?? "";
    if (!message)
        return false;
    if (hasDocumentRefreshExemption(message))
        return true;
    return FINAL_HANDOFF_MARKER_PATTERNS.some((pattern) => pattern.test(message));
}
export function buildDocumentRefreshAdvisoryOutput(warning, hookEventName) {
    return {
        hookSpecificOutput: {
            hookEventName,
...
export function formatDocumentRefreshWarning(warning) {
...
function readGitNameStatus(cwd, args) {
    try {
...
export function readStagedGitChanges(cwd) {
    return readGitNameStatus(cwd, ["diff", "--cached", "--name-status"]);
}
export function readStagedAndUnstagedGitChanges(cwd) {
...
function latestExistingMtimeMs(cwd, paths) {
    let latest = 0;
...
export function findFreshLocalPlanningTargets(cwd, changes, rules = DEFAULT_DOCUMENT_REFRESH_RULES) {
    const triggeringPathsByRule = new Map();
    for (const rule of rules) {
        const triggering = changes
            .filter((record) => recordMatchesRuleSource(record, rule))
            .flatMap(changedPathCandidates)
    ...
    for (const rule of rules) {
        ...
        if (latestTriggerMtime === 0)
            continue;
        for (const target of localPlanningFiles) {
            if (!pathMatchesAny(target, rule.refreshTargets))
                continue;
            try {
                const targetMtime = statSync(join(cwd, target)).mtimeMs;
                if (targetMtime >= latestTriggerMtime)
                    fresh.add(target);
            }
            catch {
...
export function evaluateStagedDocumentRefresh(cwd, exemptionText) {
...
export function evaluateFinalHandoffDocumentRefresh(cwd, exemptionText) {
