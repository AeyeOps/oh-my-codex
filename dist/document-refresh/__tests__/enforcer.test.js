...
import { DOCUMENT_REFRESH_EXEMPTION_PREFIX, evaluateDocumentRefresh, findFreshLocalPlanningTargets, isFinalHandoffDocumentRefreshCandidate, parseGitNameStatus, } from "../enforcer.js";
function warningFor(changes, options = {}) {
...
describe("document refresh evaluator", () => {
    ...
    it("commit path does not suppress on ignored local-only planning spec evidence", () => {
        const warning = warningFor([{ status: "M", path: "src/scripts/codex-native-hook.ts" }], { scope: "commit", localFreshTargets: [".omx/plans/prd-document-refresh-enforcer.md"] });
        assert.ok(warning);
    });
    ...
    it("suppresses explicit exemption", () => {
        const warning = warningFor([{ status: "M", path: "src/scripts/codex-native-hook.ts" }], { exemptionText: `${DOCUMENT_REFRESH_EXEMPTION_PREFIX} internal-only behavior verified` });
        assert.equal(warning, null);
    });
...
//# sourceMappingURL=enforcer.test.js.map