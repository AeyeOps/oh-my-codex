...
async function withTempTeamStateRoot(teamStateRoot, fn) {
    ...
    try {
        ...
        if (previousRoot === undefined)
            ...
            process.env.OMX_TEAM_STATE_ROOT = previousRoot;
    }
}
...
describe('rust runtime legacy-reader compatibility', () => {
    ...
    it('keeps doctor --team on the manifest-authored tmux session', async () => {
        ...
        try {
            ...
            await withTempTeamStateRoot(teamStateRoot, async () => {
                ...
                const result = runOmx(wd, ['doctor', '--team'], { PATH: `${fakeBin}:${process.env.PATH || ''}`, OMX_TEAM_STATE_ROOT: teamStateRoot });
                if (shouldSkipForSpawnPermissions(result.error))
                    return;
                assert.equal(result.status, 0, result.stderr || result.stdout);
    ...
    it('keeps HUD team state on the session-scoped compatibility file', async () => {
        ...
        try {
            ...
            await writeFile(join(stateRoot, 'team-state.json'), JSON.stringify({
                active: false,
    ...
    it('prefers bridge-authored dispatch/mailbox compatibility views over stale legacy files', async () => {
        ...
        try {
            ...
            await withTempTeamStateRoot(teamStateRoot, async () => {
                ...
                await writeFile(join(teamDir, 'dispatch', 'requests.json'), JSON.stringify([{ request_id: 'legacy-only', kind: 'mailbox', team_name: 'rust-compat-bridge', to_worker: 'worker-2', trigger_message: 'legacy', status: 'pending', attempt_count: 0, created_at: '2026-04-01T00:00:00.000Z', updated_at: '2026-04-01T00:00:00.000Z', message_id: 'legacy-msg' }], null, 2));
                await writeFile(join(teamDir, 'mailbox', 'worker-2.json'), JSON.stringify({ worker: 'worker-2', messages: [{ message_id: 'legacy-msg', from_worker: 'worker-1', to_worker: 'worker-2', body: 'legacy body', created_at: '2026-04-01T00:00:00.000Z' }] }, null, 2));
                const queued = await enqueueDispatchRequest('rust-compat-bridge', { kind: 'mailbox', to_worker: 'worker-2', message_id: 'bridge-msg', trigger_message: 'bridge trigger' }, wd);
...
//# sourceMappingURL=rust-runtime-compat.test.js.map