...
const OMX_TUI_STATUS_LINE = 'status_line = ["model-with-reasoning", "git-branch", "context-remaining", "total-input-tokens", "total-output-tokens", "five-hour-limit", "weekly-limit"]';
...
function upsertEnvSettings(config) {
    ...
    if (exploreRoutingIdx < 0) {
        lines.splice(sectionEnd, 0, `${OMX_EXPLORE_CMD_ENV} = "${OMX_EXPLORE_ROUTING_DEFAULT}"`);
    }
    return lines.join("\n");
}
...
export function stripOmxEnvSettings(config) {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (i > envStart && i < sectionEnd) {
            const isOmxEnvKey = new RegExp(`^\\s*${OMX_EXPLORE_CMD_ENV}\\s*=`).test(lines[i]);
            if (isOmxEnvKey)
                continue;
        }
        filtered.push(lines[i]);
...
function upsertTuiStatusLine(config) {
    const lines = config.split(/\r?\n/);
    const sections = [];
    for (let i = 0; i < lines.length; i++) {
 ...
 */
function getOmxTablesBlock(pkgRoot, includeTui = true) {
    const lines = [
        "",
        "# ============================================================",
...
export function buildMergedConfig(existingConfig, pkgRoot, options = {}) {
    ...
    const tablesBlock = getOmxTablesBlock(pkgRoot, includeTui && !tuiUpsert.hadExistingTui);
