...
function assertSingleOmxBlock(toml) {
    ...
    assert.equal(count(toml, /^# End oh-my-codex$/gm), 1, "End marker should appear once");
    ...
    assert.equal(count(toml, /^\[mcp_servers\.omx_wiki\]$/gm), 1, "[mcp_servers.omx_wiki] should appear once");
    assert.equal(count(toml, /^\[mcp_servers\.omx_team_run\]$/gm), 0, "[mcp_servers.omx_team_run] should not be emitted");
    assert.doesNotMatch(toml, /dist\/mcp\/team-server\.js/, "team-server path should not be emitted");
    assert.equal(count(toml, /^\[tui\]$/gm), 1, "[tui] should appear once");
    assert.equal(count(toml, /^\[features\]$/gm), 1, "[features] should appear once");
    assert.equal(count(toml, /^codex_hooks = true$/gm), 1, "codex_hooks should appear once");
    ...
    assert.equal(count(toml, /^USE_OMX_EXPLORE_CMD = "1"$/gm), 1, "USE_OMX_EXPLORE_CMD should appear once");
}
describe("config generator idempotency (#384)", () => {
    ...
    it("preserves a user-owned status_line in an existing [tui] section", async () => {
        ...
        try {
            ...
            assert.match(toml, /^status_line = \["git-branch"\]$/m, "user status_line preserved");
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
    ...
    it("seeds the default status_line into a fresh [tui] section", () => {
        ...
        assert.equal(count(toml, /^\[tui\]$/gm), 1, "[tui] should appear once");
        assert.match(toml, /^status_line = \["model-with-reasoning", "git-branch", "context-remaining", "total-input-tokens", "total-output-tokens", "five-hour-limit", "weekly-limit"\]$/m);
    });
    it("preserves a customized managed-block status_line when refreshing setup", () => {
        const firstRun = buildMergedConfig("", "/tmp/omx");
        const customized = firstRun.replace(/^status_line = \["model-with-reasoning", "git-branch", "context-remaining", "total-input-tokens", "total-output-tokens", "five-hour-limit", "weekly-limit"\]$/m, 'status_line = ["git-branch", "context-remaining"]');
        const refreshed = buildMergedConfig(customized, "/tmp/omx");
        assert.equal(count(refreshed, /^\[tui\]$/gm), 1, "[tui] should appear once");
        assert.match(refreshed, /^status_line = \["git-branch", "context-remaining"\]$/m, "customized status_line should survive managed-block stripping");
        assert.doesNotMatch(refreshed, /^status_line = \["model-with-reasoning", "git-branch", "context-remaining", "total-input-tokens", "total-output-tokens", "five-hour-limit", "weekly-limit"\]$/m, "default status_line should not overwrite customization");
    });
    ...
    it('preserves existing [env] keys and explicit explore routing opt-outs', () => {
        const toml = buildMergedConfig(['[env]', 'FOO = "bar"', 'USE_OMX_EXPLORE_CMD = "0"', ''].join('\n'), '/tmp/omx');
        assert.match(toml, /^\[env\]$/m);
    ...
    it("seeds missing auto compact limit without overwriting an existing context window", async () => {
        ...
        try {
            const configPath = join(wd, "config.toml");
            await writeFile(configPath, ['model = "gpt-5.5"', "model_context_window = 640000", ""].join("\n"));
            await mergeConfig(configPath, wd);
            const toml = await readFile(configPath, "utf-8");
            assert.match(toml, /^model = "gpt-5\.5"$/m);
    ...
    it("seeds missing context window without overwriting an existing auto compact limit", async () => {
        ...
        try {
            const configPath = join(wd, "config.toml");
            await writeFile(configPath, ['model = "gpt-5.5"', "model_auto_compact_token_limit = 150000", ""].join("\n"));
            await mergeConfig(configPath, wd);
            const toml = await readFile(configPath, "utf-8");
            assert.match(toml, /^model = "gpt-5\.5"$/m);
            assert.match(toml, /^model_context_window = 250000$/m);
    ...
    it("does not duplicate seeded model defaults across reruns", async () => {
        ...
        try {
            ...
            await mergeConfig(configPath, wd);
            const toml = await readFile(configPath, "utf-8");
            assert.equal(count(toml, /^model = "gpt-5\.5"$/gm), 1, "seeded model should appear once");
    ...
    it("syncs shared MCP registry entries in a dedicated managed block", async () => {
        ...
        try {
            ...
            assert.equal(count(second, /oh-my-codex \(OMX\) Shared MCP Registry Sync/g), 1, "shared MCP sync block should appear once");
            assert.equal(count(second, /^\[mcp_servers\.eslint\]$/gm), 1, "shared eslint MCP table should appear once");
    ...
    it("preserves explicit launcher timeouts and leaves non-launcher MCP servers untouched", () => {
        ...
        assert.doesNotMatch(merged, /\[mcp_servers\.custom\][\s\S]*?startup_timeout_sec = 15/);
    });
