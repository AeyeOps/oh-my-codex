...
import { getUnifiedMcpRegistryCandidates, loadUnifiedMcpRegistry, planClaudeCodeMcpSettingsSync, } from "../mcp-registry.js";
describe("unified MCP registry loader", () => {
    it("prefers ~/.omx/mcp-registry.json over ~/.omc/mcp-registry.json", async () => {
        ...
        try {
            ...
            await writeFile(omxPath, JSON.stringify({
                eslint: { command: "npx", args: ["@eslint/mcp@latest"], timeout: 11 },
            }));
            await writeFile(omcPath, JSON.stringify({
    ...
    it("plans Claude settings sync by adding only missing shared servers", () => {
        const plan = planClaudeCodeMcpSettingsSync(JSON.stringify({
            theme: "dark",
            mcpServers: {
                existing_server: {
    ...
    it("includes approval_mode when adding missing Claude MCP servers", () => {
        const plan = planClaudeCodeMcpSettingsSync(JSON.stringify({ mcpServers: {} }, null, 2), [
            {
                name: "eslint",
                command: "npx",
                args: ["@eslint/mcp@latest"],
    ...
    it('warns when Claude settings.json has a non-object "mcpServers" field', () => {
        const plan = planClaudeCodeMcpSettingsSync(JSON.stringify({ mcpServers: [] }), [
            {
                name: "eslint",
                command: "npx",
                args: ["@eslint/mcp@latest"],
