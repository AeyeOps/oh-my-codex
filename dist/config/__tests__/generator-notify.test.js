...
describe('config generator', () => {
    it('places top-level keys before [features]', async () => {
        ...
        try {
            ...
            const seededStartIdx = toml.indexOf('# oh-my-codex seeded behavioral defaults (uninstall removes unchanged defaults)');
            ...
            assert.ok(seededStartIdx < featuresIdx, 'seeded defaults start marker must come before [features]');
            ...
            assert.ok(seededEndIdx < featuresIdx, 'seeded defaults end marker must come before [features]');
        }
        finally {
    ...
    it('migrates a legacy OMX block and preserves user settings', async () => {
        ...
        try {
            ...
            await mergeConfig(configPath, wd);
            const toml = await readFile(configPath, 'utf-8');
            assert.equal((toml.match(/oh-my-codex \(OMX\) Configuration/g) ?? []).length, 1);
    ...
    it('escapes Windows-style backslashes for MCP server args', async () => {
        ...
        try {
            ...
            const toml = await readFile(configPath, 'utf-8');
            assert.match(toml, /args = \["C:\\\\Users\\\\alice\\\\oh-my-codex\/dist\/mcp\/state-server\.js"\]/);
            assert.match(toml, /args = \["C:\\\\Users\\\\alice\\\\oh-my-codex\/dist\/mcp\/memory-server\.js"\]/);
            assert.match(toml, /args = \["C:\\\\Users\\\\alice\\\\oh-my-codex\/dist\/mcp\/code-intel-server\.js"\]/);
            assert.match(toml, /args = \["C:\\\\Users\\\\alice\\\\oh-my-codex\/dist\/mcp\/trace-server\.js"\]/);
        }
        finally {
            await rm(wd, { recursive: true, force: true });
        }
