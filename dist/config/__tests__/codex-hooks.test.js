...
import { buildManagedCodexHooksConfig, getMissingManagedCodexHookEvents, mergeManagedCodexHooksConfig, removeManagedCodexHooks, } from "../codex-hooks.js";
describe("codex hooks helpers", () => {
    it("merges managed wrappers without dropping user hooks", () => {
        ...
        assert.equal(sessionStart.flatMap((entry) => entry.hooks ?? []).filter((hook) => String(hook.command ?? "").includes("codex-native-hook.js")).length, 1);
    ...
    it("reports missing managed hook coverage by event", () => {
        const missing = getMissingManagedCodexHookEvents(JSON.stringify({
            hooks: {
                SessionStart: [
                    {
                        hooks: [
