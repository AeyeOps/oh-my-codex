...
function toClaudeCodeMcpServerConfig(server) {
    return {
...
function normalizeTimeout(value, name, warnings) {
    if (value === undefined)
        return undefined;
...
function normalizeApprovalMode(value, name, warnings) {
    if (value === undefined)
        return undefined;
    if (typeof value !== "string") {
        warnings.push(`registry entry "${name}" has non-string approval_mode; ignoring approval_mode`);
        return undefined;
    }
    return value;
}
function normalizeEntry(name, value, warnings) {
    if (!isRecord(value)) {
    ...
    const timeoutCandidate = value.timeout ?? value.startup_timeout_sec ?? value.startupTimeoutSec;
...
export function getUnifiedMcpRegistryCandidates(homeDir = homedir()) {
    return [join(homeDir, ".omx", "mcp-registry.json")];
}
export function getLegacyUnifiedMcpRegistryCandidate(homeDir = homedir()) {
    return join(homeDir, ".omc", "mcp-registry.json");
}
export async function loadUnifiedMcpRegistry(options = {}) {
    const candidates = options.candidates ?? getUnifiedMcpRegistryCandidates(options.homeDir);
...
export function planClaudeCodeMcpSettingsSync(existingContent, servers) {
    if (servers.length === 0) {
